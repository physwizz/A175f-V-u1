...
static void s2mf301_usbpd_test_read(struct s2mf301_usbpd_data *usbpd_data)
{
	...
	static int reg_list[] = {0x00, 0x01, 0x0a, 0x15, 0x18, 0x1b, 0x26, 0x27, 0x28, 0x2e,
		0x2f, 0x40, 0x90, 0xe2, 0xb2, 0xb3, 0xb4, 0xf7};
	u8 data = 0;
	char str[1016] = {0,};
...
static int s2mf301_usbpd_update_reg(struct i2c_client *i2c,
		u8 reg, u8 val, u8 mask)
{
	int ret;
	u8 old_val = 0, new_val = 0;

	ret = s2mf301_usbpd_read_reg(i2c, reg, &old_val);
	if (ret < 0) {
		s2mf301_err("%s: reg(0x%x), ret(%d)\n", __func__, reg, ret);
		return ret;
	}
	new_val = (val & mask) | (old_val & (~mask));

	ret = s2mf301_usbpd_write_reg(i2c, reg, new_val);
	if (ret < 0) {
		s2mf301_err("%s: reg(0x%x), mask(0x%x), val(0x%x)\n", __func__, reg, mask, val);
	}
...
static void s2mf301_usbpd_self_reset(struct s2mf301_usbpd_data *pdic_data)
{
	...
	/* LPM 1 -> 0, [2]TYPEC_ENB clear */
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_PD_CTRL,
			S2MF301_REG_LPM_EN,
			S2MF301_REG_LPM_EN | S2MF301_REG_TYPEC_ENB);
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_PD_CTRL,
			0x00,
			S2MF301_REG_LPM_EN | S2MF301_REG_TYPEC_ENB);

	/* CCwakeup 0->1->0 */
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_ANALOG_OTP_08, 0x00, 0x20);
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_ANALOG_OTP_08, 0x20, 0x20);
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_ANALOG_OTP_08, 0x00, 0x20);

	/* VBUSwakeup 0->1->0, [6:5]SLEWTEST clear*/
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_PD_TRIM,
			0x00,
			S2MF301_REG_VBUS_WAKEUP_DIS | S2MF301_REG_SLEWTEST);
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_PD_TRIM,
			S2MF301_REG_VBUS_WAKEUP_DIS,
			S2MF301_REG_VBUS_WAKEUP_DIS | S2MF301_REG_SLEWTEST);
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_PD_TRIM,
			0x00,
			S2MF301_REG_VBUS_WAKEUP_DIS | S2MF301_REG_SLEWTEST);
	...
	PDIC_VBUS_WAKEUP_SEL sel)
{
	if (sel == VBUS_WAKEUP_ENABLE)
		s2mf301_usbpd_update_reg(pdic_data->i2c, S2MF301_REG_PD_TRIM,
				0x00,
				S2MF301_REG_VBUS_WAKEUP_DIS | S2MF301_REG_SLEWTEST);
...
static int s2mf301_usbpd_reg_init(struct s2mf301_usbpd_data *_data)
{
	...
	/* enable support acc, [6:1] clear */
	s2mf301_usbpd_update_reg(i2c, S2MF301_REG_PLUG_CTRL_PD_HOLD, 0x80, 0xFE);
