...
static u32 mtk_dsi_recv_cnt(u8 type, u8 *read_data)
{
	...
	case MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT:
		DDPMSG("type is 0x02, try again\n");
		break;
	default:
		DDPMSG("type(0x%x) cannot be non-recognite\n", type);
		break;
	}
				...
				struct mipi_dsi_msg *msg)
{
	...
	DDPMSG("%s type=0x%x, tx_len=%d, tx_buf[0]=0x%x, rx_len=%d\n",
...
#if 0//defined(CUSTOMER_USE_SIMPLE_API)
	if ((!(msg->flags & MIPI_DSI_MSG_USE_LPM)))
		t0.CONFG |= HSTX;
#endif
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	DDPMSG("[DSI]packet_type~recv_data_cnt = 0x%x~0x%x\n",
	...
	DDPMSG("dsi get %d byte data from the panel address(0x%x)\n", recv_data_cnt,
		*((u8 *)(msg.tx_buf)));
