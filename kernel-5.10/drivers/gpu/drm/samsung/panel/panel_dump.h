...
typedef int (*dump_show_t)(struct panel_device *panel, struct dumpinfo *dump);
...
#define call_dump_func(_panel, _dump) \
	((_dump) && (_dump)->ops.show && (_dump)->ops.show->symaddr ? \
	 ((dump_show_t)(_dump)->ops.show->symaddr)(_panel, _dump) : -EINVAL)
