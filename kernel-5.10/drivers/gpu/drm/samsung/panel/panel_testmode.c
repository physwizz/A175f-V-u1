...
void panel_testmode_on(struct panel_device *panel)
{
	if (panel->testmode.enabled) {
		panel_drv_info(panel, "[TESTMODE] testmode is already ON.\n");
		return;
	}
	panel_drv_info(panel, "[TESTMODE] testmode set to: ON\n");
...
{
	if (!panel->testmode.enabled) {
		panel_drv_info(panel, "[TESTMODE] testmode is already OFF.\n");
		return;
	}
	panel_drv_info(panel, "[TESTMODE] testmode set to: OFF\n");
...
{
	panel_drv_info(panel, "[TESTMODE] panel test mode is ON\n");
	return ret;
}
...
int panel_testmode_command(struct panel_device *panel, char *buf)
{
	...
	if (!command) {
		panel_drv_err(panel, "[TESTMODE] command is null\n");
		return -EINVAL;
	}
	...
	if (i == ARRAY_SIZE(panel_testmode_sets)) {
		panel_drv_err(panel, "[TESTMODE] unknown command: %s\n", command);
		return -EINVAL;
	}

	if (panel_testmode_sets[i].func_arg) {
		if (!pos) {
			panel_drv_err(panel, "[TESTMODE] command %s: need 1 argument\n", command);
			return -EINVAL;
		}
		rc = kstrtoint(pos, 10, &intarg);
		if (rc < 0) {
			panel_drv_err(panel, "[TESTMODE] command %s: need integer argument\n", command);
			return -EINVAL;
		}
	...
	panel_drv_err(panel, "[TESTMODE] command %s: invalid function table\n", command);
	return -EINVAL;
}
