...
int panel_property_update(struct panel_device *panel, char *name)
{
	...
	if (!property) {
		panel_drv_err(panel, "property is not exist(%s).\n", name);
		return -EINVAL;
	}
...
int panel_get_property_value(struct panel_device *panel, char *name)
{
	...
	if (property->update) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to update property(%s)\n", name);
			return ret;
		}
	}
		...
		int (*update)(struct panel_property *))
{
	...
	if (panel_find_property(panel, name)) {
		panel_drv_err(panel, "same name property is exist.\n");
		return -EINVAL;
	}
	...
	panel_drv_dbg(panel, "added: %s init_value: %d.\n",
			get_panel_property_name(property), property->value);
		...
		int num_values, int (*update)(struct panel_property *))
{
	...
	panel_drv_dbg(panel, "added: %s init_enum: %d.\n",
			get_panel_property_name(property), property->value);
		...
		struct panel_prop_list *prop_array, size_t num_prop_array)
{
	...
	for (i = 0; i < num_prop_array; i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to add property(%s:%s)\n",
		...
		struct panel_prop_list *prop_array, size_t num_prop_array)
{
	...
	for (i = 0; i < num_prop_array; i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to delete property(%s:%s)\n",
					prop_type_to_string(prop_array[i].type),
					prop_array[i].name);
			return ret;
		}
