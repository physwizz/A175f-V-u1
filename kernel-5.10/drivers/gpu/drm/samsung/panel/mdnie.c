...
#define PANEL_MDNIE_DRIVER_NAME "panel-mdnie"
#define panel_mdnie_info(_mdnie, fmt, ...)   \
	usdm_pr_info(PANEL_MDNIE_DRIVER_NAME, ((_mdnie) ? (_mdnie)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_mdnie_warn(_mdnie, fmt, ...)   \
	usdm_pr_warn(PANEL_MDNIE_DRIVER_NAME, ((_mdnie) ? (_mdnie)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_mdnie_err(_mdnie, fmt, ...)    \
	usdm_pr_err(PANEL_MDNIE_DRIVER_NAME, ((_mdnie) ? (_mdnie)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_mdnie_dbg(_mdnie, fmt, ...)\
	usdm_pr_debug(PANEL_MDNIE_DRIVER_NAME, ((_mdnie) ? (_mdnie)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)
		...
		u32 *property, unsigned int value)
{
	...
	if (!property) {
		panel_mdnie_err(mdnie, "property is null\n");
		return -EINVAL;
	}
	...
	if (!propname) {
		panel_mdnie_err(mdnie, "unknown property\n");
		return -EINVAL;
	}

	if (mdnie_set_property_value(mdnie,
				propname, value) < 0) {
		panel_mdnie_warn(mdnie, "failed to set property(%s) %d\n",
				propname, value);
		return -EINVAL;
	}
	*property = value;
...
int mdnie_current_state(struct mdnie_info *mdnie)
{
	...
	mdnie_mode == MDNIE_LIGHT_NOTIFICATION_MODE ||
	mdnie_mode == MDNIE_HMD_MODE)) {
		panel_mdnie_dbg(mdnie, "block mdnie (%s->%s) in doze mode\n",
...
char *mdnie_get_sequence_name(struct mdnie_info *mdnie)
{
	...
	default:
		seqname = NULL;
		panel_mdnie_err(mdnie, "unknown mdnie\n");
		break;
	}

	if (seqname) {
		panel_mdnie_dbg(mdnie, "mdnie mode:%s(%d), seq:%s found\n",
				mdnie_mode_name[mdnie_mode], mdnie_mode, seqname);
	} else {
		panel_mdnie_err(mdnie, "mdnie mode:%s(%d), seq not found\n",
...
__visible_for_testing int mdnie_get_coordinate(struct mdnie_info *mdnie, int *x, int *y)
{
	...
	if (!x || !y) {
		panel_mdnie_err(mdnie, "invalid parameter\n");
		return -EINVAL;
	}

	ret = panel_resource_copy(panel, coordinate, "coordinate");
	if (ret < 0) {
		panel_mdnie_err(mdnie, "failed to copy 'coordinate' resource\n");
		return -EINVAL;
	}
	...
	*y = g_coord_y;
	...
	if (*x < MIN_WCRD_X || *x > MAX_WCRD_X ||
		...
		panel_mdnie_warn(mdnie, "need to check coord_x:%d coord_y:%d\n", *x, *y);
...
static int mdnie_coordinate_area(struct mdnie_info *mdnie, int x, int y)
{
	...
	panel_mdnie_dbg(mdnie, "coord %d %d area Q%d, result %lld %lld\n",
...
static void mdnie_coordinate_tune_rgb(struct mdnie_info *mdnie, int x, int y, u8 (*tune_rgb)[MAX_COLOR])
{
	...
	panel_mdnie_info(mdnie, "coord (x:%4d y:%4d Q%d compV:%8lld compH:%8lld)\t"
...
static int panel_set_mdnie(struct panel_device *panel)
{
	...
	mdnie = &panel->mdnie;
	mdnie_mode = mdnie_current_state(mdnie);

	if (!IS_PANEL_ACTIVE(panel))
		return -EAGAIN;

#ifdef CONFIG_USDM_MDNIE_AFC
	panel_mdnie_info(mdnie, "do mdnie-seq (mode:%s, afc:%s)\n",
			mdnie_mode_name[mdnie_mode],
			!mdnie->props.afc_on ? "off" : "on");
#else
	panel_mdnie_info(mdnie, "do mdnie-seq (mode:%s)\n",
			mdnie_mode_name[mdnie_mode]);
#endif
	...
	if (is_mdnie_sequence_exist(mdnie, MDNIE_PRE_SEQ)) {
		...
		if (unlikely(ret < 0)) {
			panel_mdnie_err(mdnie, "failed to run sequence(%s)\n",
					MDNIE_PRE_SEQ);
			goto err;
		}
	}
...
static void mdnie_update_scr_white_mode(struct mdnie_info *mdnie)
{
	...
	if (!mdnie->props.wcrd_x || !mdnie->props.wcrd_y) {
		...
		panel_mdnie_warn(mdnie, "need to check coord_x:%d coord_y:%d, scr_white_mode %s\n",
	...
	panel_mdnie_dbg(mdnie, "scr_white_mode %s\n",
			scr_white_mode_name[mdnie->props.scr_white_mode]);
}
		...
		unsigned char r, unsigned char g, unsigned char b)
{
	...
	panel_mdnie_dbg(mdnie, "def_wrgb: %d(%02X) %d(%02X) %d(%02X)\n",
		...
		unsigned char r, unsigned char g, unsigned char b)
{
	...
	panel_mdnie_dbg(mdnie, "cur_wrgb: %d(%02X) %d(%02X) %d(%02X)\n",
		...
		unsigned char r, unsigned char g, unsigned char b)
{
	...
	if (mdnie->props.scr_white_mode < 0 ||
		...
		panel_mdnie_warn(mdnie, "out of range %d\n",
				mdnie->props.scr_white_mode);
		return -EINVAL;
	}
	...
	if (mdnie->props.scr_white_mode == SCR_WHITE_MODE_COLOR_COORDINATE) {
		...
		panel_mdnie_warn(mdnie, "wrgb is not updated in scr_white_mode(%d)\n",
				mdnie->props.scr_white_mode);
	}
...
int panel_mdnie_update(struct panel_device *panel)
{
	...
	if (!IS_MDNIE_ENABLED(mdnie)) {
		panel_mdnie_info(mdnie, "mdnie is off state\n");
	...
	if (ret < 0 && ret != -EAGAIN) {
		panel_mdnie_err(mdnie, "failed to set mdnie %d\n", ret);
		panel_mutex_unlock(&mdnie->lock);
		return ret;
	}
	panel_mutex_unlock(&mdnie->lock);
...
static int mdnie_update(struct mdnie_info *mdnie)
{
	...
	if (!IS_MDNIE_SUPPORTED(mdnie)) {
		panel_mdnie_warn(mdnie, "mdnie is unsupported\n");
		return 0;
	}
...
int mdnie_update_lux(struct mdnie_info *mdnie, int value)
{
	...
	if (update) {
		panel_mdnie_info(mdnie, "hbm_ce:%d anti_glare:%d (lux:%d)\n",
...
static void mdnie_coordinate_tune_test(struct mdnie_info *mdnie)
{
	...
	for (i = 0; i < 27; i++) {
		...
		g_coord_y = input[i][1];
		mdnie_update(mdnie);
		panel_mdnie_info(mdnie, "compare %02X %02X %02X : %02X %02X %02X (%s)\n",
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value >= MODE_MAX) {
		panel_mdnie_err(mdnie, "invalid value=%d\n", value);
		return -EINVAL;
	}

	panel_mdnie_info(mdnie, "value=%d\n", value);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!SCENARIO_IS_VALID(value)) {
		panel_mdnie_err(mdnie, "invalid scenario %d\n", value);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret <= 0 || ret > ARRAY_SIZE(s) + 1 ||
		...
		panel_mdnie_err(mdnie, "invalid size %d\n", ret);
		return -EINVAL;
	}

	if (value < 0 || value >= ACCESSIBILITY_MAX) {
		panel_mdnie_err(mdnie, "unknown accessibility %d\n", value);
		return -EINVAL;
	}

	panel_mdnie_info(mdnie, "value: %d, cnt: %d\n", value, ret);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!IS_MDNIE_ENABLED(mdnie)) {
		panel_mdnie_err(mdnie, "mdnie state is off\n");
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret != 3)
	 ...
	 panel_mdnie_info(mdnie, "white_r %u, white_g %u, white_b %u\n",
			 white_red, white_green, white_blue);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!IS_VALID_WRGB_OFS(wr_offset) ||
		...
		panel_mdnie_err(mdnie, "invalid offset %d %d %d\n",
				wr_offset, wg_offset, wb_offset);
		return -EINVAL;
	}

	panel_mdnie_info(mdnie, "wr_offset %d, wg_offset %d, wb_offset %d\n",
		 wr_offset, wg_offset, wb_offset);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_mdnie_info(mdnie, "night_mode %s level %d\n",
			enable ? "on" : "off", level);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_mdnie_info(mdnie, "vividness_level %d\n", level);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_mdnie_info(mdnie, "anti_glare %s\n", enable ? "on" : "off");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_mdnie_info(mdnie, "adaptive_mode %s\n", enable ? "on" : "off");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_mdnie_info(mdnie, "extra_dim level %d\n", level);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_mdnie_info(mdnie, "color_lens_mode %s color %d level %d\n",
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value < 0 || value >= MAX_LDU_MODE) {
		panel_mdnie_err(mdnie, "out of range %d\n",
				value);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel_testmode_is_on(panel)) {
		if (buf[0] != '!') {
			panel_mdnie_info(mdnie, "%s_store: testmode is running. ignore inputs.\n", attr->attr.name);
			return size;
		}
...
int mdnie_enable(struct mdnie_info *mdnie)
{
	...
	if (IS_MDNIE_ENABLED(mdnie)) {
		panel_mdnie_info(mdnie, "mdnie already enabled\n");
...
int mdnie_disable(struct mdnie_info *mdnie)
{
	...
	if (!IS_MDNIE_ENABLED(mdnie)) {
		panel_mdnie_info(mdnie, "mdnie already disabled\n");
		return 0;
	}
...
__visible_for_testing int mdnie_init_property(struct mdnie_info *mdnie, struct mdnie_tune *mdnie_tune)
{
	...
	if (ret < 0) {
		panel_mdnie_err(mdnie, "failed to add mdnie property array\n");
		return ret;
	}
...
__visible_for_testing int mdnie_deinit_property(struct mdnie_info *mdnie)
{
	...
	ret = panel_delete_property_from_array(to_panel_device(mdnie),
			...
			ARRAY_SIZE(mdnie_property_array));
	if (ret < 0) {
		panel_mdnie_err(mdnie, "failed to delete mdnie property array\n");
		return ret;
	}
...
__visible_for_testing int mdnie_create_device(struct mdnie_info *mdnie)
{
	...
	if (!to_panel_device(mdnie)->lcd_dev) {
		panel_mdnie_err(mdnie, "lcd_device is null\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(mdnie->dev)) {
		panel_mdnie_err(mdnie, "failed to create %s device\n", mdnie_get_name(mdnie));
		return -EINVAL;
	}
...
__visible_for_testing int mdnie_destroy_device(struct mdnie_info *mdnie)
{
	...
	if (!mdnie->dev) {
		panel_mdnie_err(mdnie, "mdnie device is null\n");
		return -ENODEV;
	}
...
__visible_for_testing int mdnie_create_device_files(struct mdnie_info *mdnie)
{
	...
	for (i = 0; i < ARRAY_SIZE(mdnie_dev_attrs); i++) {
		...
		if (ret < 0) {
			panel_mdnie_err(mdnie, "failed to add %s sysfs entries, %d\n",
...
int mdnie_init(struct mdnie_info *mdnie)
{
	...
	if (ret < 0) {
		panel_mdnie_err(mdnie, "failed to create mdnie device\n");
		return ret;
	}

	ret = mdnie_create_device_files(mdnie);
	if (ret < 0) {
		panel_mdnie_err(mdnie, "failed to create device files\n");
		mdnie_destroy_device(mdnie);
		return ret;
	}

	panel_mdnie_info(mdnie, "mdnie init success\n");
...
int mdnie_exit(struct mdnie_info *mdnie)
{
	...
	if (ret < 0)
		panel_mdnie_err(mdnie, "failed to remove device files\n");

	ret = mdnie_destroy_device(mdnie);
	if (ret < 0)
		panel_mdnie_err(mdnie, "failed to destroy mdnie device\n");
...
int mdnie_prepare(struct mdnie_info *mdnie, struct mdnie_tune *mdnie_tune)
{
	...
	if (!mdnie_tune) {
		panel_mdnie_warn(mdnie, "mdnie is not supported\n");
		return 0;
	}
	...
	if (ret < 0) {
		panel_mdnie_err(mdnie, "failed to panel_add_command_from_initdata_seqtbl\n");
		goto err;
	}
...
err:
	panel_mutex_unlock(&mdnie->lock);
	panel_mdnie_err(mdnie, "failed to prepare mdnie\n");
...
err:
	panel_mutex_unlock(&mdnie->lock);
	panel_mdnie_err(mdnie, "failed to unprepare mdnie\n");
...
int mdnie_probe(struct mdnie_info *mdnie, struct mdnie_tune *mdnie_tune)
{
	...
	if (unlikely(!mdnie_tune)) {
		panel_mdnie_err(mdnie, "invalid argument\n");
		return -ENODEV;
	}
	...
	if (ret < 0) {
		panel_mdnie_err(mdnie, "failed to enable mdnie\n");
		return ret;
	}

	panel_mdnie_info(mdnie, "mdnie probe success\n");

	return 0;

err:
	panel_mutex_unlock(&mdnie->lock);
	panel_mdnie_err(mdnie, "failed to probe mdnie\n");
