...
int panel_blic_brightness(struct panel_device *panel, bool need_lock)
{
	...
	for (i = 0; i < panel->nr_blic_dev; i++) {
		if (panel_blic_check_seqtbl_exist(&panel->blic_dev[i], PANEL_BLIC_I2C_BRIGHTNESS_SEQ)) {
			...
			if (ret) {
				panel_drv_err(panel, "panel_blic_do_seq is failed.(%d)\n", ret);
...
__visible_for_testing struct panel_i2c_dev *panel_blic_find_i2c_drv(struct panel_blic_dev *blic_dev, struct panel_device *panel)
{
	...
	for (i = 0; i < panel->nr_i2c_dev; i++) {
		...
		if ((i2c_read_buf[1] & blic_dev->i2c_match[1]) == blic_dev->i2c_match[2]) {
			panel_drv_info(panel, "panel_i2c found. blic id:(0x%02X) mask:(0x%02X) id&mask:(0x%02X) val:(0x%02X)\n",
...
static int of_get_panel_blic(struct panel_blic_dev *blic_dev, struct device_node *np)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "name is null.\n");
		ret = -EINVAL;
		goto err;
	}
	/* i2c info */
	...
	if (ret < 0) {
		panel_drv_err(panel, "i2c is null.\n");
		ret = -EINVAL;
		goto err;
	}
	...
	if (!temp->regulator_node) {
		panel_drv_err(panel, "regulator is null.\n");
		ret = -EINVAL;
		goto err;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "regulator name is null.\n");
		ret = -EINVAL;
		goto err;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "regulator desc name is null.\n");
		ret = -EINVAL;
		goto err;
	}
	...
	if (gpios_np) {
		for_each_child_of_node(gpios_np, gpio_np) {
			...
			if (of_get_panel_gpio(panel, gpio_np, &gpio)) {
	...
	if (!power_np) {
		panel_drv_err(panel, "'power_ctrl' node not found\n");
		ret = -EINVAL;
		goto exit;
	}
...
exit:
	...
	panel_drv_info(panel, "(%s) is registered(nr: %d).\n", blic_dev[id].name, panel->nr_blic_dev);
...
static int panel_blic_get_blic(struct panel_blic_dev *blic_dev, struct device_node *np)
{
	...
	panel = to_panel_drv(blic_dev);
	if (!np) {
		panel_drv_err(panel, "blic node is null.\n");
		return -EINVAL;
	}

	for_each_child_of_node(np, blic_np) {
		if (of_get_panel_blic(blic_dev, blic_np)) {
			panel_drv_err(panel, "failed to get panel_blic %s\n", blic_np->name);
			ret = -EINVAL;
			break;
		}
		id++;
...
__visible_for_testing int panel_blic_get_blic_regulator(struct panel_blic_dev *blic_dev)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to register blic regulator(%s)\n",
...
int panel_blic_prepare(struct panel_device *panel, struct common_panel_info *info)
{
	...
	if (!info->blic_data_tbl) {
		panel_drv_err(panel, "blic_data_tbl is null\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < panel->nr_blic_dev; i++) {
		...
		if (!blic_data) {
			panel_drv_err(panel, "blic_data_tbl[%d] is null.\n", i);
			return 0;
		}
...
int panel_blic_probe(struct panel_device *panel)
{
	...
	if (!np) {
		panel_drv_err(panel, "failed to get blic DT node.\n");
		return -EINVAL;
	}
	...
	of_node_put(np);
	if (ret) {
		panel_drv_err(panel, "failed to get blic. %d\n", ret);
		return ret;
	}
	...
	for (i = 0; i < panel->nr_blic_dev; i++) {
		...
		if (ret) {
			panel_drv_err(panel, "failed to get blic regulator. %d\n", ret);
			return ret;
		}
