...
int snprintf_dump_expects(struct panel_device *panel, char *buf, size_t size, struct dumpinfo *dump)
{
	...
	if (!is_valid_resource(dump->res)) {
		panel_drv_err(panel, "invalid dump(%s) resource\n",
				get_dump_name(dump));
		return -EINVAL;
	}
...
int oled_dump_show_expects(struct panel_device *panel, struct dumpinfo *dump)
{
	...
	len = snprintf_dump_expects(panel, buf, SZ_256, dump);
	if (len > 0)
...
{
	oled_dump_show_resource(panel, dump);
	PANEL_BUG();
...
int oled_dump_show_rddpm(struct panel_device *panel, struct dumpinfo *dump)
{
	...
	if (!res || ARRAY_SIZE(rddpm) != res->dlen) {
		panel_drv_err(panel, "invalid resource\n");
		return -EINVAL;
	}

	ret = copy_resource(rddpm, dump->res);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to copy resource(%s)\n",
	...
	return oled_dump_show_expects(panel, dump);
}
...
int oled_dump_show_rddsm(struct panel_device *panel, struct dumpinfo *dump)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to copy rddsm resource\n");
		return -EINVAL;
	}
...
int oled_dump_show_err_fg(struct panel_device *panel, struct dumpinfo *dump)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to copy err_fg resource\n");
		return -EINVAL;
	}
...
int oled_dump_show_self_mask_crc(struct panel_device *panel, struct dumpinfo *dump)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to self mask crc resource\n");
		return -EINVAL;
	}

	panel_drv_info(panel, "======= SHOW PANEL [7Fh:SELF_MASK_CRC] dump =======\n");
	panel_drv_info(panel, "* Reg Value : 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
...
{
	return oled_dump_show_resource(panel, dump);
}
