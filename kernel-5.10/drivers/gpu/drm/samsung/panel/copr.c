...
#define PANEL_COPR_DRIVER_NAME "panel-copr"
#define panel_copr_info(_copr, fmt, ...)   \
	usdm_pr_info(PANEL_COPR_DRIVER_NAME, ((_copr) ? (_copr)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_copr_warn(_copr, fmt, ...)   \
	usdm_pr_warn(PANEL_COPR_DRIVER_NAME, ((_copr) ? (_copr)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_copr_err(_copr, fmt, ...)    \
	usdm_pr_err(PANEL_COPR_DRIVER_NAME, ((_copr) ? (_copr)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_copr_dbg(_copr, fmt, ...)\
	usdm_pr_debug(PANEL_COPR_DRIVER_NAME, ((_copr) ? (_copr)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)
...
static void SET_COPR_REG_GAMMA(struct copr_info *copr, unsigned int copr_gamma)
{
	...
	if (version == COPR_VER_0)
		...
		panel_copr_warn(copr, "unsupprted in ver%d\n", version);
}
...
static inline void panel_send_coprstate_notify(struct panel_device *panel, u32 state)
{
	...
	panel_drv_info(panel, "call EVENT_COPR_STATE notifier %d\n", evt_data.state);
}
#endif
...
static int panel_set_copr(struct copr_info *copr)
{
	...
	if (unlikely(ret < 0)) {
		panel_copr_err(copr, "failed to run sequence(%s)\n",
				COPR_SET_SEQ);
		return ret;
	}
...
static int panel_clear_copr(struct copr_info *copr)
{
	...
	panel_copr_dbg(copr, "copr clear seq\n");
...
static int panel_read_copr_spi(struct copr_info *copr)
{
	...
	if (size < 0) {
		panel_copr_err(copr, "failed to get copr size (ret %d)\n", size);
		ret = -EINVAL;
		goto get_copr_error;
	}
	...
	if (ret < 0) {
		panel_copr_err(copr, "failed to get copr (ret %d)\n", ret);
		goto get_copr_error;
	}
	...
	if (version == COPR_VER_6) {
		for (i = 0; i < 5; i++) {
			...
			panel_copr_dbg(copr, "copr_dsi: copr_roi_r[%d] %d %d %d %d\n",
		...
		for (i = 0; i < 5; i++) {
			...
			panel_copr_dbg(copr, "copr_spi: copr_roi_r[%d] %d %d %d\n",
					i, props->copr_roi_r[i][RED],
					props->copr_roi_r[i][GREEN],
					props->copr_roi_r[i][BLUE]);
		}
		panel_copr_dbg(copr, "copr_spi: cur_cnt %d, cur_copr %d, avg_copr %d, s_cur_cnt %d, s_avg_copr %d, copr_ready %d, comp_copr %d\n",
		...
		props->s_cur_cnt = ((buf[4] & 0x1F) << 11) |
			(buf[5] << 3) | ((buf[6] & 0xE0) >> 5);
		props->s_avg_copr = ((buf[6] & 0x1F) << 4) | ((buf[7] & 0xF0) >> 4);
		panel_copr_dbg(copr, "copr_spi: cur_cnt %d, cur_copr %d, avg_copr %d, s_cur_cnt %d, s_avg_copr %d, copr_ready %d\n",
				props->cur_cnt, props->cur_copr, props->avg_copr,
...
static int panel_read_copr_dsi(struct copr_info *copr)
{
	...
	if (version == COPR_VER_6) {
		...
		for (i = 0; i < 5; i++) {
			...
			panel_copr_dbg(copr, "copr_dsi: copr_roi_r[%d] %d %d %d\n",
					i, props->copr_roi_r[i][RED],
					props->copr_roi_r[i][GREEN],
					props->copr_roi_r[i][BLUE]);
		}
		panel_copr_dbg(copr, "copr_dsi: cur_cnt %d, cur_copr %d, avg_copr %d, s_cur_cnt %d, s_avg_copr %d, copr_ready %d\n",
				props->cur_cnt, props->cur_copr, props->avg_copr,
				props->s_cur_cnt, props->s_avg_copr, props->copr_ready);
	} else if (version == COPR_VER_2) {
		...
		panel_copr_dbg(copr, "copr_dsi: cur_cnt %d, cur_copr %d, avg_copr %d, s_cur_cnt %d, s_avg_copr %d, copr_ready %d, comp_copr %d\n",
				props->cur_cnt, props->cur_copr, props->avg_copr,
				props->s_cur_cnt, props->s_avg_copr, props->copr_ready, props->comp_copr);
	} else if (version == COPR_VER_1) {
...
static int panel_get_copr(struct copr_info *copr)
{
	...
	if (props->state != COPR_REG_ON) {
		panel_copr_dbg(copr, "copr reg is not on state %d\n", props->state);
		ret = -EINVAL;
		goto get_copr_error;
	}

	if (atomic_read(&copr->stop)) {
		panel_copr_warn(copr, "copr_stop\n");
		ret = -EINVAL;
		goto get_copr_error;
	}
	...
	panel_copr_dbg(copr, "elapsed_usec %lld usec (%lld.%lld %lld.%lld)\n",
...
int copr_update_average(struct copr_info *copr)
{
	...
	if (!copr_is_enabled(copr)) {
		panel_copr_dbg(copr, "copr disabled\n");
		return -EIO;
	}
	...
	if (props->state == COPR_UNINITIALIZED) {
		panel_set_copr(copr);
		panel_copr_info(copr, "copr register updated\n");
	}
	...
	if (version == COPR_VER_2 ||
		...
		if (unlikely(ret < 0))
			panel_copr_err(copr, "failed to reset copr\n");
...
int copr_iter_roi_get_value(struct copr_info *copr, struct copr_roi *roi, int size, u32 *out)
{
	...
	if (ret < 0) {
		panel_copr_err(copr, "failed to update average(ret %d)\n", ret);
		panel_mutex_unlock(&copr->lock);
		return ret;
	}
	...
	panel_copr_dbg(copr, "restore roi\n");
...
static int set_spi_gpios(struct panel_device *panel, int en)
{
	...
	if (!spi) {
		panel_drv_dbg(panel, "spi is null\n");
		return 0;
	}

	panel_drv_info(panel, "en : %d\n", en);
	if (en) {
...
set_exit:
	panel_drv_err(panel, "failed to gpio:%d\n", err_num);
	return -EIO;
}
...
int copr_enable(struct copr_info *copr)
{
	...
	if (copr_is_enabled(copr)) {
		panel_copr_info(copr, "already enabled\n");
		return 0;
	}

#ifdef CONFIG_USDM_COPR_SPI
	if (set_spi_gpios(panel, 1))
		panel_copr_err(copr, "failed to set spio gpio\n");
#endif

	panel_copr_info(copr, "+\n");
	...
	panel_copr_info(copr, "-\n");
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
	panel_send_coprstate_notify(panel, PANEL_EVENT_COPR_STATE_ENABLED);
#endif
	return 0;
}
...
int copr_disable(struct copr_info *copr)
{
	...
	if (!copr_is_enabled(copr)) {
		panel_copr_info(copr, "already disabled\n");
		return 0;
	}
	...
	panel_copr_info(copr, "-\n");
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
	panel_send_coprstate_notify(panel, PANEL_EVENT_COPR_STATE_DISABLED);
#endif
	return 0;
}
...
{
	if (unlikely(!copr->props.support)) {
		panel_copr_warn(copr, "copr unsupported\n");
		return 0;
	}
	...
	if (IS_ERR_OR_NULL(copr->wq.thread)) {
		panel_copr_err(copr, "failed to run copr thread\n");
		copr->wq.thread = NULL;
				 ...
				 unsigned long event, void *data)
{
	...
	panel_copr_dbg(copr, "fb_blank:%d\n", fb_blank);
...
int copr_prepare(struct panel_device *panel, struct panel_copr_data *copr_data)
{
	...
	if (!copr_data) {
		panel_drv_info(panel, "copr is not supported\n");
		return 0;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to panel_add_command_from_initdata_maptbl\n");
		panel_mutex_unlock(&copr->lock);
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to panel_add_command_from_initdata_seqtbl\n");
		panel_mutex_unlock(&copr->lock);
		return ret;
	}
...
int copr_probe(struct panel_device *panel, struct panel_copr_data *copr_data)
{
	...
	if (!copr_data) {
		panel_drv_warn(panel, "copr is not supported\n");
		return 0;
	}
	...
	panel_copr_info(copr, "registered successfully\n");
