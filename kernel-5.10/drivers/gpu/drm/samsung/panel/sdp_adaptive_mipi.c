...
int probe_sdp_adaptive_mipi(struct panel_device *panel)
{
	...
	if (panel->adap_mipi_node == NULL) {
		panel_drv_err(panel, "null device tree node\n");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		panel_drv_info(panel, "failed to init sdp adaptive mipi\n");
		return ret;
	}

	panel_drv_info(panel, "sdp adaptive mipi: probe done\n");
