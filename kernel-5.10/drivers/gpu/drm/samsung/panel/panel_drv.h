...
#define PANEL_GPIO_NAME_PMIC_DET ("pmic-det")
#define PANEL_GPIO_NAME_PMIC_SUB_DET ("pmic-sub-det")
...
#define PANEL_PROPERTY_DISPLAY_MODE_SLOT ("display_mode_slot")
...
struct panel_device {
	...
	/* mutex lock for panel's state */
	struct panel_mutex state_lock;
...
#endif
int panel_get_gpio_value(struct panel_device *panel, enum panel_gpio_lists panel_gpio_id);

#define PRINT_PANEL_STATE_BEGIN(panel, _old_state_, _new_state_, _start_) \
	do { \
		_start_ = ktime_get(); \
		panel_drv_info(panel, "panel_state: %s -> %s: begin\n", \
				get_panel_state_names(_old_state_), \
				get_panel_state_names(_new_state_)); \
	} while (0)

#define PRINT_PANEL_STATE_END(panel, _old_state_, _new_state_, _start_) \
	do { \
		char rtc_buf[SZ_128] = { 0, }; \
		\
		usdm_snprintf_current_rtc_time(rtc_buf, sizeof(rtc_buf)); \
		panel_drv_info(panel, "panel_state: %s -> %s: took %lld ms [%s]\n", \
				get_panel_state_names(_old_state_), \
				get_panel_state_names(_new_state_), \
