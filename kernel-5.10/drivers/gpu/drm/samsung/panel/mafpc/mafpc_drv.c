...
#define PANEL_MAFPC_DRIVER_NAME "panel-mafpc"
#define panel_mafpc_info(_mafpc, fmt, ...)   \
	usdm_pr_info(PANEL_MAFPC_DRIVER_NAME, ((_mafpc) ? (_mafpc)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_mafpc_warn(_mafpc, fmt, ...)   \
	usdm_pr_warn(PANEL_MAFPC_DRIVER_NAME, ((_mafpc) ? (_mafpc)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_mafpc_err(_mafpc, fmt, ...)    \
	usdm_pr_err(PANEL_MAFPC_DRIVER_NAME, ((_mafpc) ? (_mafpc)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_mafpc_dbg(_mafpc, fmt, ...)\
	usdm_pr_debug(PANEL_MAFPC_DRIVER_NAME, ((_mafpc) ? (_mafpc)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)
...
__visible_for_testing ssize_t write_comp_image(struct mafpc_device *mafpc, const char __user *buf, size_t count)
{
	...
	if (!buf) {
		panel_mafpc_err(mafpc, "buf is null\n");
		return -EINVAL;
	}

	if (!mafpc->comp_img_buf || !mafpc->scale_buf) {
		panel_mafpc_warn(mafpc, "buffer is not prepared\n");
		return -EAGAIN;
	}

	if (count != ABC_DATA_SIZE(mafpc) &&
		count != (ABC_DATA_SIZE(mafpc) - ABC_SCALE_FACTOR_SIZE(mafpc))) {
		panel_mafpc_err(mafpc, "invalid buffer size: %ld\n", count);
		return -EINVAL;
	}
	...
	if (copy_from_user(mafpc->ctrl_cmd, buf + ABC_CTRL_CMD_OFFSET(mafpc),
		...
		panel_mafpc_err(mafpc, "failed to get ctrl_cmd\n");
		return -EFAULT;
	}
	...
	if (copy_from_user(mafpc->comp_img_buf, buf + ABC_COMP_IMG_OFFSET(mafpc),
		...
		panel_mafpc_err(mafpc, "failed to get comp_img\n");
		return -EFAULT;
	}
	...
	if (count == ABC_DATA_SIZE(mafpc)) {
		if (copy_from_user(mafpc->scale_buf, buf + ABC_SCALE_FACTOR_OFFSET(mafpc),
			...
			panel_mafpc_err(mafpc, "failed to get scale_factor\n");
			return -EFAULT;
		}
	}
	mafpc_set_written_flag(mafpc, MAFPC_UPDATED_FROM_SVC);
	panel_mafpc_info(mafpc, "mafpc image write %ld done! (img_crc 0x%x)\n",
...
static ssize_t write_lut_table(struct mafpc_device *mafpc, const char __user *buf, size_t count)
{
	...
	if (buf == NULL) {
		panel_mafpc_err(mafpc, "invalid param\n");
		return -EINVAL;
	}

	comp_mem = &mafpc->comp_mem;
	panel_mafpc_info(mafpc, "lut_size: %ld, count: %ld\n",
			comp_mem->lut_size, count);
	if (comp_mem->lut_size != count) {
		panel_mafpc_err(mafpc, "invalid lut_size: %ld\n", count);
		return -EINVAL;
	}

	lut_buf = (void *)phys_to_virt(comp_mem->lut_base);
	if (lut_buf == NULL) {
		panel_mafpc_err(mafpc, "null lut buf\n");
		return -EINVAL;
	}

	ret = copy_from_user(&header, buf, LPD_COMP_LUT_HEADER_SZ);
	if (ret) {
		panel_mafpc_err(mafpc, "failed to get user's header\n");
		return ret;
	}
	panel_mafpc_info(mafpc, "header: %c, lut_type: %d\n", header, (header >> 8) & 0xff);
...
static ssize_t abc_fops_write(struct file *file, const char __user *buf,  size_t count, loff_t *ppos)
{
	...
	panel_mafpc_dbg(mafpc, "count: %ld, header: %c\n",
	...
	case 'M':
		...
		if (ret < 0) {
			panel_mafpc_err(mafpc, "failed to write compensation image\n");
			return ret;
		}
		break;
#if IS_ENABLED(CONFIG_LPD_OLED_COMPENSATION)
	case 'L':
		...
		if (ret < 0) {
			panel_mafpc_err(mafpc, "failed to write lut table\n");
			return 0;
		}
		break;
#endif
	default:
		panel_mafpc_err(mafpc, "known identifier: %d\n", header[MAFPC_DATA_IDENTIFIER]);
		return 0;
	}
...
static int mafpc_instant_on(struct mafpc_device *mafpc)
{
	...
	panel_mafpc_info(mafpc, "+\n");
	panel_mutex_lock(&mafpc->lock);
	if (!IS_PANEL_ACTIVE(panel)) {
		panel_mafpc_warn(mafpc, "panel is not active\n");
		goto exit_write;
	}

	if (panel_get_cur_state(panel) == PANEL_STATE_ALPM) {
		panel_mafpc_warn(mafpc, "ABC not supported on LPM\n");
		goto exit_write;
	}
	 ...
	 */
	panel_mafpc_info(mafpc, "PANEL_MAFPC_IMG_SEQ +\n");
	ret = panel_do_seqtbl_by_name(panel, PANEL_MAFPC_IMG_SEQ);
	if (unlikely(ret < 0)) {
		panel_mafpc_err(mafpc, "failed to run sequence(%s)\n", PANEL_MAFPC_IMG_SEQ);
		goto exit_write;
	}
	panel_mafpc_info(mafpc, "PANEL_MAFPC_IMG_SEQ -\n");
	mafpc_set_written_to_dev(mafpc);

	panel_mafpc_info(mafpc, "PANEL_MAFPC_ON_SEQ +\n");
...
static void mafpc_instant_handler(struct work_struct *data) {
	...
	panel_mafpc_info(mafpc, "%d done %d\n", mafpc->instant_cmd, ret);
	if (ret == 0) {
...
static int mafpc_clear_image_buffer(struct mafpc_device *mafpc)
{
	...
	if (comp_mem->reserved == false) {
		panel_mafpc_err(mafpc, "does not supprot read ops\n");
		return 0;
	}

	if (comp_mem->image_size == 0) {
		panel_mafpc_err(mafpc, "wrong buffer size\n");
		return 0;
	}

	image_buf = (void *)phys_to_virt(comp_mem->image_base);
	if (image_buf == NULL) {
		panel_mafpc_err(mafpc, "failed to get virtual address\n");
		return 0;
	}

	memset(image_buf, 0x00, comp_mem->image_size);
	panel_mafpc_info(mafpc, "mafpc image buffer clear\n");
...
static int abc_set_enable(struct mafpc_device *mafpc, bool en, bool instant)
{
	...
	panel_mafpc_info(mafpc, "mafpc %s %s\n", en ? "on" : "off", instant ? "instant" : "");
	if (instant) {
		mafpc->instant_cmd = en ? INSTANT_CMD_ON : INSTANT_CMD_OFF;
		if (!mafpc->panel)
			panel_mafpc_info(mafpc, "panel is not ready. pending instant %s\n", en ? "on" : "off");
		else
...
static long abc_fops_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MAFPC_IOCTL_CLEAR_IMAGE_BUFFER:
		...
		if (ret)
			panel_mafpc_err(mafpc, "failed to clear mafpc image buffer\n");
		break;
	...
	case MAFPC_IOCTL_GET_CRC:
		...
		break;
	default:
		panel_mafpc_err(mafpc, "invalid command\n");
		break;
	}
...
static ssize_t abc_fops_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	panel_mafpc_info(mafpc, "count: %ld\n", count);

	if (mafpc == NULL) {
		panel_mafpc_err(mafpc, "mafpc is null\n");
		return -EINVAL;
	}

	comp_mem = &mafpc->comp_mem;
	if (comp_mem->reserved == false) {
		panel_mafpc_err(mafpc, "does not support read ops\n");
		return 0;
	}
	...
	if (ret) {
		panel_mafpc_err(mafpc, "failed to copy to user\n");
		return ret;
	}
...
int mafpc_device_probe(struct mafpc_device *mafpc, struct mafpc_info *info)
{
	...
	mafpc->id = panel->id;
	if (!info) {
		panel_mafpc_err(mafpc, "mafpc_info is null\n");
		return -EINVAL;
	}

	if (!info->abc_crc || !info->abc_crc_len) {
		panel_mafpc_err(mafpc, "crc not found\n");
		return -EINVAL;
	}

	if (!info->abc_img || !info->abc_img_len) {
		panel_mafpc_err(mafpc, "default image not found\n");
		return -EINVAL;
	}

	if (!info->abc_scale_factor || !info->abc_scale_factor_len) {
		panel_mafpc_err(mafpc, "default scale factor not found\n");
		return -EINVAL;
	}

	if (!info->abc_scale_map_tbl || !info->abc_scale_map_tbl_len) {
		panel_mafpc_err(mafpc, "scale brightness map not found\n");
		return -EINVAL;
	}

	if (mafpc->ctrl_cmd_len != info->abc_ctrl_cmd_len) {
		panel_mafpc_err(mafpc, "ctrl cmd size mismatch. expected %d, but %d\n",
				mafpc->ctrl_cmd_len, info->abc_ctrl_cmd_len);
		return -EINVAL;
	}

	if (mafpc->comp_img_len != info->abc_img_len) {
		panel_mafpc_err(mafpc, "image size mismatch. expected %d, but %d\n",
				mafpc->comp_img_len, info->abc_img_len);
		return -EINVAL;
	}

	if (mafpc->scale_len != info->abc_scale_factor_len) {
		panel_mafpc_err(mafpc, "scale factor size mismatch. expected %d, but %d\n",
	...
	if (!is_mafpc_updated_from_svc(mafpc) && panel_is_factory_mode(panel)) {
		...
		panel_mafpc_info(mafpc, "update with default image and scale factor\n");
	}
	...
	panel_mafpc_info(mafpc, "image size: %d\n", mafpc->comp_img_len);
	panel_mafpc_info(mafpc, "scale factor size: %d\n", mafpc->scale_len);
...
static int parse_lpd_rmem(struct mafpc_device *mafpc)
{
	...
	if (np == NULL) {
		panel_mafpc_err(mafpc, "failed to parse reserved mem for burnin\n");
		return -EINVAL;
	}

	rmem = of_reserved_mem_lookup(np);
	if (rmem == NULL) {
		panel_mafpc_err(mafpc, "failed to get reserved mem for burnin\n");
		return -EINVAL;
	}
	...
	panel_mafpc_info(mafpc, "LPD DRAM is reserved at addr %x total size %d\n", (unsigned int)rmem->base, (unsigned int)rmem->size);

	panel_mafpc_info(mafpc, "image base: %x, size: %x, lut base: %x, size: %x, total rmem: %x\n",
...
static int mafpc_parse_dt(struct mafpc_device *mafpc)
{
	...
	if (mafpc->ctrl_cmd_len > MAX_MAFPC_CTRL_CMD_SIZE) {
		panel_mafpc_err(mafpc, "mafpc ctrl cmd size(%d) exceeded %d\n",
	...
	if (ret < 0) {
		panel_mafpc_err(mafpc, "failed to parse lpd mem\n");
		return ret;
	}
#endif
