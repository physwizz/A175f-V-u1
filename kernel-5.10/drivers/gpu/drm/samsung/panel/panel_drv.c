...
static void panel_notify_panel_state(struct panel_device *panel, u32 state);
#else
static inline void panel_notify_panel_state(struct panel_device *panel, u32 state) { return; }
#endif
...
static char *panel_gpio_names[PANEL_GPIO_MAX] = {
	...
	[PANEL_GPIO_PMIC_DET] = PANEL_GPIO_NAME_PMIC_DET,
	[PANEL_GPIO_PMIC_SUB_DET] = PANEL_GPIO_NAME_PMIC_SUB_DET,
};
...
static struct panel_prop_list panel_drv_property_array[] = {
	...
	__PANEL_PROPERTY_RANGE_INITIALIZER(PANEL_PROPERTY_DISPLAY_MODE_SLOT,
			0, 0, 1),
};
		...
		u32 *property, unsigned int value)
{
	...
	if (!property) {
		panel_drv_err(panel, "property is null\n");
		return -EINVAL;
	}
	...
	else if (property == &panel->panel_data.props.display_mode_slot)
		propname = PANEL_PROPERTY_DISPLAY_MODE_SLOT;

	if (!propname) {
		panel_drv_err(panel, "unknown property\n");
		return -EINVAL;
	}

	if (panel_set_property_value(panel,
				propname, value) < 0) {
		panel_drv_warn(panel, "failed to set property(%s) %d\n",
				propname, value);
		return -EINVAL;
	}
	*property = value;
		...
		unsigned int panel_id)
{
	...
	if (panel_id >= 0x1000000) {
		panel_drv_warn(panel, "panel_id(0x%X) exceeds 3byte\n", panel_id);
		panel_id &= 0xFFFFFF;
	}
...
int panel_set_i2c_device(struct panel_i2c_dev *panel_i2c_dev)
{
	...
	if (panel->nr_i2c_dev >= PANEL_I2C_MAX) {
		panel_drv_err(panel, "i2c_dev array is full. (%d)\n", panel->nr_i2c_dev);
		return -EINVAL;
	}
	...
	if (panel_i2c_dev->client == NULL) {
		panel_drv_info(panel, "client NULL\n");
		return 0;
	}

	if (panel_i2c_dev->client->adapter == NULL) {
		panel_drv_info(panel, "adapter NULL\n");
		return 0;
	}

	panel_drv_info(panel, "(%s)(%s)(nr:%d)(addr:%d)\n",
...
int panel_initialize_regulator(struct panel_device *panel)
{
	...
	list_for_each_entry(regulator, &panel->regulator_list, head) {
		ret = panel_regulator_helper_init(panel, regulator);
		if (ret < 0) {
			panel_drv_err(panel, "%s(%s) init failed\n", regulator->node_name, regulator->name);
			result = ret;
		}
	}
	return result;
...
__visible_for_testing struct panel_gpio *panel_get_gpio(struct panel_device *panel, enum panel_gpio_lists panel_gpio_id)
{
	...
	if (!panel_gpio_helper_is_valid(panel, gp))
		return NULL;

	return gp;
}

int panel_get_gpio_value(struct panel_device *panel,
		enum panel_gpio_lists panel_gpio_id)
{
	return panel_gpio_helper_get_value(panel, panel_get_gpio(panel, panel_gpio_id));
}

__visible_for_testing int panel_set_gpio_value(struct panel_device *panel,
		enum panel_gpio_lists panel_gpio_id, int value)
{
	return panel_gpio_helper_set_value(panel, panel_get_gpio(panel, panel_gpio_id), value);
}

__visible_for_testing const char *panel_get_gpio_name(struct panel_device *panel,
		enum panel_gpio_lists panel_gpio_id)
{
	return panel_gpio_helper_get_name(panel, panel_get_gpio(panel, panel_gpio_id));
}

__visible_for_testing bool panel_is_gpio_irq_valid(struct panel_device *panel, enum panel_gpio_lists panel_gpio_id)
{
	return panel_gpio_helper_is_irq_valid(panel, panel_get_gpio(panel, panel_gpio_id));
}
...
__visible_for_testing int panel_get_gpio_state(struct panel_device *panel, enum panel_gpio_lists panel_gpio_id)
{
	...
	state = panel_gpio_helper_get_state(panel, panel_get_gpio(panel, panel_gpio_id));
	if (state < 0)
		return state;
		...
		enum panel_gpio_lists panel_gpio_id)
{
	...
	return panel_gpio_helper_is_irq_enabled(panel, panel_get_gpio(panel, panel_gpio_id));
}
		...
		enum panel_gpio_lists panel_gpio_id)
{
	...
	/* clear pending bit */
	ret = panel_gpio_helper_clear_irq_pending_bit(panel, gpio);
	if (ret < 0)
		return -EINVAL;

	return panel_gpio_helper_enable_irq(panel, gpio);
}
		...
		enum panel_gpio_lists panel_gpio_id)
{
	...
	return panel_gpio_helper_disable_irq(panel, panel_get_gpio(panel, panel_gpio_id));
}
		...
		unsigned long sleep_us, unsigned long timeout_us)
{
	...
	if (!panel_is_gpio_valid(panel, panel_gpio_id)) {
		panel_drv_err(panel, "invalid gpio(%s)\n", panel_gpio_names[panel_gpio_id]);
		return -EINVAL;
	}
...
int panel_poll_irq(struct panel_device *panel, enum panel_gpio_lists pin)
{
	...
	if (ret)
		panel_drv_info(panel, "%s pin level abnormal\n", gpio->name);
...
static void evasion_disp_det_handler(struct work_struct *work)
{
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_OFF) {
		panel_drv_warn(panel, "panel is off state\n");
		return;
	}
	...
	if (!panel_is_gpio_irq_enabled(panel, PANEL_GPIO_DISP_DET)) {
		...
		if (ret < 0)
			panel_drv_warn(panel, "failed to enable disp_det.\n");
	}
...
int panel_disable_evasion_disp_det_irq(struct panel_device *panel)
{
	...
	if (panel_is_gpio_irq_enabled(panel, PANEL_GPIO_DISP_DET)) {
		...
		if (ret < 0)
			panel_drv_warn(panel, "failed to disable disp_det.\n");
	}
...
static void panel_check_evasion_disp_det_support(struct panel_device *panel)
{
	...
	panel_drv_info(panel, "evasion disp_det is supported.\n");
	...
	for (panel_gpio_id = 0; panel_gpio_id < PANEL_GPIO_MAX; panel_gpio_id++) {
		...
		if (!panel_is_gpio_valid(panel, panel_gpio_id)) {
			if (panel->irq_invalid_log_once_flag & (0x1 << panel_gpio_id)) {
				panel_drv_info(panel, "%s is not supported.\n", panel_gpio_names[panel_gpio_id]);
		...
		if (ret < 0)
			panel_drv_warn(panel, "%s enable fail.\n", gpio->name);
...
int panel_disable_irq(struct panel_device *panel, u32 pins)
{
	...
	for (panel_gpio_id = PANEL_GPIO_MAX - 1 ; panel_gpio_id >= 0; panel_gpio_id--) {
		...
		if (ret < 0)
			panel_drv_warn(panel, "%s disable fail.\n", gpio->name);
...
int panel_disp_det_state(struct panel_device *panel)
{
	...
	if (state >= 0)
		panel_drv_dbg(panel, "disp_det:%s\n", state ? "EL-OFF" : "EL-ON");
...
int panel_conn_det_state(struct panel_device *panel)
{
	...
	if (state >= 0)
		panel_drv_dbg(panel, "conn_det:%s\n", state ? "connected" : "disconnected");
		...
		enum panel_active_state state)
{
	...
	panel_notify_panel_state(panel, state);
}
...
#ifdef CONFIG_SUPPORT_PANEL_SWAP
	evt_data.d.screen_mode = get_panel_id(panel);
#endif
	panel_notifier_call_chain(PANEL_EVENT_PANEL_STATE_CHANGED, &evt_data);
	panel_drv_dbg(panel, "notify PANEL_EVENT_PANEL_STATE_CHANGED (state:%d:%s)\n",
			evt_data.state, evt_name);
}

static inline void panel_send_esd_recovery_notify(struct panel_device *panel, u32 state)
{
	struct panel_notifier_event_data evt_data = {
		.display_index = 0,
		.state = state,
	...
	panel_drv_info(panel, "notify PANEL_EVENT_ESD_STATE_CHANGED (state:%d)\n", evt_data.state);
}

static inline void panel_send_ubconn_notify(struct panel_device *panel, u32 state)
{
	struct panel_notifier_event_data evt_data = {
		.display_index = 0,
		.state = state,
	...
	panel_drv_dbg(panel, "notify PANEL_EVENT_UB_CON_STATE_CHANGED (state:%d)\n", evt_data.state);
}

void panel_send_screen_mode_notify(struct panel_device *panel, u32 mode)
{
	struct panel_notifier_event_data evt_data = {
		.display_index = 0,
	...
	panel_drv_dbg(panel, "notify PANEL_EVENT_SCREEN_MODE_STATE_CHANGED (screen_mode:%d)\n",
...
static int panel_bind_mafpc(struct panel_device *panel, struct mafpc_info *info)
{
	...
	if (!info) {
		panel_drv_warn(panel, "mafpc is not supported\n");
		return 0;
	}

	np = of_find_compatible_node(NULL, NULL, "samsung,panel-mafpc");
	if (!np) {
		panel_drv_err(panel, "compatible(\"samsung,panel-mafpc\") node not found\n");
		return -ENOENT;
	}
	...
	of_node_put(np);
	if (!pdev) {
		panel_drv_err(panel, "mafpc device not found\n");
		return -ENODEV;
	}

	mafpc = (struct mafpc_device *)platform_get_drvdata(pdev);
	if (!mafpc) {
		panel_drv_err(panel, "failed to get mafpc device\n");
		return -ENODEV;
	}
	panel->mafpc = mafpc;
	mafpc->panel = panel;
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to mafpc probe\n");
		return -ENODEV;
	}
...
int __set_panel_fast_discharge(struct panel_device *panel, bool onoff)
{
	...
	if (ret < 0 && ret != -ENODATA) {
		panel_drv_warn(panel, "failed to execute %s, ret:%d\n", ctrl_name, ret);
		return ret;
	}
	panel_drv_info(panel, "%s done %d\n", ctrl_name, ret);
	return 0;
}
...
int __set_panel_elvxx_power_off(struct panel_device *panel)
{
	...
	if (ret < 0 && ret != -ENODATA) {
		panel_drv_warn(panel, "failed to execute panel_elvxx_power_off, ret:%d\n", ret);
		return ret;
	}

	panel_drv_info(panel, "done %d\n", ret);
	return 0;
}
...
int panel_firmware_info_print(struct panel_device *panel)
{
	...
	panel_drv_info(panel, "firmware_info_print ops is not defined.\n");
	return 0;
}
...
int panel_firmware_update(struct panel_device *panel)
{
	...
	panel_drv_info(panel, "firmware_update ops is not defined.\n");
	return 0;
}
#endif
...
int __set_panel_power(struct panel_device *panel, int power)
{
	...
	if (panel->state.power == power) {
		panel_drv_warn(panel, "same status.. skip..\n");
		return 0;
	}
	...
	panel_drv_info(panel, "power ctrl(%s) done\n", power_ctrl_name);
...
static int __panel_seq_display_on(struct panel_device *panel)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to run sequence(%s)\n",
				PANEL_DISPLAY_ON_SEQ);
		return ret;
	}

	panel_drv_info(panel, "%s done\n", PANEL_DISPLAY_ON_SEQ);
...
static int __panel_seq_init(struct panel_device *panel)
{
	...
	if (ret < 0 && ret != -ENODATA)
		panel_drv_err(panel, "failed to panel_power_init\n");
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to panel_reprobe\n");
		return ret;
	}
	...
	if (ret < 0 && ret != -ENOENT) {
		panel_drv_err(panel, "failed to panel_ddi_init\n");
	}
	...
	panel_drv_info(panel, "panel init: took %llu ms\n", time_diff);
	panel_mutex_unlock(&panel->op_lock);
	...
	time_diff = ktime_to_ms(ktime_sub(ktime_get(), timestamp));
	panel_drv_info(panel, "check disp_det .. success: took %llu ms\n", time_diff);
	...
	if (ret)
		panel_drv_err(panel, "failed to aod init_panel\n");
#endif
...
static int __panel_seq_exit(struct panel_device *panel)
{
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to panel_disable_irq\n");
	...
	panel_set_property(panel, &panel->panel_data.props.display_mode_slot, 0);
...
static int __panel_seq_hmd_on(struct panel_device *panel)
{
	...
	panel_drv_info(panel, "hmd was on, setting hmd on seq\n");
	ret = panel_do_seqtbl_by_name(panel, PANEL_HMD_ON_SEQ);
	if (ret)
		panel_drv_err(panel, "failed to set hmd on seq\n");
...
static int panel_set_hmd_on(struct panel_device *panel)
{
	...
	if (!panel) {
		panel_drv_err(panel, "panel is null\n");
		return 0;
	}
	state = &panel->state;
	...
	if (ret)
		panel_drv_err(panel, "fail to set hmd brightness\n");
...
static int __panel_seq_exit_alpm(struct panel_device *panel)
{
	...
	panel_drv_dbg(panel, "was called\n");

	ret = panel_drv_power_ctrl_execute(panel, "panel_power_exit_alpm_pre");
	if (ret < 0 && ret != -ENODATA)
		panel_drv_err(panel, "failed to panel_power_exit_alpm_pre\n");

#ifdef CONFIG_USDM_PANEL_SELF_DISPLAY
	ret = panel_aod_exit_from_lpm(panel);
	if (ret)
		panel_drv_err(panel, "failed to exit_lpm ops\n");
#endif
	...
	if (ret < 0 && ret != -ENODATA)
		panel_drv_err(panel, "failed to panel_power_exit_alpm\n");

	ret = panel_do_seqtbl_by_name_nolock(panel, PANEL_ALPM_EXIT_SEQ);
	if (ret)
		panel_drv_err(panel, "failed to alpm-exit\n");
	...
	if (check_seqtbl_exist(panel, PANEL_ALPM_EXIT_AFTER_SEQ)) {
		...
		if (ret)
			panel_drv_err(panel, "failed to alpm-exit-after seq\n");
	}
	panel_mutex_unlock(&panel->op_lock);
	panel_mutex_unlock(&panel_bl->lock);
...
static int __panel_seq_set_alpm(struct panel_device *panel)
{
	...
	if (check_seqtbl_exist(panel, PANEL_ALPM_INIT_SEQ)) {
		...
		if (ret)
			panel_drv_err(panel, "failed to alpm-init\n");
	}

	ret = panel_drv_power_ctrl_execute(panel, "panel_power_enter_alpm");
	if (ret < 0 && ret != -ENODATA)
		panel_drv_err(panel, "failed to panel_power_enter_alpm\n");
	...
	if (ret)
		panel_drv_err(panel, "failed to alpm-set-bl\n");
	...
	if (ret) {
		panel_drv_err(panel, "failed to enter to lpm\n");
		return ret;
	}
#endif
	panel_enable_irq(panel, PANEL_IRQ_DISP_DET);
...
static int __panel_seq_dump(struct panel_device *panel)
{
	...
	if (unlikely(ret < 0))
		panel_drv_err(panel, "failed to write dump seqtbl\n");
...
static int panel_debug_dump(struct panel_device *panel)
{
	...
	if (!IS_PANEL_ACTIVE(panel)) {
		panel_drv_info(panel, "Current state:%d\n", panel_get_cur_state(panel));
		goto dump_exit;
	}

	ret = __panel_seq_dump(panel);
	if (ret) {
		panel_drv_err(panel, "failed to dump\n");
		return ret;
	}

	panel_drv_info(panel, "disp_det_state:%s\n",
...
int panel_seq_cmdlog_dump(struct panel_device *panel)
{
	...
	if (unlikely(ret < 0))
		panel_drv_err(panel, "failed to write cmdlog dump seqtbl\n");

	return ret;
}
#endif

__visible_for_testing int _panel_display_on(struct panel_device *panel)
{
	int ret = 0;
	struct panel_state *state;
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_OFF ||
		...
		panel_drv_warn(panel, "invalid state\n");
		goto do_exit;
	}
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_ALPM) {
		...
		if (ret)
			panel_drv_warn(panel, "failed to black grid on\n");
	}
#endif

	ret = __panel_seq_display_on(panel);
	if (ret) {
		panel_drv_err(panel, "failed to display on\n");
		return ret;
	}
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_ALPM) {
		...
		if (ret)
			panel_drv_warn(panel, "failed to black grid off\n");
	}
#endif
...
int panel_display_on(struct panel_device *panel)
{
	...
	panel_mutex_lock(&panel->state_lock);
	ret = _panel_display_on(panel);
	panel_mutex_unlock(&panel->state_lock);

	return ret;
}

__visible_for_testing int _panel_display_off(struct panel_device *panel)
{
	int ret = 0;
	struct panel_state *state;
	...
	if (unlikely(ret < 0))
		panel_drv_err(panel, "failed to write display off seqtbl\n");
...
static int update_panel_id(struct panel_device *panel)
{
	...
	if (!panel_is_detected_in_lk(panel)) {
		panel_drv_warn(panel, "unable to update panel_id in pba booting\n");
		return 0;
	}

	if (check_seqtbl_exist(panel, PANEL_PRE_UPDATE_ID_SEQ))
		panel_do_seqtbl_by_name(panel, PANEL_PRE_UPDATE_ID_SEQ);

	memset(id, 0, sizeof(id));
	ret = read_panel_id(panel, id);
	if (ret < 0) {
		panel_drv_err(panel, "failed to read id(ret:%d)\n", ret);
...
static struct common_panel_info *panel_detect(struct panel_device *panel)
{
	...
	if (!info) {
		panel_drv_err(panel, "panel not found(id:0x%06X)\n", panel_id);
		return NULL;
	}
		...
		struct maptbl *src)
{
	...
	if (!dst) {
		panel_drv_err(panel, "failed to clone maptbl(%s)\n",
				maptbl_get_name(src));
		return -EINVAL;
	}
	dst->pdata = panel;
		...
		struct delayinfo *src)
{
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create delay(%s)\n",
				get_delay_name(src));
		return -EINVAL;
	}
		...
		struct timer_delay_begin_info *src)
{
	...
	if (!src->delay) {
		panel_drv_err(panel, "invalid timer_delay_begin(%s)\n",
	...
	pnobj = pnobj_find_by_pnobj(&panel->dly_list,
			&src->delay->base);
	if (!pnobj) {
		panel_drv_err(panel, "%s not found in delay list\n",
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create timer_delay_begin(%s)\n",
				get_pnobj_name(&src->base));
		return -EINVAL;
	}
		...
		struct condinfo *src)
{
	...
	if (!dst) {
		panel_drv_err(panel, "failed to duplicate condition(%s)\n",
		...
		struct rdinfo *src)
{
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create rx packet(%s)\n",
				get_pnobj_name(&src->base));
		return -EINVAL;
	}
		...
		struct pktinfo *src)
{
	...
	if (src->nr_pktui) {
		...
		for (i = 0; i < src->nr_pktui; i++) {
			...
			if (!pnobj) {
				panel_drv_err(panel, "%s not found in maptbl list\n",
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create tx packet(%s)\n",
				get_pnobj_name(&src->base));
		kfree(pktui);
		return -EINVAL;
	}
		...
		struct keyinfo *src)
{
	...
	if (!src->packet) {
		panel_drv_err(panel, "packet is null\n");
		return -EINVAL;
	}

	pnobj = pnobj_find_by_pnobj(&panel->pkt_list, &src->packet->base);
	if (!pnobj) {
		panel_drv_err(panel, "%s not found in packet list\n",
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create key\n");
		return -EINVAL;
	}
		...
		struct resinfo *src)
{
	...
	if (src->nr_resui) {
		...
		for (i = 0; i < src->nr_resui; i++) {
			...
			if (!pnobj) {
				panel_drv_err(panel, "%s not found in resource list\n",
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create resource(%s)\n",
				get_pnobj_name(&src->base));
		goto err;
	}
		...
		struct dumpinfo *src)
{
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create dumpinfo(%s)\n",
				get_pnobj_name(&src->base));
		return -EINVAL;
	}
		...
		struct pwrctrl *src)
{
	...
	if (!dst) {
		panel_drv_err(panel, "failed to duplicate pwrctrl(%s)\n",
		...
		struct pnobj_config *src)
{
	...
	if (!prop) {
		panel_drv_err(panel, "property(%s) not found\n", src->prop_name);
		return -EINVAL;
	}
		...
		struct seqinfo *src)
{
	...
	if (src->size == 0) {
		panel_drv_warn(panel, "%s skip empty sequence\n",
	...
	if (!dst) {
		panel_drv_err(panel, "failed to create sequence(%s)\n",
				get_pnobj_name(&src->base));
		return -EINVAL;
	}
	...
	for (i = 0, j = 0; i < src->size && src->cmdtbl[i]; i++) {
		...
		if (type == CMD_TYPE_NONE || type >= MAX_CMD_TYPE) {
			panel_drv_warn(panel, "%s:%d:%s invalid panel object\n",
		...
		if (!pnobj_list) {
			panel_drv_err(panel, "%s:%d:%s failed to get panel object list (type:%d)\n",
		...
		if (!pnobj) {
			panel_drv_err(panel, "%s:%d:%s not found in pnobj_list\n",
					get_sequence_name(src), i,
					get_pnobj_name(command));
			ret = -EINVAL;
			goto err;
		...
		struct common_panel_info *cpi)
{
	...
	if (!cpi) {
		panel_drv_err(panel, "common_panel_info is null\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < MAX_USDM_DRV_LEVEL; i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to add prop array(USDM_DRV:%s)\n",
...
__visible_for_testing int panel_unprepare_prop_list(struct panel_device *panel)
{
	...
	for (i = 0; i < MAX_USDM_DRV_LEVEL; i++) {
		...
		ret = panel_delete_property_from_array(panel,
				...
				cpi->num_prop_lists[i]);
		if (ret < 0) {
			panel_drv_err(panel, "failed to delete prop array(USDM_DRV:%s)\n",
					usdm_drv_level_to_str(i));
			return ret;
		}
	}
		...
		struct common_panel_info *info)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to add panel maptbl on initdata_list\n");
		return ret;
	}
	...
	for (i = 0; i < info->nr_rditbl; i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to add_command_list(%s:%s)\n",
				cmd_type_to_string(get_pnobj_cmd_type(pnobj)),
				get_pnobj_name(pnobj));
			return ret;
		}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to add panel sequence on initdata_list\n");
		return ret;
	}
	...
	list_for_each_entry(pnobj, &panel->command_initdata_list, list) {
		...
		if (IS_CMD_TYPE_MAP(type)) {
			...
			for (i = 0; i < ARRAY_SIZE(f); i++) {
				...
				if (!found) {
					panel_drv_err(panel, "%s not exist in panel_function_list\n", name);
					return -EINVAL;
				}
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to add function of pnobj(%s:%s)\n",
					cmd_type_to_string(get_pnobj_cmd_type(pnobj)),
					get_pnobj_name(pnobj));
			return ret;
		}

		panel_drv_dbg(panel, "pnobj(%s:%s) add function list\n",
				cmd_type_to_string(get_pnobj_cmd_type(pnobj)),
				get_pnobj_name(pnobj));
	}
		...
		struct common_panel_info *info)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to sort sequences\n");
		return -EINVAL;
	}
...
__visible_for_testing int panel_prepare_pnobj_list(struct panel_device *panel)
{
	...
	list_for_each_entry(pnobj, &panel->command_initdata_list, list) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to duplicate pnobj(%s:%s)\n",
				cmd_type_to_string(get_pnobj_cmd_type(pnobj)),
				get_pnobj_name(pnobj));
			return ret;
		}

		panel_drv_dbg(panel, "pnobj(%s:%s) duplication succeeded\n",
				cmd_type_to_string(get_pnobj_cmd_type(pnobj)),
				get_pnobj_name(pnobj));
	}
	panel_drv_info(panel, "panel object duplication succeeded\n");
...
static int panel_alloc_command_buffer(struct panel_device *panel)
{
	...
	if (fifo_size <= 0) {
		panel_drv_err(panel, "invalid fifo_size(%d)\n", fifo_size);
		return -EINVAL;
	}

	if (fifo_size > SZ_8K) {
		panel_drv_warn(panel, "fifo_size(%d) is too big\n", fifo_size);
		fifo_size = SZ_8K;
	}
	panel_drv_info(panel, "fifo_size %d\n", fifo_size);
...
static int panel_replace_pnobj_list(struct panel_device *panel, struct list_head *pnobj_list)
{
	...
	list_for_each_entry_safe(pos, next, pnobj_list, list) {
		if (IS_CMD_TYPE_MAP(get_pnobj_cmd_type(pos))) {
			...
			maptbl_for_each_dimen(m, i) {
				...
				if (!property) {
					panel_drv_err(panel, "failed to find property(%s)\n",
							m->props.name[i]);
					return -EINVAL;
				}
	...
	list_for_each_entry_safe(pos, next, pnobj_list, list) {
		...
		if (!list) {
			panel_drv_err(panel, "failed to get list head(%s)\n",
					get_pnobj_name(pos));
			return -EINVAL;
		}
		...
		if (IS_CMD_TYPE_PROP(type)) {
			...
			if (!prop1) {
				panel_drv_err(panel, "property(%s) mismatch\n",
						get_pnobj_name(pos));
...
__visible_for_testing int panel_prepare(struct panel_device *panel, struct common_panel_info *cpi)
{
	...
	if (!panel->cmdbuf) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to alloc command buffer\n");
			panel_mutex_unlock(&panel->op_lock);
			return ret;
		}
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to prepare prop list\n");
		panel_mutex_unlock(&panel->op_lock);
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to setup command initdata list\n");
		panel_mutex_unlock(&panel->op_lock);
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to sort command initdata list\n");
		panel_mutex_unlock(&panel->op_lock);
		return ret;
	}
	...
	ret = panel_firmware_load(panel,
			...
			&new_pnobj_list);
	panel_drv_info(panel, "firmware loading done %d\n", ret);
#endif

	if (is_panel_firmwarel_load_success(panel)) {
		ret = panel_replace_pnobj_list(panel, &new_pnobj_list);
		if (ret < 0) {
			panel_drv_err(panel, "failed to replace panel object list\n");
			panel_mutex_unlock(&panel->op_lock);
			return ret;
		}
	} else {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to prepare panel object list\n");
			panel_mutex_unlock(&panel->op_lock);
			return ret;
		}
	}
...
__visible_for_testing int panel_unprepare(struct panel_device *panel)
{
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to unprepare pnobj list\n");

	ret = panel_unprepare_prop_list(panel);
	if (ret < 0)
		panel_drv_err(panel, "failed to unprepare prop list\n");

#ifdef CONFIG_USDM_PANEL_BLIC
	ret = panel_blic_unprepare(panel);
	if (ret < 0)
		panel_drv_err(panel, "failed to unprepare panel_blic\n");
#endif
#ifdef CONFIG_USDM_MDNIE
	ret = mdnie_unprepare(&panel->mdnie);
	if (ret < 0)
		panel_drv_err(panel, "failed to unprepare mdnie\n");
#endif
#ifdef CONFIG_USDM_PANEL_SELF_DISPLAY
	ret = aod_drv_unprepare(panel);
	if (ret < 0)
		panel_drv_err(panel, "failed to unprepare aod_drv\n");
#endif
#ifdef CONFIG_USDM_PANEL_COPR
	ret = copr_unprepare(panel);
	if (ret < 0)
		panel_drv_err(panel, "failed to unprepare copr\n");
#endif
...
static int panel_maptbl_init(struct panel_device *panel)
{
	...
	list_for_each_entry(pnobj, &panel->maptbl_list, list) {
		...
		ret = maptbl_init(m);
		if (ret == -ENODATA)
			panel_drv_warn(panel, "empty maptbl(%s)\n", maptbl_get_name(m));
		else if (ret < 0)
			panel_drv_err(panel, "failed to init maptbl(%s)\n", maptbl_get_name(m));
	}
	panel_mutex_unlock(&panel->op_lock);
...
int panel_mtp_gamma_check(struct panel_device *panel)
{
	...
	if (!ops->mtp_gamma_check) {
		panel_drv_warn(panel, "not supported\n");
		return -1;
	}
...
int panel_default_ssr_test(struct panel_device *panel)
{
	...
	if (!check_seqtbl_exist(panel, PANEL_SSR_TEST_SEQ)) {
		panel_drv_warn(panel, "sequence(%s) not exist\n", PANEL_SSR_TEST_SEQ);
		return -ENOENT;
	}

	ret = panel_init_dumpinfo(panel, "ssr");
	if (ret < 0) {
		panel_drv_err(panel, "failed to init dump\n");
		return ret;
	}
...
int panel_get_temperature_range(struct panel_device *panel, char *buf)
{
	...
	if (!ops->get_temperature_range) {
		panel_drv_info(panel, "use default temperature range\n");
...
int panel_ddi_init(struct panel_device *panel)
{
	...
	if (!ops->ddi_init) {
		panel_drv_dbg(panel, "not supported\n");
		return -ENOENT;
	}
...
int panel_update_dim_type(struct panel_device *panel, u32 dim_type)
{
	...
	if (dim_type == DIM_TYPE_DIM_FLASH) {
		if (!panel->dim_flash_result) {
			panel_drv_err(panel, "dim buffer not found\n");
			return -ENOMEM;
		}
		...
		for (index = 0; index < panel->max_nr_dim_flash_result; index++) {
			...
			do {
				...
				if (unlikely(ret < 0)) {
					panel_drv_err(panel, "failed to read gamma flash(ret %d)\n", ret);
				...
				if (unlikely(ret < 0)) {
					panel_drv_err(panel, "failed to check dim_flash exist\n");
					state = GAMMA_FLASH_ERROR_READ_FAIL;
					break;
				}

				if (ret == PARTITION_WRITE_CHECK_NOK) {
					panel_drv_err(panel, "dim partition not exist(%d)\n", ret);
...
#if !defined(CONFIG_SEC_PANEL_DIM_FLASH_NO_VALIDATION)
				if (unlikely(ret < 0)) {
					panel_drv_err(panel, "failed to get chksum(ret %d)\n", ret);
					state = GAMMA_FLASH_ERROR_READ_FAIL;
					break;
				}
				if (result->dim_chksum_by_calc !=
					result->dim_chksum_by_read) {
					panel_drv_err(panel, "dim flash checksum(%04X,%04X) mismatch\n",
							result->dim_chksum_by_calc,
							result->dim_chksum_by_read);
				...
				if (unlikely(ret)) {
					panel_drv_err(panel, "failed to read mtp flash(ret %d)\n", ret);
					state = GAMMA_FLASH_ERROR_READ_FAIL;
					break;
				}
				...
				if (result->mtp_chksum_by_calc != result->mtp_chksum_by_read) {
					panel_drv_err(panel, "mtp flash checksum(%04X,%04X) mismatch\n",
				...
				if (unlikely(ret < 0)) {
					panel_drv_err(panel, "failed to get resource mtp size (ret %d)\n", ret);
					state = GAMMA_FLASH_ERROR_READ_FAIL;
					break;
				}
				sz_mtp_reg = ret;
				...
				if (unlikely(ret < 0)) {
					panel_drv_err(panel, "failed to copy resource mtp (ret %d)\n", ret);
					state = GAMMA_FLASH_ERROR_READ_FAIL;
					break;
				}
				...
				if (cmp_poc_partition_data(&panel->poc_dev,
					...
					panel_drv_err(panel, "mismatch mtp(ret %d)\n", ret);
					state = GAMMA_FLASH_ERROR_MTP_OFFSET;
					break;
				}
#endif
		...
		if (unlikely(ret)) {
			panel_drv_err(panel, "failed to resource init\n");
...
int panel_dt_lut_get_option_value(struct panel_device *panel, const char *option_name)
{
	...
	if (!options_np) {
		panel_drv_dbg(panel, "options node is not found\n");
		return -ENODEV;
	}

	ret = of_property_read_u32(options_np, option_name, &option_value);
	if (ret < 0) {
		panel_drv_dbg(panel, "option %s is not found in options node\n", option_name);
		return -ENODATA;
	}

	panel_drv_dbg(panel, "option %s: %d\n", option_name, option_value);
	return option_value;
}
...
int panel_reprobe(struct panel_device *panel)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to update panel_id\n");
		return -EINVAL;
	}
#endif
	...
	if (unlikely(!info)) {
		panel_drv_err(panel, "panel detection failed\n");
		return -ENODEV;
	}

	/* detected panel is changed */
	if (panel->cpi != info) {
		panel_drv_info(panel, "common_panel_info changed(%s->%s)\n",
				panel->cpi->name, info->name);

		ret = panel_unprepare(panel);
		if (unlikely(ret)) {
			panel_drv_err(panel, "failed to panel_unprepare\n");
			return ret;
		}
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to init copr driver\n");
			PANEL_BUG();
			return -ENODEV;
		}
#endif
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to prepare aod_drv\n");
			return ret;
		}
#endif
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to prepare mdnie\n");
			return -ENODEV;
		}
#endif
		...
		if (unlikely(ret)) {
			panel_drv_err(panel, "failed to panel_prepare\n");
			return ret;
		}
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove poc device\n");
		return ret;
	}

	ret = panel_poc_probe(panel, info->poc_data);
	if (unlikely(ret)) {
		panel_drv_err(panel, "failed to probe poc driver\n");
		return -ENODEV;
	}
	...
	ret = panel_maptbl_init(panel);
	if (unlikely(ret)) {
		panel_drv_err(panel, "failed to maptbl init\n");
		return -ENODEV;
	}

	ret = panel_bl_remove(&panel->panel_bl);
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove panel-bl driver\n");
		return ret;
	}

	ret = panel_bl_probe(&panel->panel_bl);
	if (unlikely(ret)) {
		panel_drv_err(panel, "failed to probe backlight driver\n");
		return -ENODEV;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to create debugfs\n");
		return ret;
	}
#endif
		...
		struct list_head *pnobj_list)
{
	...
	ret = panel_unprepare(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to unprepare panel\n");
		return ret;
	}
...
static void dim_flash_handler(struct work_struct *work)
{
	...
	if (atomic_read(&w->running) >= 2) {
		panel_drv_info(panel, "already running\n");
		panel_mutex_unlock(&panel->panel_bl.lock);
		return;
	}
	atomic_set(&w->running, 2);
	panel_drv_info(panel, "+\n");
	ret = panel_update_dim_type(panel, DIM_TYPE_DIM_FLASH);
	if (ret < 0) {
		panel_drv_err(panel, "failed to update dim_flash %d\n", ret);
		w->ret = ret;
	} else {
		panel_drv_info(panel, "update dim_flash done %d\n", ret);
		w->ret = ret;
	}
	panel_drv_info(panel, "-\n");
...
bool show_copr_value(struct panel_device *panel, int frame_cnt)
{
	...
	if (copr_is_enabled(copr)) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to get copr\n");
			return retVal;
		}
		...
		panel_drv_info(panel, "copr(frame_cnt=%d) -> %s", frame_cnt, write_buf);
		if (copr->props.cur_copr > 0) /* not black */
			retVal = true;
	} else {
		panel_drv_info(panel, "copr do not support\n");
	}
#else
	panel_drv_info(panel, "copr feature is disabled\n");
#endif
	return retVal;
}
...
static void panel_condition_handler(struct work_struct *work)
{
	...
	case PRINT_NORMAL_PANEL_INFO:
		...
		if (unlikely(ret < 0))
			panel_drv_err(panel, "failed to write panel check\n");
...
static void panel_check_start(struct panel_device *panel)
{
	...
	if (pcc->frame_cnt < 100) {
		...
		case NO_CHECK_STATE:
			...
			break;
		default:
			panel_drv_warn(panel, "state is invalid %d %d %d\n",
		...
		if (pcc->check_state == CHECK_NORMAL_PANEL_INFO)
			panel_drv_warn(panel, "screen is black in first 100 frame %d %d %d\n",
					pcc->is_panel_check, pcc->frame_cnt, pcc->check_state);
		else
			panel_drv_warn(panel, "state is invalid %d %d %d\n",
					pcc->is_panel_check, pcc->frame_cnt, pcc->check_state);
...
static void panel_update_handler(struct work_struct *work)
{
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_OFF ||
		panel_get_cur_state(panel) == PANEL_STATE_ON) {
		panel_drv_info(panel, "state %d, skip update\n", panel_get_cur_state(panel));
		goto exit;
	}
	...
	if (vrr_updated) {
		...
		if (ret < 0)
			panel_drv_err(panel, "failed to update vrr & brightness\n");
		panel_mutex_unlock(&panel->op_lock);
	}
exit:
	panel_mutex_unlock(&panel_bl->lock);
...
static int panel_init_property(struct panel_device *panel)
{
	...
	panel_set_property(panel, &panel_data->props.display_mode_slot, 0);
...
static int panel_set_node_from_panel_lut(struct panel_device *panel)
{
	...
	if (IS_ERR_OR_NULL(panel_lut)) {
		panel_drv_err(panel, "failed to find panel_lut\n");
		return -EINVAL;
	}
	...
	if (panel_lut->panel_modes_node) {
		...
		if (!panel->panel_modes)
			panel_drv_warn(panel, "failed to get panel_display_modes\n");
	}
#endif
...
__visible_for_testing int panel_create_lcd_device(struct panel_device *panel, unsigned int id)
{
	...
	if (IS_ERR_OR_NULL(panel->lcd_dev)) {
		panel_drv_err(panel, "failed to create lcd device\n");
...
int panel_probe(struct panel_device *panel)
{
	...
	if (panel->probe_done) {
		panel_drv_info(panel, "already done.\n");
		return 0;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse power_ctrl\n");
		return ret;
	}

	ret = panel_initialize_regulator(panel);
	if (ret < 0) {
		panel_drv_warn(panel, "error occurred during initialize regulator\n");
		return ret;
	}

	ret = panel_drv_set_regulators(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to set regulators\n");
		return ret;
	}
#endif
	...
	if (!panel->spi)
		panel_drv_warn(panel, "panel spi device unsupported\n");
#endif

	ret = panel_init_property(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to initialize property\n");
		return -EINVAL;
	}
	...
	ret = panel_prepare(panel, info);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to prepare common panel driver\n");
		PANEL_BUG();
		return -ENODEV;
	}
	...
	if (unlikely(ret))
		panel_drv_err(panel, "failed to probe panel spi driver\n");
	...
	if (panel_dt_lut_get_option_value(panel, "probe-self-display") != 0) {
		...
		if (unlikely(ret)) {
			panel_drv_err(panel, "failed to probe aod driver\n");
			PANEL_BUG();
			return -ENODEV;
		}
	} else
		panel_drv_info(panel, "probe-self-display was set to 0 in dt\n");
#endif
	...
	if (panel_dt_lut_get_option_value(panel, "probe-copr") != 0) {
		...
		if (unlikely(ret)) {
			panel_drv_err(panel, "failed to probe copr driver\n");
			PANEL_BUG();
			return -ENODEV;
		}
	} else
		panel_drv_info(panel, "probe-copr was set to 0 in dt\n");
#endif
	...
	if (unlikely(ret)) {
		panel_drv_err(panel, "failed to panel boot on seq\n");
		return -ENODEV;
	}
...
/* check options node in panel-lut DT, do not probe mdnie when 'probe-mdnie' property was set as 0 */
	if (panel_dt_lut_get_option_value(panel, "probe-mdnie") != 0) {
		...
		if (unlikely(ret)) {
			panel_drv_err(panel, "failed to probe mdnie driver\n");
			return -ENODEV;
		}
	} else
		panel_drv_info(panel, "probe-mdnie was set to 0 in dt\n");
#endif

#ifdef CONFIG_USDM_PANEL_MAFPC
	ret = panel_bind_mafpc(panel, info->mafpc_info);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to probe mafpc driver\n");
		return -ENODEV;
	}
#endif
	...
	if (ret)
		panel_drv_err(panel, "failed to register dynamic mipi module\n");
#endif

#if defined(CONFIG_USDM_SDP_ADAPTIVE_MIPI)
	ret = probe_sdp_adaptive_mipi(panel);
	if (ret)
		panel_drv_err(panel, "failed to probe sdp adaptive mipi\n");
#elif defined(CONFIG_USDM_ADAPTIVE_MIPI)
	ret = probe_adaptive_mipi(panel, info->rf_elements, info->rf_element_nr);
	if (ret)
		panel_drv_err(panel, "failed to probe adaptive mipi\n");
#endif
	...
	for (i = 0; i < MAX_PANEL_BL_SUBDEV; i++) {
		...
		if (panel_data->panel_dim_info[i]->dim_flash_on) {
			...
			panel_drv_info(panel, "dim_flash : on\n");
			break;
		}
	}
	panel_mutex_unlock(&panel->op_lock);
	...
	if (ret >= 0) {
		...
		panel_drv_info(panel, "apply default panel_mode %d\n", ret);

	} else {
		panel_drv_err(panel, "failed to update panel_mode\n");
	}
	panel_mutex_unlock(&panel->op_lock);
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to panel_update_display_mode\n");

	ret = panel_seq_display_mode_post(panel);
	if (ret < 0 && ret != -ENOENT)
		panel_drv_err(panel, "failed to panel_update_display_mode_post\n");
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to probe_lpd_panel_cmd\n");
#endif
...
static void panel_late_probe_handler(struct work_struct *work)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse regulator\n");
		return;
	}
	...
	list_for_each_entry(panel_itr, &panel_device_list, list) {
		list_for_each_entry(lut_info, &panel_itr->panel_lut_list, head) {
			...
			if (!strncmp(lut_info->name, info->name, 128)) {
				...
				found = true;
				panel_info("found(panel-%d)(%s)(%s)\n", panel_itr->id, lut_info->name, info->name);
			}
		}
	}
...
int panel_remove(struct panel_device *panel)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove aod driver\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove copr driver\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove mdnie driver\n");
		return ret;
	}
#endif
	...
	ret = panel_poc_remove(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove poc driver\n");
		return ret;
	}
#endif /* CONFIG_USDM_PANEL_DDI_FLASH */
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove panel spi driver\n");
		return ret;
	}
#endif
	panel->funcs = NULL;
...
__visible_for_testing int _panel_sleep_in(struct panel_device *panel)
{
	int ret = 0;
	struct panel_state *state;
	...
	PRINT_PANEL_STATE_BEGIN(panel, prev_state, PANEL_STATE_ON, start);

	switch (panel_get_cur_state(panel)) {
	case PANEL_STATE_ON:
		panel_drv_warn(panel, "panel already %s state\n", panel_state_names[panel_get_cur_state(panel)]);
		goto do_exit;
	case PANEL_STATE_NORMAL:
	case PANEL_STATE_ALPM:
		...
		ret = _panel_display_off(panel);
		if (unlikely(ret < 0))
			panel_drv_err(panel, "failed to write display_off seqtbl\n");
		ret = __panel_seq_exit(panel);
		if (unlikely(ret < 0))
			panel_drv_err(panel, "failed to write exit seqtbl\n");
		break;
	default:
		panel_drv_err(panel, "invalid state(%d)\n", panel_get_cur_state(panel));
		goto do_exit;
	}

	panel_set_cur_state(panel, PANEL_STATE_ON);
	PRINT_PANEL_STATE_END(panel, prev_state, panel_get_cur_state(panel), start);
...
__visible_for_testing int panel_sleep_in(struct panel_device *panel)
{
	...
	panel_mutex_lock(&panel->state_lock);
	ret = _panel_sleep_in(panel);
	panel_mutex_unlock(&panel->state_lock);

	return ret;
}

__visible_for_testing int _panel_power_on(struct panel_device *panel)
{
	int ret = 0;
	struct panel_state *state;
	enum panel_active_state prev_state;
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_OFF) {
		...
		if (ret) {
			panel_drv_err(panel, "failed to panel power on\n");
			goto do_exit;
		}
		panel_set_cur_state(panel, PANEL_STATE_ON);
	}
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
	panel_send_ubconn_notify(panel, panel->state.connected == PANEL_STATE_OK ?
...
do_exit:
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
	panel_send_ubconn_notify(panel, PANEL_EVENT_UB_CON_STATE_DISCONNECTED);
#endif
	return ret;
}
...
__visible_for_testing int panel_power_on(struct panel_device *panel)
{
	...
	panel_mutex_lock(&panel->state_lock);
	ret = _panel_power_on(panel);
	panel_mutex_unlock(&panel->state_lock);

	return ret;
}

__visible_for_testing int _panel_power_off(struct panel_device *panel)
{
	int ret = 0;
	struct panel_state *state;
	enum panel_active_state prev_state;
	...
	state = &panel->state;
	prev_state = panel_get_cur_state(panel);
	PRINT_PANEL_STATE_BEGIN(panel, prev_state, PANEL_STATE_OFF, start);

	switch (panel_get_cur_state(panel)) {
	case PANEL_STATE_OFF:
		panel_drv_info(panel, "panel already %s state\n", panel_state_names[panel_get_cur_state(panel)]);
		goto do_exit;
	case PANEL_STATE_ALPM:
	case PANEL_STATE_NORMAL:
		_panel_sleep_in(panel);
	case PANEL_STATE_ON:
		ret = __set_panel_power(panel, PANEL_POWER_OFF);
		if (ret) {
			panel_drv_err(panel, "failed to panel power off\n");
			goto do_exit;
		}
		break;
	default:
	...
	if (ret)
		panel_drv_err(panel, "failed to aod power off\n");
#endif
	PRINT_PANEL_STATE_END(panel, prev_state, panel_get_cur_state(panel), start);
...
__visible_for_testing int panel_reset_lp11(struct panel_device *panel)
{
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_ON) {
		...
		if (ret < 0 && ret != -ENODATA)
			panel_drv_err(panel, "failed to panel_reset_lp11\n");
	}
...
__visible_for_testing int _panel_sleep_out(struct panel_device *panel)
{
	int ret = 0;
	struct panel_state *state;
	enum panel_active_state prev_state;
	...
	state = &panel->state;
	prev_state = panel_get_cur_state(panel);
	PRINT_PANEL_STATE_BEGIN(panel, prev_state, PANEL_STATE_NORMAL, start);
	...
	case PANEL_STATE_ALPM:
		...
		if (ret) {
			panel_drv_err(panel, "failed to panel exit alpm\n");
			goto do_exit;
		}
#endif
		break;
	case PANEL_STATE_OFF:
		ret = _panel_power_on(panel);
		if (ret) {
			panel_drv_err(panel, "failed to power on\n");
			goto do_exit;
		}
	case PANEL_STATE_ON:
		...
		if (ret) {
			...
			if (pcd_state == PANEL_STATE_NOK) {
				...
				if (panel->panel_data.ddi_props.no_set_pcd_bypass == false)
					panel_dsi_set_bypass(panel, true); /* ignore TE checking & frame update request*/
				panel_drv_err(panel, "failed to panel init seq. but PCD is NOT OK. keep going.\n");

				/* Below irq is not enabled in "__panel_seq_init", so turn them on here*/
				panel_enable_irq(panel, PANEL_IRQ_ALL_WITHOUT_CONN_DET);
			} else {
				panel_drv_err(panel, "failed to panel init seq\n");
				PANEL_BUG();
			}
		}
...
#ifdef CONFIG_USDM_PANEL_LPM
__visible_for_testing int _panel_doze(struct panel_device *panel)
{
	int ret = 0;
	struct panel_state *state;
	enum panel_active_state prev_state;
	...
	state = &panel->state;
	prev_state = panel_get_cur_state(panel);
	PRINT_PANEL_STATE_BEGIN(panel, prev_state, PANEL_STATE_ALPM, start);
	...
	case PANEL_POWER_OFF:
		ret = _panel_sleep_out(panel);
		if (ret) {
			panel_drv_err(panel, "failed to set normal\n");
			goto do_exit;
		}
	case PANEL_STATE_NORMAL:
		ret = __panel_seq_set_alpm(panel);
		if (ret)
			panel_drv_err(panel, "failed to write alpm\n");
...
static int panel_register_cb(struct panel_device *panel, int cb_id, void *arg)
{
	...
	if (cb_id >= MAX_PANEL_CB) {
		panel_drv_err(panel, "out of range (cb_id:%d)\n", cb_id);
		return -EINVAL;
	}
...
int panel_vrr_cb(struct panel_device *panel)
{
	...
	if (vrr_cb_info->cb) {
		...
		if (ret < 0)
			panel_drv_err(panel, "failed to vrr callback\n");
	}
	...
	if (old_dms_data[panel->id].fps != dms_data.fps) {
		...
		panel_drv_dbg(panel, "PANEL_EVENT_VRR_STATE_CHANGED fps:%d lfd_freq:%d~%dHz\n",
	...
	if (memcmp(&old_dms_data[panel->id], &dms_data, sizeof(dms_data))) {
		...
		panel_drv_dbg(panel, "PANEL_EVENT_LFD_STATE_CHANGED fps:%d lfd_freq:%d~%dHz\n",
				dms_data.fps, dms_data.lfd_min_freq, dms_data.lfd_max_freq);
	}
...
int panel_get_rcd_info(struct panel_device *panel, void *arg)
{
	...
	if (!rcd_info) {
		panel_drv_err(panel, "invalid argument\n");
		return -EINVAL;
	}

	if (!panel->panel_data.rcd_data) {
		panel_drv_info(panel, "rcd_data is empty\n");
		return -ENODATA;
	}
...
int panel_set_brightness(struct panel_device *panel, unsigned int level)
{
	...
	if (level > panel->panel_bl.bd->props.max_brightness) {
		panel_drv_err(panel, "level(%d) is over max(%d)!\n",
...
int panel_get_display_mode(struct panel_device *panel, void *arg)
{
	...
	if (!panel->panel_modes) {
		panel_drv_err(panel, "panel_modes not prepared\n");
		return -EINVAL;
	}
...
int panel_display_mode_cb(struct panel_device *panel)
{
	...
	if (!panel_display_mode_is_supported(panel)) {
		panel_drv_err(panel, "panel_display_mode not supported\n");
		return -EINVAL;
	}

	if (panel_mode < 0 ||
			panel_mode >= common_panel_modes->num_modes) {
		panel_drv_err(panel, "invalid panel_mode %d\n", panel_mode);
		return -EINVAL;
	}
	...
	if (cb_info->cb) {
		...
		if (ret)
			panel_drv_err(panel, "failed to display_mode callback\n");
	}
	panel_vrr_cb(panel);
...
static bool check_display_mode_cond(struct panel_device *panel)
{
	...
	if (props->alpm_mode != ALPM_OFF) {
		panel_drv_warn(panel, "could not change display mode in lpm(%d) state\n",
			   props->alpm_mode);
		return false;
	}
#endif
	if (props->mcd_on == true) {
		panel_drv_warn(panel, "could not change display mode in mcd(%d) state\n",
			   props->mcd_on);
		return false;
	}
		...
		struct panel_display_mode *pdm)
{
	...
	if (!common_panel_modes) {
		panel_drv_err(panel, "common_panel_modes not prepared\n");
		return NULL;
	}
	...
	for (i = 0; i < common_panel_modes->num_modes; i++) {
		...
		if (!strncmp(common_panel_modes->modes[i]->name,
			...
			panel_drv_dbg(panel, "pdm:%s cpdm:%d:%s\n",
		...
		struct panel_display_mode *pdm)
{
	...
	if (!cpdm) {
		panel_drv_err(panel, "panel_display_mode(%s) not found\n", pdm->name);
		return -EINVAL;
	}
	panel_drv_dbg(panel, "%s:%s\n", pdm->name, cpdm->name);
...
static struct panel_display_mode *panel_get_default_pdm(struct panel_device *panel)
{
	...
	if (!panel_modes) {
		panel_drv_err(panel, "panel_modes is null\n");
		return NULL;
	}

	if (panel_modes->num_modes <= 0) {
		panel_drv_err(panel, "panel_modes->num_modes is 0\n");
		return NULL;
	}

	if (panel_modes->native_mode >=
			panel_modes->num_modes) {
		panel_drv_err(panel, "panel_modes->native_mode(%d) is out of range\n",
...
int panel_display_mode_get_native_mode(struct panel_device *panel)
{
	...
	if (panel_mode < 0) {
		panel_drv_err(panel, "could not find panel_display_mode(%s)\n", pdm->name);
		return -EINVAL;
	}
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to set %s\n", PANEL_PROPERTY_DSI_OP_MODE);
...
int panel_display_mode_get_vrr_idx(struct panel_device *panel, int panel_mode)
{
	...
	if (vrr == NULL) {
		panel_drv_err(panel, "vrr is null of panel_mode(%d)\n", panel_mode);
		return -EINVAL;
	}
	...
	if (i == num_vrrs) {
		panel_drv_warn(panel, "panel_mode(%d) vrr(%d%s) not found\n",
		...
		int panel_mode, char *buf, size_t size)
{
	...
	if (mres_idx < 0 || mres_idx >= mres->nr_resol) {
		panel_drv_err(panel, "out of range mres_idx(%d)\n", mres_idx);
		return 0;
	}
	...
	if (vrr_idx < 0 || vrr_idx >= panel->panel_data.nr_vrrtbl) {
		panel_drv_err(panel, "out of range vrr_idx(%d)\n", vrr_idx);
		return 0;
	}
...
static int panel_update_display_mode_props(struct panel_device *panel, int panel_mode)
{
	...
	mres_mode = panel_display_mode_get_mres_mode(panel, panel_mode);
	if (mres_mode < 0) {
		panel_drv_err(panel, "failed to get mres_mode from panel_mode(%d)\n", panel_mode);
		return -EINVAL;
	}

	if (mres_mode >= mres->nr_resol) {
		panel_drv_err(panel, "out of range mres_mode(%d)\n", mres_mode);
		return -EINVAL;
	}

	resol = &mres->resol[mres_mode];
	if (props->mres_mode == mres_mode) {
		panel_drv_dbg(panel, "same resolution(%d:%dx%d)\n",
	...
	vrr_idx = panel_display_mode_get_vrr_idx(panel, panel_mode);
	if (vrr_idx < 0) {
		panel_drv_err(panel, "failed to get vrr_idx from panel_mode(%d)\n",
				panel_mode);
		return -EINVAL;
	}
...
static int panel_seq_display_mode(struct panel_device *panel)
{
	...
	panel_set_property(panel, &panel->panel_data.props.display_mode_slot,
			(!panel->panel_data.props.display_mode_slot)&1);

	ret = panel_do_seqtbl_by_name_nolock(panel,
			PANEL_DISPLAY_MODE_SEQ);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to do display-mode-seq\n");
		return ret;
	}
...
int panel_set_display_mode_nolock(struct panel_device *panel, int panel_mode)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to update display mode properties\n");
		return ret;
	}

	ret = panel_seq_display_mode(panel);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to panel_seq_display_mode\n");
		return ret;
	}
...
static int panel_set_display_mode(struct panel_device *panel, void *arg)
{
	...
	if (unlikely(!pdm)) {
		panel_drv_err(panel, "panel_display_mode is null\n");
		return -EINVAL;
	}
	...
	if (props->panel_mode == panel_mode) {
		panel_drv_dbg(panel, "same panel_mode(%d)\n", panel_mode);
		goto out;
	}
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to panel_set_display_mode_nolock\n");
...
static int panel_seq_display_mode_post(struct panel_device *panel)
{
	...
	if (!check_seqtbl_exist(panel, PANEL_DISPLAY_MODE_POST_SEQ)) {
		panel_drv_dbg(panel, "display-mode-post-seq is not exist\n");
		return -ENOENT;
	}
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to do display-mode-post-seq\n");
		panel_mutex_unlock(&panel->op_lock);
		return ret;
	}
	panel_mutex_unlock(&panel->op_lock);
...
static int panel_set_display_mode_post(struct panel_device *panel, void *arg)
{
	...
	if (unlikely(ret < 0 && ret != -ENOENT)) {
		panel_drv_err(panel, "failed to panel_seq_display_mode_post\n");
		return ret;
	}
...
static int panel_set_ffc_seq(struct panel_device *panel, u32 dsi_freq)
{
	...
	panel_drv_info(panel, "panel update ffc frequency %d -> %dkhz\n", origin, dsi_freq);
...
static int panel_set_osc_seq(struct panel_device *panel, u32 osc_freq)
{
	...
	panel_drv_info(panel, "panel update osc frequency %d -> %d KHz\n", props->osc_freq, osc_freq);
...
static int panel_request_set_clock(struct panel_device *panel, void *arg)
{
	...
	case CLOCK_ID_OSC:
		...
		break;
	default:
		panel_drv_err(panel, "Invalid clock id: %d\n", info->clock_id);
		ret = -EINVAL;
	}
...
int panel_ioctl_attach_adapter(struct panel_device *panel, void *arg)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse ap vendor node\n");
		return ret;
	}
...
int panel_set_adapter_fifo_size(struct panel_device *panel, unsigned int fifo_size)
{
	...
	panel_drv_info(panel, "adapter fifo_size = (%d)\n", panel->adapter.fifo_size);
...
int panel_ioctl_get_panel_state(struct panel_device *panel, void *arg)
{
	...
	int *connect = arg;
	panel->state.connected = panel_conn_det_state(panel);
	panel_drv_info(panel, "connected : %d\n", panel->state.connected);
...
static int panel_error_cb(struct panel_device *panel)
{
	...
	if (error_cb_info->error_cb) {
		ret = error_cb_info->error_cb((void *)(error_cb_info->data),
				&panel_check_info);
		if (ret)
			panel_drv_err(panel, "failed to recovery panel\n");
	}
...
static int panel_powerdown_cb(struct panel_device *panel)
{
	...
	if (error_cb_info->powerdown_cb) {
		ret = error_cb_info->powerdown_cb((void *)(error_cb_info->data),
				&panel_check_info);
		if (ret)
			panel_drv_err(panel, "failed to powerdown panel\n");
	}
...
static int panel_set_mask_layer(struct panel_device *panel, void *arg)
{
	...
	if (!panel->lcd_dev) {
		panel_drv_err(panel, "lcd device not exist\n");
		return -EINVAL;
	}
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_ALPM) {
		tx_cmd = false;
		panel_drv_info(panel, "tx cmd skip!(ALPM)\n");
	}
	...
	case MASK_LAYER_ON_BEFORE:
		...
		panel_drv_info(panel, "enter (%d)->(%d)%s\n",
	...
	case MASK_LAYER_OFF_BEFORE:
		...
		panel_drv_info(panel, "exit (%d)->(%d)%s\n",
	...
	default:
		panel_drv_err(panel, "Invalid req: %d\n", req_data->req);
	}
...
static int panel_seq_first_frame(struct panel_device *panel)
{
	int ret;
	...
	if (!check_seqtbl_exist(panel, PANEL_FIRST_FRAME_SEQ)) {
		panel_drv_dbg(panel, "first-frame-seq is not exist\n");
		return -ENOENT;
	}

	panel_mutex_lock(&panel->op_lock);
	ret = panel_do_seqtbl_by_name_nolock(panel, PANEL_FIRST_FRAME_SEQ);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to do first-frame-seq %d\n", ret);
		panel_mutex_unlock(&panel->op_lock);
		return ret;
	}
	panel_mutex_unlock(&panel->op_lock);
...
__visible_for_testing int panel_first_frame(struct panel_device *panel)
{
	...
	panel_drv_info(panel, "updated: %llu\n", ktime_to_ms(panel->ktime_first_frame));

	panel_seq_first_frame(panel);
...
int panel_ioctl_event_frame_done(struct panel_device *panel, void *arg)
{
	...
	if (panel->state.fsync_event_on == PANEL_FSYNC_EVENT_OFF) {
		panel_drv_warn(panel, "FRAME_DONE (fsync event off)\n");
		return 0;
	}

	if (panel_get_cur_state(panel) != PANEL_STATE_NORMAL &&
			panel_get_cur_state(panel) != PANEL_STATE_ALPM) {
		panel_drv_warn(panel, "FRAME_DONE (panel_state:%s, disp_on:%s)\n",
				panel_state_names[panel_get_cur_state(panel)],
	...
	if (unlikely(panel->state.disp_on == PANEL_DISPLAY_OFF)) {
		panel_drv_info(panel, "FRAME_DONE (panel_state:%s, display on)\n",
				panel_state_names[panel_get_cur_state(panel)]);
		ret = _panel_display_on(panel);
...
int panel_drv_set_gpios(struct panel_device *panel)
{
	...
	if (panel_is_gpio_valid(panel, PANEL_GPIO_RESET)) {
		...
		panel_drv_info(panel, "Fixed reg(%s) is exist.\n", PANEL_REGULATOR_NAME_RESET);
	} else {
		panel_drv_err(panel, "gpio(%s) not exist\n",
	...
	panel_send_ubconn_notify(panel, panel->state.connected == PANEL_STATE_OK ?
		PANEL_EVENT_UB_CON_STATE_CONNECTED : PANEL_EVENT_UB_CON_STATE_DISCONNECTED);
#endif
	panel_drv_info(panel, "rst:%d, disp_det:%d (init_at:%s, ub_con:%d(%s) panel:(%s))\n",
...
int panel_drv_set_regulators(struct panel_device *panel)
{
	...
	if (panel->state.init_at == PANEL_INIT_BOOT) {
		...
		if (ret < 0 && ret != -ENODATA)
			panel_drv_err(panel, "failed to execute panel_boot_on\n");

		if (panel_bypass_is_on(panel)) {
			ret = panel_drv_power_ctrl_execute(panel, "panel_power_off");
			if (ret < 0 && ret != -ENODATA)
				panel_drv_err(panel, "failed to execute panel_power_off\n");
		}
	}
	return ret;
}
...
static int panel_parse_pinctrl(struct panel_device *panel)
{
	...
	if (IS_ERR(panel->pinctrl)) {
		panel_drv_err(panel, "failed to get device's pinctrl\n");
	...
	if (IS_ERR(panel->default_gpio_pinctrl)) {
		panel_drv_err(panel, "can't get default pinctrl setting\n");
		goto exit_parse_pinctrl;
	}

	if (pinctrl_select_state(panel->pinctrl, panel->default_gpio_pinctrl)) {
		panel_drv_err(panel, "failed to set default pinctrl\n");
		goto exit_parse_pinctrl;
	}

	panel_drv_info(panel, "pinctrl setting done\n");
...
struct panel_gpio *panel_gpio_list_add(struct panel_device *panel, struct panel_gpio *_gpio)
{
	...
	if (!_gpio) {
		panel_drv_err(panel, "invalid args\n");
		return NULL;
	}
...
static int panel_parse_gpio(struct panel_device *panel)
{
	...
	if (!gpios_np) {
		panel_drv_err(panel, "'gpios' node not found\n");
		return -EINVAL;
	}

	for_each_child_of_node(gpios_np, np) {
		memset(&gpio, 0, sizeof(gpio));
		if (of_get_panel_gpio(panel, np, &gpio)) {
			panel_drv_err(panel, "failed to get gpio %s\n", np->name);
			break;
		}

		p_gpio = panel_gpio_list_add(panel, &gpio);
		if (!p_gpio) {
			panel_drv_err(panel, "failed to add gpio list %s\n", gpio.name);
			break;
		}
	}
...
static int panel_parse_regulator(struct panel_device *panel)
{
	...
	if (!regulators_np) {
		panel_drv_err(panel, "'regulators' node not found\n");
		return -EINVAL;
	}

	for_each_child_of_node(regulators_np, np) {
		memset(&regulator, 0, sizeof(regulator));
		if (of_get_panel_regulator(panel, np, &regulator) < 0) {
			panel_drv_err(panel, "failed to get regulator %s\n", np->name);
			of_node_put(regulators_np);
			return -EINVAL;
		}

		if (IS_ERR_OR_NULL(panel_regulator_list_add(panel, &regulator))) {
			panel_drv_err(panel, "failed to add regulator list %s\n", regulator.name);
			break;
		}

		panel_drv_info(panel, "found regulator name:%s type:%d\n", regulator.name, regulator.type);
	}
	of_node_put(regulators_np);

	panel_drv_dbg(panel, "done\n");
...
static int panel_parse_power_ctrl(struct panel_device *panel)
{
	...
	if (!power_np) {
		panel_drv_err(panel, "'power-ctrl' node not found\n");
		ret = -EINVAL;
		goto exit;
	}
	...
	if (!seq_np) {
		panel_drv_err(panel, "'sequences' node not found\n");
		ret = -EINVAL;
		goto exit_power;
	}
	...
	for_each_property_of_node(seq_np, pp) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to get power_ctrl %s\n", pp->name);
			break;
		}
		list_add_tail(&p_seq->head, &panel->power_ctrl_list);
		panel_drv_info(panel, "power_ctrl '%s' initialized\n", p_seq->name);
	}
	of_node_put(seq_np);
exit_power:
		...
		enum panel_gpio_lists panel_gpio_id)
{
	...
	if (!work) {
		panel_drv_err(panel, "failed to find work(%s)\n", gpio->name);
		return -EINVAL;
	}

	return panel_gpio_helper_devm_request_irq(panel, gpio,
...
int panel_register_isr(struct panel_device *panel)
{
	...
	if (panel->dev == NULL) {
		panel_drv_err(panel, "panel->dev is NULL\n");
		return -ENODEV;
	}
	...
	for (i = 0; i < PANEL_GPIO_MAX; i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to register irq(%s)\n",
					panel_gpio_names[i]);
			break;
		}
	}
...
static int panel_parse_panel_lookup(struct panel_device *panel)
{
	...
	if (unlikely(!lookup_np)) {
		panel_drv_warn(panel, "No DT node for panel-lut\n");
		return -EINVAL;
	}
	...
	for_each_property_of_node(lookup_np, pp) {
		...
		panel_np = of_parse_phandle(lookup_np, pp->name, 0);
		if (!panel_np) {
			panel_drv_err(panel, "failed to get phandle of panel %s\n", pp->name);
			return -EINVAL;
		}
		...
		if (!node) {
			panel_drv_err(panel, "failed to get phandle of ap-vendor-setting\n");
			return -EINVAL;
		}
		...
		if (!node) {
			panel_drv_err(panel, "failed to get phandle of display-mode\n");
			return -EINVAL;
		}
		...
		if (!node) {
			panel_drv_err(panel, "failed to get phandle of power-ctrl\n");
			return -EINVAL;
		}
		...
		if (ret != 0 || lut->dqe_suffix == NULL) {
			panel_drv_warn(panel, "dqe-suffix is empty\n");
		} else {
			panel_drv_info(panel, "found dqe-suffix:%s\n", lut->dqe_suffix);
		}

		ret = of_property_read_string(panel_np, "aiqe-suffix", &lut->aiqe_suffix);
		if (ret != 0 || lut->aiqe_suffix == NULL) {
			panel_drv_warn(panel, "aiqe-suffix is empty\n");
		} else {
			panel_drv_info(panel, "found aiqe-suffix:%s\n", lut->aiqe_suffix);
		}

		node = of_parse_phandle(panel_np, "options", 0);
		if (node) {
			panel_drv_info(panel, "found options node in DT\n");
		...
		if (node) {
			...
			panel_drv_err(panel, "failed to get phandle of %s\n", DT_NAME_FREQ_TABLE);
#endif

#if defined(CONFIG_USDM_SDP_ADAPTIVE_MIPI) ||\
	defined(CONFIG_USDM_ADAPTIVE_MIPI)
		panel_drv_info(panel, "found adaptive mipi dt file\n");
		...
		if (sz <= 0) {
			panel_drv_err(panel, "failed to get count of id-mask property\n");
			return -EINVAL;
		}

		if (sz % 2 > 0) {
			panel_drv_err(panel, "id-mask value must be pair (size:%d)\n", sz);
			return -EINVAL;
		}

		if (sz > ARRAY_SIZE(tmparr)) {
			panel_drv_warn(panel, "id mask size(%d) exceeded %ld\n", sz, ARRAY_SIZE(tmparr));
			sz = ARRAY_SIZE(tmparr);
		}
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to get id-mask values\n");
			return -EINVAL;
		}
		...
		panel_drv_info(panel, "%s node:\"%s\" name:\"%s\"\n", pp->name, panel_np->name, lut->name);
		print_panel_lut(lut);
...
int panel_parse_dt(struct panel_device *panel)
{
	...
	if (IS_ERR_OR_NULL(dev->of_node)) {
		panel_drv_err(panel, "failed to get dt info\n");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse pinctrl\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse gpio\n");
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse panel lookup\n");
		return ret;
	}

	ret = panel_set_node_from_panel_lut(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to set nodes from panel_lut\n");
		return ret;
	}
...
static void panel_power_off_bypass_on(struct panel_device *panel)
{
	...
	panel_mutex_lock(&panel->state_lock);
	if (panel_get_cur_state(panel) == PANEL_STATE_OFF) {
		panel_drv_err(panel, "panel is already off state\n");
		panel_mutex_unlock(&panel->state_lock);
		return;
	}
...
static void disp_det_handler(struct work_struct *work)
{
	...
	panel_drv_err(panel, "disp_det is abnormal state\n");
	...
	panel_drv_info(panel, "disp_det_state: %s, con_det_state: %s, reset: %d\n",
	...
	con_det_state = panel_conn_det_state(panel);
	panel_drv_info(panel, "con_det_state(2nd): %s\n", con_det_state == PANEL_STATE_OK ? "OK" : "NOK");
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_OFF) {
		panel_drv_err(panel, "panel is off state\n");
		return;
	}

#ifdef CONFIG_USDM_PANEL_UEVENT_RECOVERY
	panel_send_esd_recovery_notify(panel, 0);
 	ret = panel_trigger_recovery(panel);
	if (ret == -EBUSY) {
		panel_drv_info(panel, "recovery already running, skip\n");
	} else if (ret) {
		panel_drv_err(panel, "recovery failed: %d\n", ret);
...
void conn_det_handler(struct work_struct *data)
{
	...
	panel_drv_info(panel, "state:%d cnt:%d\n",
...
void err_fg_handler(struct work_struct *data)
{
	...
	panel_drv_info(panel, "err_fg_state:%s recover:%s powerdown: %s\n",
	...
	case PANEL_STATE_NORMAL:
		...
		usleep_range(10000, 11000);
		if (err_fg_powerdown) {
			panel_drv_err(panel, "powerdown: err_fg is abnormal state\n");
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
			panel_send_ubconn_notify(panel, PANEL_EVENT_UB_CON_STATE_DISCONNECTED);
#endif
			panel_power_off_bypass_on(panel);
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
			panel_send_ubconn_notify(panel, PANEL_EVENT_UB_CON_STATE_CONNECTED);
#endif
		} else if (err_fg_recovery) {
			panel_drv_err(panel, "recovery: err_fg is abnormal state\n");
#ifdef CONFIG_USDM_PANEL_UEVENT_RECOVERY
			panel_send_esd_recovery_notify(panel, 0);
			ret = panel_trigger_recovery(panel);
			if (ret == -EBUSY) {
...
static int panel_fb_notifier(struct notifier_block *self, unsigned long event, void *data)
{
	...
	case FB_EVENT_FB_REGISTERED:
		panel_drv_dbg(panel, "FB Registeted\n");
		return 0;
	default:
		return 0;
	}
	...
	blank = fb_event->data;
	if (!blank || !panel) {
		panel_drv_err(panel, "blank is null\n");
		return 0;
	}
	...
	case FB_BLANK_NORMAL:
		if (event == FB_EARLY_EVENT_BLANK)
			panel_drv_dbg(panel, "EARLY BLANK POWER DOWN\n");
		else
			panel_drv_dbg(panel, "BLANK POWER DOWN\n");
		break;
	case FB_BLANK_UNBLANK:
		if (event == FB_EARLY_EVENT_BLANK)
			panel_drv_dbg(panel, "EARLY UNBLANK\n");
		else
			panel_drv_dbg(panel, "UNBLANK\n");
		break;
	}
#endif
	return 0;
...
int panel_fast_discharge_set(struct panel_device *panel)
{
	...
	if (unlikely(ret < 0))
		panel_drv_err(panel, "failed to write fast discharge seqtbl\n");
		...
		struct panel_work *w, char *name, panel_wq_handler handler)
{
	if (w == NULL || name == NULL || handler == NULL) {
		panel_drv_err(panel, "invalid parameter\n");
		return -EINVAL;
	}
	...
	if (w->wq == NULL) {
		panel_drv_err(panel, "failed to create %s workqueue\n", name);
		return -ENOMEM;
	}
	atomic_set(&w->running, 0);
	panel_drv_info(panel, "%s:done\n", name);
...
static int panel_drv_init_work(struct panel_device *panel)
{
	...
	for (i = 0; i < PANEL_WORK_MAX; i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to initialize panel_work(%s)\n",
					panel_work_names[i]);
			return ret;
		}
	}
...
static int panel_create_thread(struct panel_device *panel)
{
	...
	if (unlikely(!panel)) {
		panel_drv_warn(panel, "panel is null\n");
		return 0;
	}
	...
	for (i = 0; i < ARRAY_SIZE(panel->thread); i++) {
		...
		panel_drv_info(panel, "%s:init\n", panel_thread_names[i]);
		...
		if (IS_ERR_OR_NULL(panel->thread[i].thread)) {
			panel_drv_err(panel, "failed to run %s-%d thread\n",
...
int panel_device_register_notifiers(struct panel_device *panel)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to register fb notifier callback\n");
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to register dpui notifier callback\n");
		return ret;
	}
#endif
...
int panel_device_init(struct panel_device *panel)
{
	...
	panel_mutex_init(panel, &panel->state_lock);
	...
	panel_drv_info(panel, "panel-id: %d\n", panel->id);

	ret = panel_add_property_from_array(panel,
			panel_drv_property_array,
			ARRAY_SIZE(panel_drv_property_array));
	if (ret < 0) {
		panel_drv_err(panel, "failed to add prop array\n");
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse i2c\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse blic\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to parse dt\n");
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to set gpios %d\n", ret);
		return ret;
	}

	ret = panel_drv_init_work(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to init work\n");
		return ret;
	}

	ret = panel_create_thread(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to create thread\n");
		return ret;
	}
...
/* TODO: remove debugfs */
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to init panel_bl\n");
		return -ENODEV;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to init mdnie\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to register notifiers\n");
		return ret;
	}

	ret = panel_register_isr(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to register isr\n");
		return ret;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to init sysfs\n");
		return -ENODEV;
	}
#endif
...
int panel_device_exit(struct panel_device *panel)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove panel-sysfs driver\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to exit mdnie driver\n");
		return ret;
	}
#endif
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to destroy lcd device\n");
		return ret;
	}

	ret = panel_remove(panel);
	if (ret < 0) {
		panel_drv_err(panel, "failed to remove panel\n");
		return ret;
	}
...
static int panel_drv_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to initialize panel device\n");
		goto err;
	}
	panel_drv_info(panel, "done\n");
