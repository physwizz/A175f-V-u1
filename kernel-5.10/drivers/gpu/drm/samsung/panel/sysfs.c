	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret != readlen)) {
		panel_drv_err(panel, "failed to read reg %02Xh pos %d len %d\n",
				readreg, readpos, readlen);
		ret = -EIO;
		goto store_err;
	}

	panel_drv_info(panel, "READ_Reg addr: %02x, pos : %d len : %d\n",
			readreg, readpos, readlen);
	for (i = 0; i < readlen; i++)
		panel_drv_info(panel, "READ_Reg %dth : %02x\n", i + 1, readbuf[i]);
	mutex_unlock(&sysfs_lock);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	while ((p = strsep(&arg, " \t\n=")) != NULL) {
		...
		if (ret != 1) {
			panel_drv_err(panel, "failed to scan payload %d\n", tx_len);
			ret = -EINVAL;
	...
	panel_mutex_unlock(&panel->op_lock);
	if (ret < 0) {
		panel_drv_err(panel, "failed to execute write_mtp_seq(ret:%d)\n", ret);
		goto err_write_mtp_store;
	}
	panel_drv_info(panel, "%d byte(s) sent.\n", tx_len);
	mutex_unlock(&sysfs_lock);
...
err_write_mtp_store:
	...
	mutex_unlock(&sysfs_lock);
	panel_drv_info(panel, "failed to write_mtp\n");
...
static int mafpc_get_target_crc(struct panel_device *panel, u8 *crc)
{
	...
	if (!mafpc) {
		panel_drv_err(panel, "failed to get mafpc info\n");
		return -EINVAL;
	}

	if (mafpc->comp_crc_len < MAFPC_CRC_LEN) {
		panel_drv_err(panel, "crc len must be %d\n", MAFPC_CRC_LEN);
		return -EINVAL;
	}
...
static void clear_mafpc_check_mode(struct panel_device *panel)
{
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to panel_enable_irq\n");
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!IS_PANEL_ACTIVE(panel)) {
		panel_drv_err(panel, "panel is not active\n");
		goto exit;
	}

	if (panel_get_cur_state(panel) == PANEL_STATE_ALPM) {
		panel_drv_err(panel, "gct not supported on LPM\n");
		goto exit;
	}
...
#endif
	panel_mutex_lock(&panel->op_lock);
	if (!(panel_data->ddi_props.no_prepare_mafpc_crc))
		prepare_mafpc_check_mode(panel);

	ret = panel_do_seqtbl_by_name_nolock(panel, PANEL_MAFPC_CHECKSUM_SEQ);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write panel_mafpc_crc seq\n");
		goto out;
	}

	ret = panel_resource_copy_and_clear(panel, target_crc, "mafpc_crc");
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to read mafpc crc\n");
		goto out;
	}

	ret  = mafpc_get_target_crc(panel, origin_crc);
	if (ret)
		panel_drv_err(panel, "failed to get target mAFPC crc value\n");

	panel_drv_info(panel, "target crc : %x :%x\n", target_crc[0], target_crc[1]);
	panel_drv_info(panel, "origin crc : %x :%x\n", origin_crc[0], origin_crc[1]);
	...
	struct device_attribute *attr, char *buf)
{
	...
	snprintf(buf, PAGE_SIZE, "%02x %02x %02x\n",
			panel_data->id[0], panel_data->id[1], panel_data->id[2]);
	panel_drv_info(panel, "%02x %02x %02x\n", panel_data->id[0], panel_data->id[1], panel_data->id[2]);
	return strlen(buf);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (br = 0; br <= max_brightness; br++) {
		...
		prev_br = br;
		if (len >= PAGE_SIZE) {
			panel_drv_info(panel, "print buffer overflow %d\n", len);
			len = PAGE_SIZE - 1;
			goto exit;
		}
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value > 100) {
		panel_drv_err(panel, "invalid adaptive_control %d\n", value);
		return -EINVAL;
	}
	...
	panel_drv_info(panel, "adaptive_control %d\n", panel_data->props.adaptive_control);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "siop_enable %d\n",
			panel_data->props.siop_enable);
	return size;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "temperature %d\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write mcd seq\n");
		panel_mutex_unlock(&panel->io_lock);
		return ret;
	}
	panel_drv_info(panel, "mcd %s (%d %s mode)\n",
...
static void print_mcd_resistance(struct panel_device *panel, u8 *mcd_nok, int size)
{
	int code, len;
	char buf[1024];
...
static int read_mcd_resistance(struct panel_device *panel)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write mcd_3_0_on seq\n");
		goto out;
	}
	...
	for (code = 0; code < 0x80; code++) {
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to write mcd_rs_read seq\n");
			goto out;
		}
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to copy resource(mcd_resistance) (ret %d)\n", ret);
			goto out;
		}
		panel_drv_dbg(panel, "%02X : %02X\n", code, mcd_nok[code]);
	}
	print_mcd_resistance(panel, mcd_nok, ARRAY_SIZE(mcd_nok));
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write mcd_3_0_off seq\n");
		goto out;
	}
...
out:
	...
	panel_drv_info(panel, "done (elapsed %2lld.%03lld msec)\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (!!value) {
		...
		panel_mutex_unlock(&panel->op_lock);
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to check mcd resistance\n");
			return ret;
		}

		for (i = 0; i < MAX_MCD_RS; i++)
			panel_drv_info(panel, "%s:(%d, %d)\n", mcd_rs_name[i],
		...
		if (unlikely(ret)) {
			panel_drv_err(panel, "failed to read mcd(ret %d)\n", ret);
			return ret;
		}
		ret = panel_resource_update_by_name(panel, "flash_mcd");
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to update flash_mcd res (ret %d)\n", ret);
			return ret;
		}

		ret = panel_resource_copy(panel, flash_mcd, "flash_mcd");
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to copy flash_mcd res (ret %d)\n", ret);
			return ret;
		}
		...
		for (i = 0; i < MAX_MCD_RS; i++)
			panel_drv_info(panel, "SDC_%s:(%d, %d)\n", mcd_rs_name[i],
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_send_screen_mode_notify(panel, panel_data->props.irc_mode);
#endif
	panel_drv_info(panel, "irc_mode %s\n", panel_data->props.irc_mode ? "on" : "off");
	panel_drv_dbg(panel, " --\n");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write %s\n", PANEL_DIA_ONOFF_SEQ);
		return ret;
	}
	panel_drv_info(panel, "set %s\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (updated) {
		...
		panel_drv_info(panel, "already set %s, so skip\n",
				panel_data->props.panel_partial_disp ? "on" : "off");
	}
...
ssize_t mst_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write mst seq\n");
		return ret;
	}
	panel_drv_info(panel, "mst %s\n", panel_data->props.mst_on ? "on" : "off");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (!check_seqtbl_exist(panel, PANEL_GCT_ENTER_SEQ)) {
		panel_drv_warn(panel, "cannot found gct seq. skip\n");
		panel_data->props.gct_on = GRAM_TEST_SKIPPED;
		return -EINVAL;
	}
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_ALPM) {
		panel_drv_warn(panel, "gct not supported on LPM\n");
		panel_mutex_unlock(&panel->io_lock);
		return -EINVAL;
	}
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write gram-checksum-test-enter seq\n");
		result = ret;
		goto out;
	}
	...
	for (vddm = VDDM_LV; vddm < MAX_VDDM; vddm++) {
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to write gram-checksum-on seq\n");
			result = ret;
			goto out;
		}
		...
		for (pattern = GCT_PATTERN_1; pattern < MAX_GCT_PATTERN; pattern++) {
			...
			if (unlikely(ret < 0)) {
				panel_drv_err(panel, "failed to write gram-img-update seq\n");
				result = ret;
				goto out;
			}
			...
			if (unlikely(ret < 0)) {
				panel_drv_err(panel, "failed to copy gram_checksum[%d] (ret %d)\n", index, ret);
				result = ret;
				goto out;
			}
			panel_drv_info(panel, "gram_checksum[%d] 0x%x\n", index, checksum[index]);
			index++;
		}
	}
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write gram-checksum-off seq\n");
		result = ret;
	}
out:
	...
	if (panel_data->ddi_props.support_avoid_sandstorm) {
		panel_drv_info(panel, "display on\n");
		ret = panel_display_on(panel);
		if (ret < 0)
			panel_drv_err(panel, "failed to display on\n");
	} else {
		panel_drv_info(panel, "wait display on\n");
		for (i = 0; i < 20; i++) {
	...
	panel_drv_info(panel, "-- chksum %s 0x%08x\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (!check_seqtbl_exist(panel, PANEL_DECODER_TEST_SEQ)) {
		panel_drv_warn(panel, "sequence(%s) not found\n", PANEL_DECODER_TEST_SEQ);
	...
	if (panel_get_cur_state(panel) == PANEL_STATE_ALPM) {
		panel_drv_warn(panel, "dsc_crc not supported on LPM\n");
		panel_mutex_unlock(&panel->io_lock);
		ret = -EINVAL;
		goto exit;
	}
	...
	panel_drv_info(panel, "-- chksum %s %s\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "xtalk_mode %d\n",
			panel_data->props.xtalk_mode);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to chkpoc (ret %d)\n", ret);
		return ret;
	}

	ret = set_panel_poc(poc_dev, POC_OP_CHECKSUM, NULL);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to chksum (ret %d)\n", ret);
		return ret;
	}
	...
	panel_drv_info(panel, "poc:%d chk:%d gray:%02x\n", poc_info->poc,
			poc_info->poc_chksum[4], poc_info->poc_ctrl[3]);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (rc < 1) {
		panel_drv_err(panel, "poc_op required\n");
		return -EINVAL;
	}

	if (!IS_VALID_POC_OP(value)) {
		panel_drv_warn(panel, "invalid poc_op %d\n", value);
		return -EINVAL;
	}

	if (value == POC_OP_WRITE || value == POC_OP_READ) {
		panel_drv_warn(panel, "unsupported poc_op %d\n", value);
		return size;
	}
	...
	if (value == POC_OP_SET_SPI_SPEED) {
		...
		if (rc < 1) {
			panel_drv_warn(panel, "SET_SPI_SPEED need 2 params\n");
			return -EINVAL;
		}
	...
	if (value == POC_OP_CANCEL) {
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to poc_op %d(ret %d)\n", value, ret);
			panel_mutex_unlock(&panel->io_lock);
			return -EINVAL;
		}
		panel_mutex_unlock(&panel->io_lock);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to update poc_mca_chksum res (ret %d)\n", ret);
		return ret;
	}
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to copy poc_mca_chksum res (ret %d)\n", ret);
		return ret;
	}
	...
	panel_drv_info(panel, "poc_mca_checksum: %s", buf);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to get poc partition size (ret %d)\n", ret);
		return ret;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write grayspot on/off seq\n");
		return ret;
	}
	panel_drv_info(panel, "grayspot %s\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "brightness : %d\n", value);
	...
	if (panel->state.hmd_on != PANEL_HMD_ON) {
		panel_drv_info(panel, "hmd off\n");
		goto exit_store;
	}

	ret = panel_bl_set_brightness(panel_bl, PANEL_BL_SUBDEV_TYPE_HMD, SEND_CMD);
	if (ret) {
		panel_drv_err(panel, "fail to set brightness\n");
		goto exit_store;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value != PANEL_HMD_OFF && value != PANEL_HMD_ON) {
		panel_drv_err(panel, "invalid parameter %d\n", value);
		return -EINVAL;
	}

	panel_drv_info(panel, "hmd set to: %s\n", (value == PANEL_HMD_ON) ? "on" : "off");
	...
	if (value == state->hmd_on) {
		panel_drv_warn(panel, "already set : %d\n", value);
		ret = -EBUSY;
		goto exit;
	}
	...
	if (vrr != NULL && value == PANEL_HMD_ON) {
		if (vrr->fps != 60 || vrr->mode != 0) {
			panel_drv_err(panel, "failed to set hmd %s: fps %d mode %d\n",
					(value == PANEL_HMD_ON) ? "on" : "off",
					vrr->fps, vrr->mode);
			ret = -EINVAL;
			goto exit;
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to set hmd %s seq\n",
				(value == PANEL_HMD_ON) ? "on" : "off");
	...
	if (ret < 0)
		panel_drv_err(panel, "failed to set %s brightness\n",
				(value == PANEL_HMD_ON) ? "hmd" : "normal");

	panel_drv_info(panel, "hmd_on %d -> %d\n", state->hmd_on, value);
	state->hmd_on = value;
exit:
	panel_mutex_unlock(&panel->op_lock);
	panel_mutex_unlock(&panel_bl->lock);
...
static int set_alpm_mode(struct panel_device *panel, int mode)
{
	...
	case HLPM_HIGH_BR:
		panel_data->props.alpm_mode = lpm_mode;
		if (panel_get_cur_state(panel) != PANEL_STATE_ALPM) {
			panel_drv_info(panel, "panel state(%d) is not lpm mode\n",
		...
		break;
	default:
		panel_drv_err(panel, "invalid alpm_mode: %d\n", lpm_mode);
		break;
	}
...
static int set_alpm_mode_factory(struct panel_device *panel, int mode)
{
	...
	case ALPM_OFF:
		...
		if (ret)
			panel_drv_err(panel, "failed to write set_alpm\n");
	...
	case HLPM_HIGH_BR:
		...
		if (ret)
			panel_drv_err(panel, "failed to set_alpm\n");
		break;
	default:
		panel_drv_err(panel, "invalid alpm_mode: %d\n", lpm_mode);
		break;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	rc = kstrtoint(buf, 0, &value);
	if (rc < 0) {
		panel_drv_warn(panel, "invalid param (ret %d)\n", rc);
		panel_mutex_unlock(&panel->io_lock);
		return rc;
	}
	...
	if (rc)
		panel_drv_err(panel, "failed to set alpm (value %d, ret %d)\n", value, rc);
#endif

	panel_mutex_unlock(&panel->io_lock);
	panel_drv_info(panel, "value %d, alpm_mode %d\n", value, panel_data->props.alpm_mode);
	return size;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "value %d, lpm_opr %d\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	while ((p = strsep(&arg, " \t")) != NULL) {
		...
		if (index < 0) {
			panel_drv_err(panel, "arg(%s) not found\n", p);
			continue;
		}
		...
		if (rc != 1) {
			panel_drv_err(panel, "invalid argument name:%s ret:%d\n", name, rc);
			continue;
		}

		rc = copr_reg_store(copr, index, value);
		if (rc < 0) {
			panel_drv_err(panel, "failed to store to copr_reg (index %d, value %d)\n",
					index, value);
			continue;
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!copr_is_enabled(copr)) {
		panel_drv_err(panel, "copr is off state\n");
		return snprintf(buf, PAGE_SIZE, "-1\n");
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (copr->props.version == COPR_VER_1 ||
		...
		if (rc < 0) {
			panel_drv_err(panel, "invalid roi input(rc %d)\n", rc);
			return -EINVAL;
		}
		nr_roi = rc / 4;
		...
		panel_drv_err(panel, "roi is unsupported in copr ver%d\n", copr->props.version);
		return -EINVAL;
	}
	...
	if ((copr->props.version == COPR_VER_6) ||
		...
		for (i = 0; i < nr_roi; i++)
			panel_drv_info(panel, "set roi[%d] %d %d %d %d\n",
					i, roi[i].roi_xs, roi[i].roi_ys,
					roi[i].roi_xe, roi[i].roi_ye);

		for (i = 0; i < copr->props.nr_roi; i++)
			panel_drv_info(panel, "cur roi[%d] %d %d %d %d\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (copr->props.nr_roi == 0) {
		panel_drv_warn(panel, "copr roi disabled\n");
		return -ENODEV;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (brt_avg < 0) {
		panel_drv_err(panel, "failed to get average brt1\n");
		return snprintf(buf, PAGE_SIZE, "-1\n");
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to get log %d\n", ret);
		return ret;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((aod == NULL) || (props == NULL)) {
		panel_drv_err(panel, "aod is null\n");
		return -EINVAL;
	}
	...
	if (props->self_mask_en != value) {
		if (value == 0) {
			...
			if (unlikely(ret < 0))
				panel_drv_err(panel, "failed to disable self mask\n");
		} else {
			ret = panel_do_aod_seqtbl_by_name(aod, SELF_MASK_IMG_SEQ);
			if (unlikely(ret < 0))
				panel_drv_err(panel, "failed to write self mask image\n");

			ret = panel_do_aod_seqtbl_by_name(aod, SELF_MASK_ENA_SEQ);
			if (unlikely(ret < 0))
				panel_drv_err(panel, "failed to enable self mask\n");
		}
		props->self_mask_en = value;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!find_panel_dumpinfo(panel, "self_mask_crc")) {
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to send cmd selfmask crc\n");
			kfree(recv_crc);
			goto err_clear;
		}
		...
		if (unlikely(ret < 0)) {
			panel_drv_err(panel, "failed to get selfmask crc\n");
			kfree(recv_crc);
			goto err_clear;
		}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (pattern < NORMAL_SELF_MOVE_PATTERN_OFF ||
		...
		panel_drv_err(panel, "out of range(%d)\n", pattern);
		return -EINVAL;
	}

	panel_drv_info(panel, "pattern : %d\n", pattern);
	panel_mutex_lock(&panel->io_lock);
	props->self_move_pattern = pattern;
	ret = panel_self_move_pattern_update(panel);
	if (ret < 0)
		panel_drv_info(panel, "failed to set self move pattern\n");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value) {
		...
		if (unlikely(ret < 0))
			panel_drv_err(panel, "failed to write ics defect seq\n");
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret < 0))
		panel_drv_err(panel, "failed to write brightdot seq\n");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret < 0))
		panel_drv_err(panel, "failed to run %s\n", PANEL_VGLHIGHDOT_TEST_SEQ);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (check_seqtbl_exist(panel,
		...
		panel_drv_info(panel, "spi if on/off unsupported\n");
		return size;
	}
	...
	if (unlikely(ret < 0))
		panel_drv_err(panel, "failed to write spi-if-%s seq\n", value ? "on" : "off");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write trim\n");
		return ret;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to read err_flag\n");
		return ret;
	}
	...
	panel_drv_info(panel, "comp %s err_flag[0]: 0x%02x 0x%02x %d\n", (retVal == 1) ? "Pass" : "Fail", err_flag[0], err_flag[1], retVal);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (unlikely(info == NULL)) {
		panel_drv_err(panel, "failed to get ccd resource\n");
		return -EINVAL;
	}

	ccd_size = info->dlen;
	if (ccd_size < 1 || ccd_size > CCD_STATE_SIZE) {
		panel_drv_err(panel, "invalid ccd size %d %d\n", ccd_size, CCD_STATE_SIZE);
		return -EINVAL;
	}

	ret = panel_do_seqtbl_by_name(panel, PANEL_CCD_TEST_SEQ);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write ccd seq\n");
		return ret;
	}

	ret = panel_resource_copy_and_clear(panel, ccd_state, "ccd_state");
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to read ccd_state \n");
		return ret;
	}
	...
	for (ires = 0; ires < ARRAY_SIZE(ccd_resource_name); ires++) {
		...
		if (info) {
			panel_drv_info(panel, "find ccd compare resource.(%s)\n", (char *)ccd_resource_name[ires]);
			break;
		}
	}
	...
	if (ires != ARRAY_SIZE(ccd_resource_name)) {
		if (ires < CCD_CHKSUM_PASS_LIST) {
			if (info->dlen != ccd_size) {
				panel_drv_err(panel, "%s: size mismatch %d %d\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	vrr_fps = vrr->fps;
	vrr_mode = vrr->mode;
	panel_drv_dbg(panel, "display(%d%s) panel(%d%s)\n",
		    ...
		    struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel_mode < 0 ||
		...
		panel_drv_err(panel, "panel_mode(%d) exceeded num_modes(%d)\n",
				panel_mode, common_panel_modes->num_modes);
		panel_mutex_unlock(&panel->op_lock);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!panel_vrr_is_supported(panel)) {
		panel_drv_warn(panel, "vrr is not supported\n");
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	while ((p = strsep(&arg, " \t\n=")) != NULL) {
		...
		if (vrr_lfd_arglist[index].nargs > 0) {
			...
			if (len < 0) {
				panel_drv_err(panel, "failed to parse sysfs arg(%s)\n", arg);
				panel_mutex_unlock(&panel->panel_bl.lock);
				return -EINVAL;
			}
			arg += len;
		...
		if (argc == 0) {
			...
			if (!strcmp(vrr_lfd_arglist[index].name, VRR_LFD_ARG_KEY_CLIENT)) {
				...
				if (client_index < 0) {
					panel_drv_err(panel, "client(%s) not found\n", out.d[0].val_str);
					panel_mutex_unlock(&panel->panel_bl.lock);
					return -EINVAL;
				}
				argc++;
		...
		if (client_index == -1) {
			if (!strcmp("0", vrr_lfd_arglist[index].name)) {
				...
				panel_drv_err(panel, "undefined argument(%s)\n",
		...
		if (!strcmp(vrr_lfd_arglist[index].name, VRR_LFD_ARG_KEY_SCOPE)) {
			...
			while ((p = strsep(&arg1, " ,|")) != NULL) {
				...
				if (scope < 0) {
					panel_drv_err(panel, "scope(%s) not found\n", p);
					panel_mutex_unlock(&panel->panel_bl.lock);
					return -EINVAL;
				}
		...
		if (scope_mask == 0) {
			panel_drv_err(panel, "argument(scope=) not found\n");
			panel_mutex_unlock(&panel->panel_bl.lock);
			return -EINVAL;
		}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	while (cmd_scanned < size && spi_flash_writelen < SPI_BUF_LEN) {
		...
		panel_drv_dbg(panel, "readed %d %d ret %d target str %s\n", cmd_input, parse, ret, buf + cmd_scanned);
		if (parse <= 0 || ret <= 0)
			break;
	...
	panel_drv_info(panel, "send %d byte(s), receive %d byte(s)\n", spi_flash_writelen, spi_flash_readlen);
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to spi cmd 0x%0x, ret %d\n", spi_flash_writebuf[0], ret);
		ret = -EIO;
		goto store_err;
	}

	panel_drv_info(panel, "received %d byte(s)\n", ret);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel_data->props.enable_fd == value) {
		panel_drv_info(panel, "fast discharge is already %s\n",
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to write fast discharge set\n");
		panel_mutex_lock(&panel->op_lock);
	...
	panel_drv_info(panel, "fast discharge set to %s\n", panel_data->props.enable_fd ? "on" : "off");
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value > panel_bl->bd->props.max_brightness) {
		panel_drv_err(panel, "input:%d bd max:%d\n", value, panel_bl->bd->props.max_brightness);
		return -EINVAL;
	}

	panel_drv_info(panel, "%d->%d target br updated.\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value < 0 || value >= MAX_FP_GREEN_CIRCLE) {
		panel_drv_err(panel, "input is error(%d).\n", value);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value >= LOCAL_HBM_SYSFS_MAX || value < 0) {
		panel_drv_err(panel, "input is invalid %d\n", value);
		return -EINVAL;
	}
	...
	if ((last_local_hbm != req_local_hbm) && (last_circle != req_circle)) {
		panel_drv_err(panel, "SKIP, local HBM and Circle can not be changed sametime.\n");
		skip = true;
	}

	if (panel_get_cur_state(panel) != PANEL_STATE_NORMAL) {
		panel_drv_err(panel, "SKIP, screen state is not NORMAL (%s)\n",
			get_panel_state_names(panel_get_cur_state(panel)));
		skip = true;
	}

	if (skip) {
		panel_drv_err(panel, "SKIP, req_val:%d req_hbm:%d req_circle:%d \n",
				value, req_local_hbm, req_circle);
		panel_drv_err(panel, "SKIP, last_val:%d last_hbm:%d last_circle:%d\n",
	...
	panel_drv_info(panel, "val:%d->%d hbm:%d->%d circle:%d->%d ++\n",
			last_value, value,
			last_local_hbm, panel_bl->props.local_hbm,
			last_circle, panel_bl->props.local_hbm_circle);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "%d (sysfs:%d mask_layer:%d)\n", panel_bl->props.smooth_transition,
		panel_bl->props.smooth_transition_sysfs_req,
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "[TESTMODE] failed to command %s: %d\n", command, ret);
		return -EINVAL;
	}
	return size;
#else
	panel_drv_err(panel, "[TESTMODE] testmode is not supported.\n");
	return -ENODEV;
#endif
}
#endif
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel_testmode_is_on(panel)) {
		if (buf[0] != '!') {
			panel_drv_info(panel, "[TESTMODE] %s_store: testmode is running. ignore inputs.\n", attr->attr.name);
			return size;
		}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ret == -EINVAL) {
		panel_drv_err(panel, "gpio('disp-te') is not supported\n");
		return -EINVAL;
	}

	if (ret == -ETIMEDOUT)
		panel_drv_info(panel, "polling gpio('disp-te') timeout\n");

	return sprintf(buf, "%d\n", (ret == 0) ? 1 : 0);
}

static ssize_t pmic_check_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct panel_device *panel = dev_get_drvdata(dev);
	int level = 0, sub_level = 0;

	if (!panel)
		return -EINVAL;

	/* expected gpio level : high */
	level = panel_get_gpio_value(panel, PANEL_GPIO_PMIC_DET);
	sub_level = panel_get_gpio_value(panel, PANEL_GPIO_PMIC_SUB_DET);

	panel_drv_info(panel, "%d %d\n", level, sub_level);

	return sprintf(buf, "%d %d\n",
		(level == -EINVAL) ? -1 : level, (sub_level == -EINVAL) ? -1 : sub_level);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (ret == -ENOENT) {
		panel_drv_info(panel, "this panel doesn't supported. test pass.\n");
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to read firmware_ver_read_seq seq\n");
		return ret;
	}
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "fw_ver_major failed\n");
		return -ENODATA;
	}

	ret = panel_resource_copy(panel, &ver_minor, "fw_ver_minor");
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "fw_ver_minor copy failed\n");
		return -ENODATA;
	}

	panel_drv_info(panel, "major: %d, minor: %d\n", ver_major, ver_minor);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "panel_aging %s\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (rc < 2) {
		panel_drv_err(panel, "need 2 parameters: %d\n", rc);
		return -EINVAL;
	}

	if (dsi < 0 || osc < 0) {
		panel_drv_err(panel, "invalid input(%d,%d)\n", dsi, osc);
		return -EINVAL;
	}
	...
	panel_drv_info(panel, "dsi:%d osc:%d ret:%d\n", dsi, osc, ret);
...
struct panel_device_attr panel_attrs[] = {
	...
	__PANEL_ATTR_RO(pmic_check, 0440, PA_DEFAULT),
#if defined(CONFIG_USDM_PANEL_VCOM_TRIM_TEST)
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	panel_drv_info(panel, "enable all sysfs node.\n");
...
int panel_remove_svc_octa(struct panel_device *panel)
{
	...
	if (kn) {
		...
		if (!IS_ERR_OR_NULL(svc_kobj)) {
			...
			panel_drv_err(panel, "fail to find svc\n");
			ret = -ENODEV;
			goto exit;
		}
		panel_drv_info(panel, "succeed to remove svc/OCTA\n");
	}
...
int panel_create_svc_octa(struct panel_device *panel)
{
	...
	if (!svc_kobj) {
		panel_drv_err(panel, "fail to create svc\n");
		ret = -ENODEV;
		goto exit;
	}
	...
	if (!IS_ERR_OR_NULL(svc_kobj)) {
		...
		if (ret)
			panel_drv_err(panel, "failed to create svc/%s/\n", name);
		else
			panel_drv_info(panel, "succeeded to create svc/%s/\n", name);
	} else {
		panel_drv_err(panel, "failed to find svc kobject\n");
	}
...
int panel_remove_sysfs(struct panel_device *panel)
{
	...
	panel_drv_info(panel, "remove sysfs done\n");
...
int panel_create_sysfs(struct panel_device *panel)
{
	...
	for (i = 0; i < ARRAY_SIZE(panel_attrs); i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to add sysfs(%s) entries, %d\n",
