...
struct panel_dt_lut *find_panel_lut(struct panel_device *panel, u32 panel_id)
{
	...
	list_for_each_entry(lut_info, &panel->panel_lut_list, head) {
		if (IS_ERR_OR_NULL(lut_info)) {
			panel_drv_warn(panel, "Null lut_info in %s\n", panel->of_node_name);
			continue;
		}
		list_for_each_entry(id_mask, &lut_info->id_mask_list, head) {
			if (IS_ERR_OR_NULL(id_mask)) {
				panel_drv_warn(panel, "Null id_mask in lut name %s %s\n", panel->of_node_name, lut_info->name);
				continue;
			}
			if (id_mask->id == 0 && id_mask->mask == 0) {
				panel_drv_info(panel, "default %s (id:0x%08X lut_id:0x%08X lut_mask:0x%08X)\n",
			...
			if ((panel_id & id_mask->mask) == id_mask->id) {
				panel_drv_info(panel, "found %s (id:0x%08X lut_id:0x%08X lut_mask:0x%08X)\n",
					lut_info->name, panel_id, id_mask->id, id_mask->mask);
				return lut_info;
			}
			panel_drv_dbg(panel, "panel name %s (id:0x%08X lut_id:0x%08X lut_mask:0x%08X) - not same\n",
				lut_info->name, panel_id, id_mask->id, id_mask->mask);
		}
	}
	if (default_lut) {
		panel_drv_info(panel, "use default lut %s\n", default_lut->name);
		return default_lut;
	}
	panel_drv_err(panel, "panel not found!! (id 0x%08X)\n", panel_id);
...
struct common_panel_info *find_panel(struct panel_device *panel, u32 panel_id)
{
	...
	if (!lut_info->name) {
		panel_drv_err(panel, "invalid name\n");
		return NULL;
	}
...
struct seqinfo *find_panel_seq_by_name(struct panel_device *panel, char *seqname)
{
	...
	if (list_empty(&panel->seq_list)) {
		panel_drv_err(panel, "sequence is empty\n");
		return NULL;
	}

	if (!seqname) {
		panel_drv_err(panel, "sequence is null\n");
		return NULL;
	}
...
__visible_for_testing u32 get_frame_delay(struct panel_device *panel, struct delayinfo *delay)
{
	...
	if (panel->panel_data.props.vrr_origin_fps == 0 ||
		...
		panel_drv_err(panel, "invalid fps(N:%d, N+1:%d)\n",
		...
		ktime_t start, ktime_t now)
{
	...
	if (ktime_after(start, now)) {
		panel_drv_err(panel, "invalid start time(start > now)\n");
		return 0;
	}
...
static int panel_wait_nvsync(struct panel_device *panel, struct delayinfo *delay)
{
	...
	for (i = 0; i < delay->nvsync; i++) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "vsync timeout(elapsed:%dms, ret:%d)\n", timeout, ret);
			return -ETIMEDOUT;
		}
	}
	usec = ktime_to_us(ktime_sub(ktime_get(), s_time));
	panel_drv_dbg(panel, "elapsed:%2d.%03d ms in %d FPS\n",
			usec / 1000, usec % 1000,
			panel->panel_data.props.vrr_fps);
...
static int panel_do_delay(struct panel_device *panel, struct delayinfo *delay, ktime_t start)
{
	...
	if (delay->nvsync > 0) {
		...
		if (ret < 0)
			panel_drv_warn(panel, "failed to wait vsync\n");
	} else {
		...
		struct timer_delay_begin_info *begin_info, ktime_t s_time)
{
	...
	if (unlikely(!begin_info || !begin_info->delay)) {
		panel_drv_err(panel, "invalid parameter\n");
		return -EINVAL;
	}
...
static int panel_do_timer_delay(struct panel_device *panel, struct delayinfo *info, ktime_t dummy)
{
	...
	if (ktime_after(now, target)) {
		panel_drv_dbg(panel, "skip delay (elapsed %lld.%03lld ms >= requested %d.%03d ms)\n",
	...
	if (remained_usec > 0) {
		...
		panel_drv_info(panel, "sleep remining %lld.%03lld ms\n",
...
int REAL_ID(panel_cmdq_flush)(struct panel_device *panel)
{
	...
	if (unlikely(!panel->adapter.funcs))
		return -EINVAL;
	...
	if ((panel->adapter.funcs->write_table != NULL) &&
		...
		if (ret < 0)
			panel_drv_err(panel, "failed to panel_dsi_write_table %d\n", ret);
		...
		for (i = 0; i < panel_cmdq_get_size(panel); i++) {
			...
			if (ret != panel->cmdq.cmd[i].size) {
				panel_drv_err(panel, "failed to panel_dsi_write_data %d\n", ret);
				break;
			}
		}
	}
...
int panel_cmdq_push(struct panel_device *panel, u8 cmd_id, const u8 *buf, int size)
{
	...
	if (panel_cmdq_get_size(panel) + (buf[0] == 0xB0 ? 2 : 1) > MAX_PANEL_CMD_QUEUE) {
		...
		if (ret < 0) {
			panel_drv_err(panel, "failed to panel_cmdq_flush %d\n", ret);
			return ret;
		}
	}
	...
	panel_drv_dbg(panel, "cmdq[%d] id:%02X cmd:%02X size:%d\n",
		...
		u8 cmd_id, const u8 *buf, u32 ofs, int size, u32 option)
{
	...
	if (ofs > 0) {
		...
		ret = panel_cmdq_push(panel, MIPI_DSI_WR_GEN_CMD,
				gpara, gpara_len);
		if (ret < 0) {
			panel_drv_err(panel, "failed to panel_cmdq_push %d\n", ret);
			panel_mutex_unlock(&panel->cmdq.lock);
			return ret;
		}
	}
		...
		u8 cmd_id, const u8 *buf, u32 ofs, int size, u32 option)
{
	...
	cmdbuf = panel->cmdbuf;
	if (!cmdbuf) {
		panel_drv_err(panel, "command buffer is null\n");
		return -EINVAL;
	}

	adapter_fifo_size = get_panel_adapter_fifo_size(panel);
	if (!adapter_fifo_size) {
		panel_drv_err(panel, "invalid panel adapter fifo size(%d)\n",
				adapter_fifo_size);
		return -EINVAL;
	}
	...
	if (cmd_id == MIPI_DSI_WR_GRAM_CMD) {
		...
		panel_drv_err(panel, "invalid cmd_id %d\n", cmd_id);
		return -EINVAL;
	}
	...
	do {
		...
		panel_drv_dbg(panel, "tx_size %d len %d, remained %d\n", data_size, len, remained);
	} while (remained > 0);
	...
	if (on) {
		if (level >= 1) {
			...
			if (ret != ARRAY_SIZE(SEQ_TEST_KEY_ON_9F)) {
				panel_drv_err(panel, "fail to write CMD : SEQ_TEST_KEY_ON_9F\n");
				return -EIO;
			}
		}
		...
		if (level >= 2) {
			...
			if (ret != ARRAY_SIZE(SEQ_TEST_KEY_ON_F0)) {
				panel_drv_err(panel, "fail to write CMD : SEQ_TEST_KEY_ON_F0\n");
				return -EIO;
			}
		}
		...
		if (level >= 3) {
			...
			if (ret != ARRAY_SIZE(SEQ_TEST_KEY_ON_FC)) {
				panel_drv_err(panel, "fail to write CMD : SEQ_TEST_KEY_ON_FC\n");
				return -EIO;
			}
		}
	} else {
		if (level >= 3) {
			...
			if (ret != ARRAY_SIZE(SEQ_TEST_KEY_ON_FC)) {
				panel_drv_err(panel, "fail to write CMD : SEQ_TEST_KEY_OFF_FC\n");
				return -EIO;
			}
		}
		...
		if (level >= 2) {
			...
			if (ret != ARRAY_SIZE(SEQ_TEST_KEY_ON_F0)) {
				panel_drv_err(panel, "fail to write CMD : SEQ_TEST_KEY_OFF_F0\n");
				return -EIO;
			}
		}
		...
		if (level >= 1) {
			...
			if (ret != ARRAY_SIZE(SEQ_TEST_KEY_ON_9F)) {
				panel_drv_err(panel, "fail to write CMD : SEQ_TEST_KEY_OFF_9F\n");
				return -EIO;
			}
		}
	}
...
int panel_verify_tx_packet(struct panel_device *panel, u8 *src, u32 ofs, u8 len)
{
	...
	for (i = 0; i < len - 1; i++) {
		if (buf[i] != data[i]) {
			panel_drv_warn(panel, "%02Xh[%2d] - (tx 0x%02X, rx 0x%02X) not match\n",
					addr, i, data[i], buf[i]);
			ret = -EINVAL;
		} else {
			panel_drv_info(panel, "%02Xh[%2d] - (tx 0x%02X, rx 0x%02X) match\n",
					addr, i, data[i], buf[i]);
		}
	}
	kfree(buf);
...
__visible_for_testing int panel_do_dsi_tx_packet(struct panel_device *panel, struct pktinfo *info, bool block)
{
	...
	if (cmd_id == MIPI_DSI_WR_UNKNOWN) {
		panel_drv_err(panel, "%s unknown packet type(%d)\n",
	...
	panel_drv_dbg(panel, "%s ofs:0x%X opt:0x%X\n",
	...
	if (ret != info->dlen) {
		panel_drv_err(panel, "failed to send packet %s (ret %d)\n",
...
__visible_for_testing int panel_do_power_ctrl(struct panel_device *panel, struct pwrctrl *pwrctrl)
{
	...
	if (!pwrctrl) {
		panel_drv_err(panel, "pwrctrl is null\n");
		return -EINVAL;
	}

	if (get_pnobj_cmd_type(&pwrctrl->base) != CMD_TYPE_PCTRL) {
		panel_drv_err(panel, "got invalid type\n");
		return -EINVAL;
	}

	if (!get_pwrctrl_name(pwrctrl)) {
		panel_drv_err(panel, "got null name\n");
		return -EINVAL;
	}

	key = get_pwrctrl_key(pwrctrl);
	if (!key) {
		panel_drv_err(panel, "got null power control key\n");
		return -EINVAL;
	}

	if (!panel_drv_power_ctrl_exists(panel, key)) {
		panel_drv_warn(panel, "cannot found %s in power control list\n", key);
		return -ENODATA;
	}
	panel_drv_dbg(panel, "run power_ctrl: %s\n", key);
...
int __mockable panel_spi_packet(struct panel_device *panel, struct pktinfo *info)
{
	...
	spi_dev = &panel->panel_spi_dev;
	if (!spi_dev->ops) {
		panel_drv_err(panel, "spi_dev.ops not found\n");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to send spi packet %s (ret %d type %d)\n",
...
static int panel_do_i2c_packet(struct panel_device *panel, struct pktinfo *info)
{
	...
	if (!i2c_dev) {
		panel_drv_err(panel, "i2c_dev not found\n");
		return -EINVAL;
	}

	if (!i2c_dev->ops) {
		panel_drv_err(panel, "i2c_dev.ops not found\n");
		return -EINVAL;
	}
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to send i2c packet %s (ret %d type %d)\n", get_pktinfo_name(info), ret, type);
		return -EINVAL;
	}
	return 0;
}
...
static int panel_do_setkey(struct panel_device *panel, struct keyinfo *info, bool block)
{
	...
	if (info->level < 0 || info->level >= MAX_CMD_LEVEL) {
		panel_drv_err(panel, "invalid level %d\n", info->level);
		return -EINVAL;
	}

	if (info->en != KEY_ENABLE && info->en != KEY_DISABLE) {
		panel_drv_err(panel, "invalid key type %d\n", info->en);
		return -EINVAL;
	}
	...
	if (info->en == KEY_ENABLE) {
		...
		if (panel_data->props.key[info->level] < 1) {
			panel_drv_err(panel, "unbalanced key [%d]%s(%d)\n", info->level,
	...
	if (send_packet) {
		...
		if (ret < 0)
			panel_drv_err(panel, "failed to tx packet(%s)\n", get_pktinfo_name(info->packet));
	} else {
		panel_drv_warn(panel, "ignore send key %d %s packet\n",
				info->level, info->en ? "ENABLED" : "DISABLED");
	}

	panel_drv_dbg(panel, "key [1]%s(%d) [2]%s(%d) [3]%s(%d)\n",
...
int panel_do_init_maptbl(struct panel_device *panel, struct maptbl *maptbl)
{
	...
	if (ret < 0) {
		panel_drv_err(panel, "failed to init maptbl(%s) ret %d\n", maptbl_get_name(maptbl), ret);
		return ret;
	}
		...
		struct seqinfo *seq, int depth)
{
	...
	cmdtbl = seq->cmdtbl;
	if (unlikely(!cmdtbl)) {
		panel_drv_err(panel, "invalid command table\n");
		return -EINVAL;
	}

	for (i = 0; i < seq->size; i++) {
		if (cmdtbl[i] == NULL) {
			panel_drv_dbg(panel, "end of cmdtbl %d\n", i);
			break;
		}
		type = get_pnobj_cmd_type((struct pnobj *)cmdtbl[i]);
		if (type >= MAX_CMD_TYPE) {
			panel_drv_warn(panel, "invalid cmd type %u\n", type);
			break;
		}

		panel_drv_dbg(panel, "SEQ: %s %s %s+ %s\n",
		...
		if (!condition) {
			...
			case CMD_TYPE_COND_IF:
				panel_drv_dbg(panel, "condition skip %d to %d\n", cond_skip_count, cond_skip_count + 1);
				cond_skip_count++;
				cont = true;
				break;
		...
		case CMD_TYPE_TX_PACKET:
			...
			if (i + 1 < seq->size && cmdtbl[i + 1] &&
				...
				block = true;
				panel_drv_dbg(panel, "blocking call : next cmd is not tx packet\n");
			}

			/* blocking call if end of seq */
			if (depth == 0 && (i + 1 == seq->size)) {
				block = true;
				panel_drv_dbg(panel, "blocking call : end of seq\n");
			}
		...
		case CMD_TYPE_COND_IF:
			...
			panel_drv_dbg(panel, "condition set to: %s\n", condition ? "true" : "false");
			break;
		case CMD_TYPE_COND_EL:
			condition = !condition;
			panel_drv_dbg(panel, "condition else, set to %s\n", condition ? "true" : "false");
			break;
		case CMD_TYPE_COND_FI:
			condition = true;
			panel_drv_dbg(panel, "condition clear\n");
			break;
		case CMD_TYPE_PCTRL:
		...
		case CMD_TYPE_NONE:
		default:
			panel_drv_warn(panel, "unknown command type %d\n", type);
			break;
		}

		if (ret < 0) {
			panel_drv_err(panel, "failed to do(seq:%s type:%s cmd:%s)\n",
					get_sequence_name(seq),
					cmd_type_to_string(type),
					(get_pnobj_name((struct pnobj *)cmdtbl[i]) ?: "none"));
	...
	if (!condition)
		panel_drv_err(panel, "condition end missing!\n");

	if (cond_skip_count != 0)
		panel_drv_err(panel, "invalid condition pair: %d\n", cond_skip_count);
...
int execute_sequence_nolock(struct panel_device *panel, struct seqinfo *seq)
{
	...
	if (panel_bypass_is_on(panel)) {
		panel_drv_warn(panel, "panel no use\n");
		return 0;
	}
	...
	if (panel_data->props.key[CMD_LEVEL_1] != 0 ||
		...
		panel_drv_warn(panel, "before seq:%s unbalanced key [1]%s(%d) [2]%s(%d) [3]%s(%d)\n",
				get_sequence_name(seq),
				(panel_data->props.key[CMD_LEVEL_1] > 0 ? "ENABLED" : "DISABLED"),
				panel_data->props.key[CMD_LEVEL_1],
				(panel_data->props.key[CMD_LEVEL_2] > 0 ? "ENABLED" : "DISABLED"),
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "failed to execute seqtbl %s\n",
				get_sequence_name(seq));
		ret = -EIO;
		goto do_exit;
	}
...
do_exit:
	if (panel_data->props.key[CMD_LEVEL_1] != 0 ||
			panel_data->props.key[CMD_LEVEL_2] != 0 ||
			panel_data->props.key[CMD_LEVEL_3] != 0) {
		panel_drv_warn(panel, "after seq:%s unbalanced key [1]%s(%d) [2]%s(%d) [3]%s(%d)\n",
				get_sequence_name(seq),
				(panel_data->props.key[CMD_LEVEL_1] > 0 ? "ENABLED" : "DISABLED"),
				panel_data->props.key[CMD_LEVEL_1],
				(panel_data->props.key[CMD_LEVEL_2] > 0 ? "ENABLED" : "DISABLED"),
	...
	panel_drv_dbg(panel, "seq:%s done (elapsed %2lld.%03lld msec)\n",
			get_sequence_name(seq),
...
bool is_panel_resource_initialized(struct panel_device *panel, char *name)
{
	...
	if (unlikely(!res)) {
		panel_drv_err(panel, "%s not found in resource\n", name);
		return false;
	}
...
int panel_resource_clear(struct panel_device *panel, char *name)
{
	...
	if (unlikely(!name)) {
		panel_drv_warn(panel, "invalid parameter\n");
		return -EINVAL;
	}

	res = find_panel_resource(panel, name);
	if (unlikely(!res)) {
		panel_drv_warn(panel, "%s not found in resource\n", name);
		return -EINVAL;
	}
...
int panel_resource_copy_and_clear(struct panel_device *panel, u8 *dst, char *name)
{
	...
	if (!is_resource_initialized(res)) {
		panel_drv_warn(panel, "%s not initialized\n", get_resource_name(res));
		return -EINVAL;
	}
		...
		u32 type, u8 *buf, u8 addr, u32 offset, u32 len)
{
	...
	if (!IS_RX_PKT_TYPE(type)) {
		panel_drv_err(panel, "invalid type %d\n", type);
		return -EINVAL;
	}
	...
	while (remained > 0) {
		...
		if (type == DSI_PKT_TYPE_RD) {
			...
			if (pos != 0) {
				...
				if (ret != gpara_len)
					panel_drv_err(panel, "failed to set gpara %d (ret %d)\n", pos, ret);
			}
		...
		if (ret != read_len) {
			panel_drv_err(panel, "failed to read addr:0x%02X, pos:%d, len:%u ret %d\n",
					addr, pos, len, ret);
			return -EINVAL;
		}
		index += read_len;
	...
	panel_drv_dbg(panel, "addr:%2Xh, pos:%u, len:%u\n", addr, offset, len);
...
int read_panel_id(struct panel_device *panel, u8 *buf)
{
	...
	panel_drv_info(panel, "id read from 0x%02X 3bytes\n", PANEL_ID_REG);
	panel_mutex_lock(&panel->op_lock);
...
int panel_rdinfo_update(struct panel_device *panel, struct rdinfo *rdi)
{
	...
	if (unlikely(ret != rdi->len)) {
		panel_drv_err(panel, "read failed\n");
		return -EIO;
	}
...
int panel_rdinfo_update_by_name(struct panel_device *panel, char *name)
{
	...
	if (unlikely(!rdi)) {
		panel_drv_err(panel, "read info %s not found\n", name);
		return -ENODEV;
	}

	ret = panel_rdinfo_update(panel, rdi);
	if (ret) {
		panel_drv_err(panel, "failed to update read info\n");
		return -EINVAL;
	}
...
int panel_resource_update(struct panel_device *panel, struct resinfo *res)
{
	...
	for (i = 0; i < res->nr_resui; i++) {
		...
		if (unlikely(!rdi)) {
			panel_drv_warn(panel, "read info not exist\n");
			return -EINVAL;
		}
		ret = panel_rdinfo_update(panel, rdi);
		if (ret) {
		...
		if (!res->data) {
			panel_drv_warn(panel, "resource data not exist\n");
			return -EINVAL;
		}
	...
	panel_drv_dbg(panel, "[panel_resource : %12s %3d bytes] %s\n",
...
int __mockable panel_resource_update_by_name(struct panel_device *panel, char *name)
{
	...
	if (unlikely(ret)) {
		panel_drv_err(panel, "failed to update resource\n");
		return ret;
	}
...
int panel_dumpinfo_update(struct panel_device *panel, struct dumpinfo *info)
{
	...
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "dump:%s failed to update resource\n", get_dump_name(info));
		return ret;
	}

	info->result = DUMP_STATUS_NONE;
	ret = call_dump_func(panel, info);
	if (unlikely(ret < 0)) {
		panel_drv_err(panel, "dump:%s failed to show\n", get_dump_name(info));
		return ret;
	}
...
int check_panel_active(struct panel_device *panel, const char *caller)
{
	...
	state = &panel->state;
	if (panel_bypass_is_on(panel)) {
		panel_drv_warn(panel, "%s:panel no use\n", caller);
		return 0;
	}
	ctrl_interface_state = panel_dsi_get_state(panel);
	if (ctrl_interface_state < 0) {
		panel_drv_warn(panel, "%s:unable to get dsi state\n", caller);
		return 0;
	}

	if (ctrl_interface_state == CTRL_INTERFACE_STATE_INACTIVE) {
		panel_drv_warn(panel, "%s:dsi is inactive state\n", caller);
		return 0;
	}
