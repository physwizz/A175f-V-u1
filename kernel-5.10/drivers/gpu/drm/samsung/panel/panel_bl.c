...
#define PANEL_BL_DRIVER_NAME "panel-bl"
#define panel_bl_info(_panel_bl, fmt, ...)   \
	usdm_pr_info(PANEL_BL_DRIVER_NAME, ((_panel_bl) ? (_panel_bl)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_bl_warn(_panel_bl, fmt, ...)   \
	usdm_pr_warn(PANEL_BL_DRIVER_NAME, ((_panel_bl) ? (_panel_bl)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_bl_err(_panel_bl, fmt, ...)    \
	usdm_pr_err(PANEL_BL_DRIVER_NAME, ((_panel_bl) ? (_panel_bl)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)

#define panel_bl_dbg(_panel_bl, fmt, ...)\
	usdm_pr_debug(PANEL_BL_DRIVER_NAME, ((_panel_bl) ? (_panel_bl)->id : -1), panel_log_level, fmt, ##__VA_ARGS__)
		...
		int *property, unsigned int value)
{
	...
	if (!property) {
		panel_bl_err(panel_bl, "property is null\n");
		return -EINVAL;
	}
	...
	if (!propname) {
		panel_bl_err(panel_bl, "unknown property\n");
		return 0;
	}

	if (panel_set_property_value(panel,
				propname, value) < 0) {
		panel_bl_warn(panel_bl, "failed to set property(%s) %d\n",
				propname, value);
		return -EINVAL;
	}
	*property = value;
...
bool panel_bl_subdev_brt_tbl_exist(struct panel_bl_device *panel_bl, int id)
{
	...
	if (id < 0 || id >= MAX_PANEL_BL_SUBDEV) {
		panel_bl_err(panel_bl, "bl-%d exceeded max subdev\n", id);
		return false;
	}
	brt_tbl = &panel_bl->subdev[id].brt_tbl;

	if (!brt_tbl->brt || !brt_tbl->sz_brt) {
		panel_bl_dbg(panel_bl, "bl-%d brightness table is empty\n", id);
		return false;
	}
...
static int get_subdev_max_brightness(struct panel_bl_device *panel_bl, int id)
{
	...
	max_brt = max_brt_tbl(&subdev->brt_tbl);
	if (max_brt < 0)
		panel_bl_err(panel_bl, "bl-%d invalid max brightness %d\n", id, max_brt);

	/* TODO: FIX: return zero? */
	return max_brt;
}
...
static bool is_valid_brightness(struct panel_bl_device *panel_bl, int brightness)
{
	...
	if (brightness < 0 || brightness > max_brightness) {
		panel_bl_err(panel_bl, "out of range %d, (min:0, max:%d)\n",
...
bool __mockable is_hbm_brightness(struct panel_bl_device *panel_bl, int brightness)
{
	...
	if (sz_ui_lum <= 0 || sz_ui_lum > subdev->brt_tbl.sz_lum) {
		panel_bl_err(panel_bl, "bl-%d out of range (sz_ui_lum %d)\n",
...
bool is_ext_hbm_brightness(struct panel_bl_device *panel_bl, int brightness)
{
	...
	if (sz_hbm_lum <= 0 || sz_ui_lum + sz_hbm_lum > subdev->brt_tbl.sz_lum) {
		panel_bl_err(panel_bl, "bl-%d out of range (sz_hbm_lum %d)\n",
		...
		int id, int brightness)
{
	...
	if (!is_valid_brightness(panel_bl, brightness)) {
		panel_bl_err(panel_bl, "bl-%d invalid brightness\n", id);
		return -EINVAL;
	}
	...
	if (brightness >= subdev->sz_brt_cache_tbl) {
		panel_bl_err(panel_bl, "bl-%d exceeded brt_cache_tbl size %d\n", id, brightness);
		return -EINVAL;
	}
	...
	if (subdev->brt_cache_tbl[brightness] == -1) {
		...
		if (index < 0) {
			panel_bl_err(panel_bl, "bl-%d failed to search %d, ret %d\n",
					id, brightness, index);
			return index;
		}
		subdev->brt_cache_tbl[brightness] = index;
#ifdef DEBUG_PAC
		panel_bl_info(panel_bl, "bl-%d brightness %d, brt_cache_tbl[%d] = %d\n",
...
int get_brightness_pac_step_by_subdev_id(struct panel_bl_device *panel_bl, int id, int brightness)
{
	...
	if (unlikely(id >= MAX_PANEL_BL_SUBDEV)) {
		panel_bl_err(panel_bl, "invalid id %d\n", id);
		return -EINVAL;
	}
	...
	if (index < 0) {
		panel_bl_err(panel_bl, "failed to search %d, ret %d\n", brightness, index);
...
#ifdef DEBUG_PAC
	panel_bl_info(panel_bl, "bl-%d brightness %d, pac step %d, brt %d\n",
			id, brightness, index,
...
int get_brightness_of_brt_to_step(struct panel_bl_device *panel_bl, int id, int brightness)
{
	...
	if (step < 0) {
		panel_bl_err(panel_bl, "bl-%d invalid pac stap %d\n", id, step);
		return -EINVAL;
	}
...
int get_subdev_actual_brightness(struct panel_bl_device *panel_bl, int id, int brightness)
{
	...
	if (index < 0) {
		panel_bl_err(panel_bl, "bl-%d failed to get actual_brightness_index %d\n", id, index);
		return index;
	}
...
int get_subdev_actual_brightness_interpolation(struct panel_bl_device *panel_bl, int id, int brightness)
{
	...
	if (unlikely(!panel_bl)) {
		panel_bl_err(panel_bl, "invalid parameter\n");
		return -EINVAL;
	}
...
void panel_bl_update_acl_state(struct panel_bl_device *panel_bl)
{
	...
	if (panel_data->props.adaptive_control > 100) {
		panel_bl_warn(panel_bl, "invalid range %d\n", panel_data->props.adaptive_control);
		return;
	}
...
int panel_bl_set_brightness(struct panel_bl_device *panel_bl, int id, u32 send_cmd)
{
	...
	if (!subdev->brt_tbl.brt || subdev->brt_tbl.sz_brt == 0) {
		panel_bl_err(panel_bl, "bl-%d brightness table not exist\n", id);
		return -EINVAL;
	}
	...
	panel_bl_info(panel_bl, "bl-%d br:%d nit:%d acl:%d cnt:%d%s\n",
	...
	if (unlikely(ret < 0)) {
		panel_bl_err(panel_bl, "failed to write set_bl seqtbl %s\n", seqname);
		goto set_br_exit;
	}
#ifdef CONFIG_USDM_BLIC_I2C
	ret = panel_bl_set_brightness_blic(panel_bl);
	if (unlikely(ret < 0)) {
		panel_bl_err(panel_bl, "failed to write panel_bl_set_brightness_blic\n");
		goto set_br_exit;
	}
#endif
	...
	const int subdev_id, const int brt)
{
	...
	if (max < 0) {
		panel_bl_err(panel_bl, "failed to update subdev[%d] brightness\n", subdev_id);
		return 0;
	} else if (max < brt) {
		panel_bl_dbg(panel_bl, "subdev[%d] brightness set to %d -> %d\n", subdev_id, brt, max);
		return max;
	}
	return brt;
}
...
int panel_update_subdev_brightness(struct panel_device *panel, u32 subdev_id, u32 brightness)
{
	...
	if (subdev_id >= MAX_PANEL_BL_SUBDEV) {
		panel_bl_err(panel_bl, "invalid subdev id %u\n", subdev_id);
		return -EINVAL;
	}
...
int _panel_update_brightness_nolock(struct panel_device *panel, u32 send_cmd)
{
	...
	if (bd == NULL) {
		panel_bl_dbg(panel_bl, "panel_bl not prepared\n");
		return -ENODEV;
	}

#ifdef CONFIG_USDM_PANEL_TESTMODE
	if (panel_testmode_is_on(panel)) {
		panel_bl_info(panel_bl, "testmode is running. ignore brightness updates.");
		return 0;
	}
#endif
	...
	if (brightness < 0) {
		panel_bl_err(panel_bl, "brightness %d is out of range\n", brightness);
		brightness = 0;
	}
	...
	if (panel_bl->props.mask_layer_br_hook == MASK_LAYER_HOOK_ON) {
		...
		panel_bl_info(panel_bl, "mask_layer_br_hook (%d)->(%d), skip brighntess\n",
...
#ifdef CONFIG_USDM_PANEL_HMD
	if (id == PANEL_BL_SUBDEV_TYPE_HMD) {
		panel_bl_info(panel_bl, "keep plat_br:%d\n", brightness);
		ret = -EINVAL;
		goto exit_set;
	}
#endif
	...
	if (ret) {
		panel_bl_err(panel_bl, "failed to set_brightness (ret %d)\n", ret);
		goto exit_set;
	}
...
static int panel_bl_create_thread(struct panel_bl_device *panel_bl)
{
	...
	if (IS_ERR_OR_NULL(panel_bl->wq.thread)) {
		panel_bl_err(panel_bl, "failed to run panel_bl thread\n");
...
__visible_for_testing int panel_bl_init_property(struct panel_bl_device *panel_bl)
{
	...
	if (ret < 0) {
		panel_bl_err(panel_bl, "failed to add prop array\n");
		return ret;
	}
...
int panel_bl_init(struct panel_bl_device *panel_bl)
{
	...
	if (ret < 0) {
		panel_bl_err(panel_bl, "failed to init property\n");
		return ret;
	}
	...
	if (IS_ERR(bd)) {
		panel_bl_err(panel_bl, "failed to register backlight\n");
	...
	if (ret < 0) {
		panel_bl_err(panel_bl, "failed to create panel_bl thread\n");
		return ret;
	}

	panel_bl_info(panel_bl, "panel_bl init success\n");
...
int panel_bl_exit(struct panel_bl_device *panel_bl)
{
	...
	if (IS_ERR_OR_NULL(panel_bl->bd)) {
		panel_bl_err(panel_bl, "backlight device is null\n");
		return -EINVAL;
	}
...
__visible_for_testing int panel_bl_init_backlight_device_property(struct panel_bl_device *panel_bl)
{
	...
	if (!panel_bl->bd) {
		panel_bl_err(panel_bl, "backlight_device is null\n");
		return -EINVAL;
	}
...
__visible_for_testing int panel_bl_alloc_brt_cache(struct panel_bl_device *panel_bl)
{
	...
	for (id = 0; id < MAX_PANEL_BL_SUBDEV; id++) {
		...
		if (size <= 0) {
			panel_bl_warn(panel_bl, "invalid brightness table (size %d)\n", size);
			panel_bl->subdev[id].sz_brt_cache_tbl = -1;
			continue;
		}

		if (panel_bl->subdev[id].brt_cache_tbl) {
			panel_bl_info(panel_bl, "bl-%d free brt_cache\n", id);
...
int panel_bl_probe(struct panel_bl_device *panel_bl)
{
	...
	if (ret < 0) {
		panel_bl_err(panel_bl, "failed to init backlight device property\n");
		goto err;
	}

	ret = panel_bl_alloc_brt_cache(panel_bl);
	if (ret < 0) {
		panel_bl_err(panel_bl, "failed to alloc brt cache\n");
		goto err;
	}

	panel_bl_info(panel_bl, "panel_bl probe success\n");
