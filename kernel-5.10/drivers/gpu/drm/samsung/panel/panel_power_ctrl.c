...
static int panel_power_ctrl_action_delay(struct panel_device *panel,
		...
		struct panel_power_ctrl_action *paction)
{
	...
	if (paction->value == 0) {
		panel_drv_warn(panel, "%s:%s delay must be grater than 0\n", pctrl->name, paction->name);
		return -EINVAL;
	}
	...
	case PANEL_POWER_CTRL_ACTION_DELAY_USLEEP:
		...
		break;
	default:
		panel_drv_err(panel, "%s:%s is not delay action\n", pctrl->name, paction->name);
		break;
	}
	return 0;
}

static int panel_power_ctrl_action_gpio(struct panel_device *panel,
		struct panel_power_ctrl *pctrl,
		struct panel_power_ctrl_action *paction)
{
	int ret = 0;
	...
	case PANEL_POWER_CTRL_ACTION_GPIO_ENABLE:
		if (!paction->gpio) {
			panel_drv_err(panel, "%s %s gpio_enable: invalid gpio\n", pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		ret = panel_gpio_helper_set_value(panel, paction->gpio, 1);
		if (ret < 0) {
			panel_drv_err(panel, "failed to execute gpio_enable %s %s %s %d\n",
				pctrl->name, paction->name, paction->gpio->name, ret);
		}
		break;
	case PANEL_POWER_CTRL_ACTION_GPIO_DISABLE:
		if (!paction->gpio) {
			panel_drv_err(panel, "%s %s gpio_disable: invalid gpio\n", pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		ret = panel_gpio_helper_set_value(panel, paction->gpio, 0);
		if (ret < 0) {
			panel_drv_err(panel, "failed to execute gpio_disable %s %s %s %d\n",
				pctrl->name, paction->name, paction->gpio->name, ret);
		}
		break;
	default:
		panel_drv_err(panel, "%s:%s is not gpio action\n", pctrl->name, paction->name);
		break;
	}
	return ret;
}

static int panel_power_ctrl_action_regulator(struct panel_device *panel,
		struct panel_power_ctrl *pctrl,
		struct panel_power_ctrl_action *paction)
{
	int ret = 0;
	...
	case PANEL_POWER_CTRL_ACTION_REGULATOR_ENABLE:
		if (!paction->reg) {
			panel_drv_err(panel, "%s %s regulator_enable: invalid regulator\n",
				pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		ret = panel_regulator_helper_enable(panel, paction->reg);
		if (ret < 0) {
			panel_drv_err(panel, "failed to execute regulator_enable %s %s %s %d\n",
				pctrl->name, paction->name, paction->reg->node_name, ret);
		}
		break;
	case PANEL_POWER_CTRL_ACTION_REGULATOR_DISABLE:
		if (!paction->reg) {
			panel_drv_err(panel, "%s %s regulator_disable: invalid regulator\n",
				pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		ret = panel_regulator_helper_disable(panel, paction->reg);
		if (ret < 0) {
			panel_drv_err(panel, "failed to execute regulator_disable %s %s %s %d\n",
				pctrl->name, paction->name, paction->reg->node_name, ret);
		}
		break;
	case PANEL_POWER_CTRL_ACTION_REGULATOR_SET_VOLTAGE:
		if (!paction->reg) {
			panel_drv_err(panel, "%s %s regulator_set_voltage: invalid regulator\n",
				pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		if (!paction->value) {
			panel_drv_err(panel, "%s %s regulator_set_voltage: invalid value\n",
				pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		ret = panel_regulator_helper_set_voltage(panel, paction->reg, paction->value);
		if (ret < 0) {
			panel_drv_err(panel, "failed to execute regulator_set_voltage %s %s %s %u %d\n",
				pctrl->name, paction->name, paction->reg->node_name, paction->value, ret);
		}
		break;
	case PANEL_POWER_CTRL_ACTION_REGULATOR_SSD_CURRENT:
		if (!paction->reg) {
			panel_drv_err(panel, "%s %s regulator_ssd_current: invalid regulator\n",
				pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		ret = panel_regulator_helper_set_current_limit(panel, paction->reg, paction->value);
		if (ret < 0) {
			panel_drv_err(panel, "failed to execute regulator_ssd_current %s %s %s %u %d\n",
				pctrl->name, paction->name, paction->reg->node_name, paction->value, ret);
		}
		break;
	case PANEL_POWER_CTRL_ACTION_REGULATOR_FORCE_DISABLE:
		if (!paction->reg) {
			panel_drv_err(panel, "%s %s regulator_force_disable: invalid regulator\n",
				pctrl->name, paction->name);
			ret = -ENODEV;
			break;
		}
		ret = panel_regulator_helper_force_disable(panel, paction->reg);
		if (ret < 0) {
			panel_drv_err(panel, "failed to execute regulator_disable %s %s %s %d\n",
				pctrl->name, paction->name, paction->reg->node_name, ret);
		}
		break;
	default:
		panel_drv_err(panel, "%s:%s is not regulator action\n", pctrl->name, paction->name);
		break;
	}
	return ret;
}

static int panel_power_ctrl_action_gpio_wait(struct panel_device *panel,
		struct panel_power_ctrl *pctrl,
		struct panel_power_ctrl_action *paction)
{
	...
	case PANEL_POWER_CTRL_ACTION_GPIO_WAIT_HIGH:
		...
		break;
	default:
		panel_drv_err(panel, "%s:%s is not gpio wait action\n", pctrl->name, paction->name);
		ret = -EINVAL;
		return ret;
	}
	...
	for (i = 0 ; i <= paction->value; i += check_duration) {
		if (panel_gpio_helper_get_value(panel, paction->gpio) == expected_level) {
			ret = 0;
			break;
		}
	...
	panel_drv_info(panel, "%s(%s) took %llu ms (%s)\n",
		paction->gpio->name,
		(expected_level ? "High" : "Low"), time_diff,
		(ret == 0 ? "Success" : "Timeout!!!"));

	if (ret < 0) {
		panel_drv_err(panel, "failed to execute gpio_wait %s %s %s(%s) %d\n",
...
static int panel_power_ctrl_action_execute(struct panel_device *panel,
		struct panel_power_ctrl *pctrl)
{
	struct panel_power_ctrl_action *paction;
	int ret = 0;
	...
	list_for_each_entry(paction, &pctrl->action_list, head) {
		panel_drv_dbg(panel, "pctrl %s action %s type %d value %u\n",
		...
		case PANEL_POWER_CTRL_ACTION_REGULATOR_SSD_CURRENT:
		case PANEL_POWER_CTRL_ACTION_REGULATOR_FORCE_DISABLE:
			ret = panel_power_ctrl_action_regulator(panel, pctrl, paction);
			break;
		case PANEL_POWER_CTRL_ACTION_GPIO_ENABLE:
		case PANEL_POWER_CTRL_ACTION_GPIO_DISABLE:
			ret = panel_power_ctrl_action_gpio(panel, pctrl, paction);
			break;
		case PANEL_POWER_CTRL_ACTION_DELAY_MSLEEP:
		case PANEL_POWER_CTRL_ACTION_DELAY_USLEEP:
			ret = panel_power_ctrl_action_delay(panel, pctrl, paction);
			break;
		case PANEL_POWER_CTRL_ACTION_GPIO_WAIT_LOW:
		case PANEL_POWER_CTRL_ACTION_GPIO_WAIT_HIGH:
			ret = panel_power_ctrl_action_gpio_wait(panel, pctrl, paction);
			break;
		default:
			panel_drv_warn(panel, "%s:%s invalid action type %d\n",
		...
		if (ret < 0) {
			panel_drv_err(panel, "%s:%s action failed\n",
					pctrl->name, paction->name);
			return ret;
		}

		panel_drv_dbg(panel, "%s:%s action done\n",
				pctrl->name, paction->name);
	}
...
int panel_power_ctrl_helper_execute(struct panel_device *panel, struct panel_power_ctrl *pctrl)
{
	if (!pctrl)
		return -EINVAL;

	return call_panel_power_ctrl_func(panel, pctrl, execute);
}
	...
	struct device_node *action_np, struct panel_power_ctrl_action *paction)
{
	...
	panel_drv_dbg(panel, "found %s in gpio_list\n", paction->gpio->name);
exit:
	of_node_put(reg_np);
	return ret;
}
	...
	struct device_node *action_np, struct panel_power_ctrl_action *paction)
{
	...
	panel_drv_dbg(panel, "found %s %s in regulator_list\n", paction->reg->name, paction->reg->node_name);
exit:
	of_node_put(reg_np);
	return ret;
}
	...
	struct device_node *action_np, struct panel_power_ctrl_action *paction)
{
	...
	panel_drv_dbg(panel, "find value in node %s: %u\n", action_np->name, paction->value);
	return ret;
}
	...
	const char *prop_name, struct panel_power_ctrl *pctrl)
{
	...
	if (sz <= 0) {
		panel_drv_warn(panel, "sequence '%s' is empty\n", prop_name);
		return 0;
	}
	...
	for (i = 0; i < sz; i++) {
		...
		if (!action_np) {
			panel_drv_warn(panel, "failed to get phandle of %s[%d]\n", prop_name, i);
			return -EINVAL;
		}
		...
		if (of_property_read_u32(action_np, "type", &action->type)) {
			panel_drv_err(panel, "%s:property('type') not found\n", action_np->name);
			return -EINVAL;
		}

		if (action->type >= MAX_PANEL_POWER_CTRL_ACTION) {
			panel_drv_err(panel, "%s: invalid action type %d\n", action->name, action->type);
			return -EINVAL;
		}
		panel_drv_dbg(panel, "%s %s %d\n", prop_name, action->name, action->type);

		if (REQUIRE_PARSE_REG(action_table[action->type])) {
			ret = of_get_action_reg(panel, action_np, action);
			if (ret < 0) {
				panel_drv_err(panel, "error occurred during of_get_action_reg %s %d\n", action->name, ret);
				kfree(action);
				continue;
			}
		}
		if (REQUIRE_PARSE_GPIO(action_table[action->type])) {
			...
			if (ret < 0) {
				panel_drv_err(panel, "error occurred during of_get_action_gpio %s %d\n", action->name, ret);
				kfree(action);
				continue;
			}
		}
		if (REQUIRE_PARSE_VALUE(action_table[action->type])) {
			...
			if (ret < 0) {
				panel_drv_err(panel, "error occurred during of_get_action_value %s %d\n", action->name, ret);
				kfree(action);
				continue;
			}
		}
		...
		panel_drv_info(panel, "%s type %d reg %pK gpio %pK value %d\n",
	...
	const char *dev_name, const char *name)
{
	...
	panel_drv_dbg(panel, "find: %s, %s\n", dev_name, name);
	list_for_each_entry(pctrl, &panel->power_ctrl_list, head) {
		if (!pctrl->dev_name) {
			panel_drv_err(panel, "invalid 'dev_name', skip to check\n");
			continue;
		}
		if (!pctrl->name) {
			panel_drv_err(panel, "invalid 'name', skip to check\n");
			continue;
		}
	...
	const char *dev_name, const char *name)
{
	...
	if (!dev_name || !name) {
		panel_drv_err(panel, "invalid arg\n");
		return false;
	}
	...
	if (IS_ERR_OR_NULL(pctrl)) {
		if (PTR_ERR(pctrl) == -ENODATA)
			panel_drv_dbg(panel, "not found %s\n", name);
		else
			panel_drv_err(panel, "error occurred when find %s, %ld\n", name, PTR_ERR(pctrl));
		return false;
	}
	return true;
}
	...
	const char *dev_name, const char *name)
{
	...
	if (IS_ERR_OR_NULL(pctrl)) {
		if (PTR_ERR(pctrl) == -ENODATA) {
			panel_drv_dbg(panel, "%s not found\n", name);
			return -ENODATA;
		}
		panel_drv_err(panel, "error occurred when find %s, %ld\n", name, PTR_ERR(pctrl));
		return PTR_ERR(pctrl);
	}
	return panel_power_ctrl_helper_execute(panel, pctrl);
}
