...
#if IS_ENABLED(CONFIG_SEC_NFC_PVDDOFF_ONSHUTDOWN)
static void sec_nfc_shutdown_work(struct work_struct *work)
{
	struct sec_nfc_platform_data *pdata = g_nfc_info->pdata;

	NFC_LOG_INFO("on shutdown\n");
	gpio_set_value(pdata->ven, SEC_NFC_PW_ON);
	msleep(20);
	sec_nfc_regulator_onoff(pdata, NFC_I2C_LDO_OFF);
}
#endif
...
#endif
{
	...
	if (buf[0] == '5') {
		...
		schedule_delayed_work(&pdata->shutdown_work,
...
#if IS_ENABLED(CONFIG_SEC_NFC_PVDDOFF_ONSHUTDOWN)
	INIT_DELAYED_WORK(&pdata->shutdown_work, sec_nfc_shutdown_work);
#endif
