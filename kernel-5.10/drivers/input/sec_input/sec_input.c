...
out:
	if (IS_ERR_OR_NULL(pdata->dvdd))
		snprintf(str_dvdd, sizeof(str_dvdd), "null");
	else
		snprintf(str_dvdd, sizeof(str_dvdd), "%s", regulator_is_enabled(pdata->dvdd) ? "on" : "off");

	if (IS_ERR_OR_NULL(pdata->avdd))
		snprintf(str_avdd, sizeof(str_avdd), "null");
	else
		snprintf(str_avdd, sizeof(str_avdd), "%s", regulator_is_enabled(pdata->avdd) ? "on" : "off");

	input_info(true, dev, "%s: %s: avdd:%s, dvdd:%s\n", __func__, on ? "on" : "off", str_avdd, str_dvdd);
...
EXPORT_SYMBOL(sec_input_utc_marker);
