...
int sec_input_parse_dt(struct device *dev)
{
	...
	if (gpio_is_valid(pdata->irq_gpio)) {
		...
		ret = devm_gpio_request_one(dev, pdata->irq_gpio, GPIOF_IN, label);
		if (ret) {
...
static void sec_input_get_tclm_property_per_panel(struct device *dev, const char *prop_name, u32 panel_idx, int *out)
{
	struct device_node *np = dev->of_node;
	u32 dt_value;

	if (of_property_read_u32_index(np, prop_name, panel_idx, &dt_value) != 0) {
		dt_value = 0;
		input_err(true, dev, "%s: Failed to get %s property\n", __func__, prop_name);
	}
	*out = (int)dt_value;
}
...
void sec_tclm_parse_dt(struct device *dev, struct sec_tclm_data *tdata)
{
	struct device_node *np = dev->of_node;
	struct sec_ts_plat_data *pdata = dev->platform_data;
	u32 panel_idx = 0, last_panel_idx = 0;
	int afe_base_cnt;

	afe_base_cnt = of_property_count_u32_elems(np, "sec,afe_base");
    		
	if (afe_base_cnt < 0) {
		tdata->tclm_level = 0;
		tdata->afe_base = 0;
	} else {
		panel_idx = pdata->panel_index;
		last_panel_idx = afe_base_cnt - 1;

		if (panel_idx > last_panel_idx) {
			input_info(true, dev, "%s: panel_idx(%u) > last_panel_idx(%u) Use last panel_idx\n", __func__, panel_idx, last_panel_idx);
			panel_idx = last_panel_idx;
		}

		sec_input_get_tclm_property_per_panel(dev, "sec,tclm_level", panel_idx, &tdata->tclm_level);
		sec_input_get_tclm_property_per_panel(dev, "sec,afe_base", panel_idx, &tdata->afe_base);
	}

	tdata->support_tclm_test = of_property_read_bool(np, "support_tclm_test");

	input_info(true, dev, "%s: tclm_level %d, afe_base %04X\n", __func__, tdata->tclm_level, tdata->afe_base);
}
