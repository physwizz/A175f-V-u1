...
void sec_input_coord_event_fill_slot(struct device *dev, int t_id)
{
	...
	if (pdata->coord[t_id].action == SEC_TS_COORDINATE_ACTION_RELEASE) {
		...
		if (pdata->prev_coord[t_id].action == SEC_TS_COORDINATE_ACTION_NONE
			...
			sec_input_coord_report(dev, t_id);
			if (pdata->debug_flag & SEC_TS_DEBUG_PRINT_MOVE_EVENT)
				sec_input_coord_log(dev, t_id, SEC_TS_COORDINATE_ACTION_MOVE);
		}
	} else {
		input_dbg(true, dev,
...
void sec_input_store_status_event(struct device *dev, u8 *event_buff)
{
	struct sec_ts_plat_data *pdata;
	struct sec_input_ic_mode_status ic_mode_status;
	struct sec_input_event_coordinate_protocol *coord = (struct sec_input_event_coordinate_protocol *)event_buff;
	u8 t_id;
	char buffer[256];
	...
	ic_mode_status.wet = ((coord->ttype_3_2 << 2 | coord->ttype_1_0) == SEC_TS_TOUCHTYPE_WET);
	ic_mode_status.glove = ((coord->ttype_3_2 << 2 | coord->ttype_1_0) == SEC_TS_TOUCHTYPE_GLOVE);
	ic_mode_status.game = coord->game_mode;
	ic_mode_status.noise = coord->noise_status ? 1 : 0;
	ic_mode_status.data[0] = event_buff[12];
	t_id = coord->tid;

	if (pdata->ic_mode_status.data[0] == ic_mode_status.data[0] && pdata->ic_mode_status.data[1] == ic_mode_status.data[1])
		return;

	if (pdata->sec) {
		memset(buffer, 0, sizeof(buffer));
		snprintf(buffer, sizeof(buffer), "RESULT=EVENT_STATUS,WET=%d,GLOVE=%d,GAME=%d,NOISE=%d,COVER=%d,NOTE=%d,TA=%d,WIRELESS=%d,SIP=%d,LS=%d,PK=%d,CALL=%d,id=%d)",
		...
		sec_cmd_send_event_to_user(pdata->sec, NULL, buffer);
	}

	input_info(true, dev, "EVENT_STATUS: WET:%d GLOVE:%d GAME:%d NOISE:%d COVER:%d NOTE:%d TA:%d WIRELESS:%d SIP:%d LS:%d PK:%d CALL:%d (id:%d)\n",
			ic_mode_status.wet, ic_mode_status.glove, ic_mode_status.game, ic_mode_status.noise,
			ic_mode_status.cover, ic_mode_status.note,
			ic_mode_status.ta, ic_mode_status.wireless,
			ic_mode_status.sip, ic_mode_status.lowsen,
			ic_mode_status.pocket, ic_mode_status.call, t_id);
	memcpy(&pdata->ic_mode_status, &ic_mode_status, sizeof(struct sec_input_ic_mode_status));
}
EXPORT_SYMBOL(sec_input_store_status_event);
...
static void coord_log_press(struct sec_ts_plat_data *pdata, u8 t_id, const char *action)
{
	...
	input_info(true, pdata->dev,
			"[%s] tID:%d.%d x:%d y:%d z:%d M:%d m:%d loc:%s tc:%d type:%X noise:(%x,%d%d) nlvl:%d maxS:%d hid:%d fid:%d fod:%x nlvl1:%d nlvl2:%d nlvl3:%d\n",
			...
			pdata->coord[t_id].freq1_noise_level, pdata->coord[t_id].freq2_noise_level, pdata->coord[t_id].freq3_noise_level);
#else
	input_info(true, pdata->dev,
			"[%s] tID:%d.%d z:%d M:%d m:%d loc:%s tc:%d type:%X noise:(%x,%d%d) nlvl:%d maxS:%d hid:%d fid:%d fod:%x nlvl1:%d nlvl2:%d nlvl3:%d\n",
			action, t_id, input_mt_get_value(&pdata->input_dev->mt->slots[t_id], ABS_MT_TRACKING_ID),
...
#if !IS_ENABLED(CONFIG_SAMSUNG_PRODUCT_SHIP)
	input_info(true, pdata->dev,
			"[%s] tID:%d loc:%s dd:%d,%d mc:%d tc:%d lx:%d ly:%d p:%d noise:(%x,%d%d) nlvl:%d maxS:%d hid:%d fid:%d fod:%x nlvl1:%d nlvl2:%d nlvl3:%d\n",
...
#else
	input_info(true, pdata->dev,
			"[%s] tID:%d loc:%s dd:%d,%d mc:%d tc:%d p:%d noise:(%x,%d%d) nlvl:%d maxS:%d hid:%d fid:%d fod:%x nlvl1:%d nlvl2:%d nlvl3:%d\n",
			action, t_id, pdata->location,
			pdata->coord[t_id].x - pdata->coord[t_id].p_x,
			pdata->coord[t_id].y - pdata->coord[t_id].p_y,
			pdata->coord[t_id].mcount, pdata->touch_count,
			...
			pdata->coord[t_id].freq1_noise_level, pdata->coord[t_id].freq2_noise_level, pdata->coord[t_id].freq3_noise_level);
#endif
	pdata->coord[t_id].freq1_noise_level = 0;
	pdata->coord[t_id].freq2_noise_level = 0;
	pdata->coord[t_id].freq3_noise_level = 0;
}
/************************************************************
 *  720  * 1480 : <48 96 60>
