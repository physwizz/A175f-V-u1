...
bool sec_input_need_fold_off(struct sec_input_multi_device *mdev)
{
	...
	if ((mdev->device_count == MULTI_DEV_MAIN) &&
		(mdev->flip_status_current == FOLD_STATUS_FOLDING) && !plat_data->support_touchscreen_lid_switch)
		return true;
		...
		if (flip_changed) {
			if (mdev->flip_status_current == FOLD_STATUS_FOLDING) {
				if (sec_input_cmp_ic_status(mdev->dev, CHECK_LPMODE)) {
					if (plat_data->support_touchscreen_lid_switch) {
						input_info(true, mdev->dev, "%s: [%s] TSP IC LP => LP : LID support device\n",
							__func__, mdev->name);
					} else {
