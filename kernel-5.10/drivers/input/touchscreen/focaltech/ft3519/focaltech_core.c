...
static void fts_parse_finger(struct fts_ts_data *ts_data, unsigned int tid, u8 *buf)
{
	...
	fts_read_reg(FTS_GET_WETMODE, &wet_mode);
	...
	pdata->coord[tid].freq_id = buf[11] & 0x07;
	pdata->coord[tid].awd = (buf[11] >> 3) & 0x01;;
	pdata->coord[tid].fod_debug = (buf[11] >> 4) & 0x0F;
	pdata->coord[tid].freq1_noise_level = max(pdata->coord[tid].freq1_noise_level, buf[13]);
	pdata->coord[tid].freq2_noise_level = max(pdata->coord[tid].freq1_noise_level, buf[14]);
	pdata->coord[tid].freq3_noise_level = max(pdata->coord[tid].freq1_noise_level, buf[15]);
	...
	for (i = 0; i < event_num; i++) {
		...
		case SEC_TS_COORDINATE_EVENT:
			if (!data->suspended) {
				...
				sec_input_store_status_event(data->dev, event_data);
			}
			break;
