...
#define MAX_EVENT_BUFFER_COUNT			(32)
...
#define TOUCH_REF_ABNORMAL_TEST_MODE		33 /* hw cal */
...
#define ZT_FOD_STATUS_REG				0x0220
#define ZT_VI_STATUS_REG				0x0221
...
#define	REG_SNR_TEST_RESULT				0x0220
...
union coord_byte13_t {
	u8 value_u8bit;
	struct {
		u8 noise_level_hopping_1 : 8;
	} value;
};

union coord_byte14_t {
	u8 value_u8bit;
	struct {
		u8 noise_level_hopping_2 : 8;
	} value;
};

union coord_byte15_t {
	u8 value_u8bit;
	struct {
		u8 noise_level_hopping_3 : 8;
	} value;
};
...
struct point_info {
	...
	union coord_byte11_t byte11;
	union coord_byte12_t byte12;
	union coord_byte13_t byte13;
	union coord_byte14_t byte14;
	union coord_byte15_t byte15;
};
...
enum {
	STATUS_TYPE_CMDDRIVEN = 0,
	STATUS_TYPE_ERROR,
	STATUS_TYPE_INFO,
	STATUS_TYPE_USERINPUT,
	STATUS_TYPE_VENDORINFO = 7
};

enum {
	STATUS_ID_TSC_READY_STATUS = 0,
	STATUS_ID_WET_MODE,
};
...
struct zt_ts_info {
	...
	struct point_info touch_info[MAX_EVENT_BUFFER_COUNT];
