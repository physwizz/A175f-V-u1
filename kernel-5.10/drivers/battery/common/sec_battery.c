...
#endif
			break;
		case POWER_SUPPLY_EXT_PROP_DEVICE_SHUTDOWN:
			pr_info("%s : charger driver shutdown %d\n", __func__, val->intval);
			if (val->intval)
				sec_vote(battery->chgen_vote, VOTER_DEVICE_SHUTDOWN, true, SEC_BAT_CHG_MODE_CHARGING_OFF);
			else
				pr_info("%s : %d is wrong command\n", __func__, val->intval);
			break;
		default:
			return -EINVAL;
		}
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	change_sec_voter_pri(battery->chgen_vote, VOTER_DEVICE_SHUTDOWN, VOTE_PRI_10);
