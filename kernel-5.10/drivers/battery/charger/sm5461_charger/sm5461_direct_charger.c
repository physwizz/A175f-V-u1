...
static inline u32 _calc_pps_v_init_offset(struct sm_dc_info *sm_dc)
{
	...
	c_init = MIN(sm_dc->ta.c_max, ((sm_dc->target_ibus * 50) / 100));
	c_init = pps_c(MAX(c_init, sm_dc->config.ta_min_current));
	offset = (c_init * sm_dc->config.r_ttl) / 1000000;
	offset += 200;   /* add to extra initial offset */
	pr_info("%s %s: c_init_target=%dmA, v_init_offset=%dmV\n",
		sm_dc->name, __func__, c_init, offset);
...
static inline int _adjust_pps_v(struct sm_dc_info *sm_dc, int pps_v_original)
{
	...
	for (i = 0; i < 3; ++i) {
		...
		if (i == 1 && (sm_dc->config.factory_vbus_3time_adjust == 0))
			break;
#endif
		vbus_diff = adc_vbus - pps_v_original;
		if (abs(vbus_diff) <= (PPS_V_STEP * 5))
			break;

		if (vbus_diff > 0)
			sm_dc->ta.v -= pps_v(vbus_diff - (PPS_V_STEP * 5));
		else
			sm_dc->ta.v += pps_v(abs(vbus_diff));
...
#if defined(CONFIG_SEC_FACTORY)
	msleep(sm_dc->config.factory_vbus_check_delay);
#else
