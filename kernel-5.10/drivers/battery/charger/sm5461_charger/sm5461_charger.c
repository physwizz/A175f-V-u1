...
static void sm5461_init_reg_param(struct sm5461_charger *sm5461)
{
	...
	if (sm5461->pdata->en_them_adc)
		sm5461_write_reg(sm5461, SM5461_REG_THEM, 0x18); /* THEM_ADC = on */
}
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_D2D_REVERSE_VBUS:
			val->intval = sm5461_convert_adc(sm5461, SM5461_ADC_VBUS);
			break;
		default:
			return -EINVAL;
		}
									...
									struct sm5461_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np_sm5461, "sm5461,en_them_adc", &pdata->en_them_adc);
	if (ret) {
		pr_info("%s: sm5461,en_them_adc is Empty\n", __func__);
		pdata->en_them_adc = 0; /* them adc on/off */
	}
	pr_info("%s: sm5461,en_them_adc is %d\n", __func__, pdata->en_them_adc);

	ret = of_property_read_u32(np_sm5461, "sm5461,factory_vbus_check_delay", &pdata->factory_vbus_check_delay);
	if (ret) {
		pr_info("%s: sm5461,factory_vbus_check_delay is Empty\n", __func__);
		pdata->factory_vbus_check_delay = 250; /* Preset State VBUS_ADC Check Delay(ms) in factory mode*/
	}
	pr_info("%s: sm5461,factory_vbus_check_delay is %dms\n", __func__, pdata->factory_vbus_check_delay);

	ret = of_property_read_u32(np_sm5461, "sm5461,factory_vbus_3time_adjust", &pdata->factory_vbus_3time_adjust);
	if (ret) {
		pr_info("%s: sm5461,factory_vbus_3time_adjust is Empty\n", __func__);
		pdata->factory_vbus_3time_adjust = 0; /* Preset State pps_v tree-time adjust in factory mode */
	}
	pr_info("%s: sm5461,factory_vbus_3time_adjust is %d\n", __func__, pdata->factory_vbus_3time_adjust);

	ret = of_property_read_u32(np_sm5461, "sm5461,init_pps_c_rate", &pdata->init_pps_c_rate);
	if (ret) {
		pr_info("%s: sm5461,init_pps_c_rate is Empty\n", __func__);
		pdata->init_pps_c_rate = 50; /* 50% pps_c of target_ibus */
	}
	pr_info("%s: sm5461,init_pps_c_rate is %d\n", __func__, pdata->init_pps_c_rate);
...
#endif
{
	...
	pps_dc->config.factory_vbus_check_delay = pdata->factory_vbus_check_delay;
	pps_dc->config.factory_vbus_3time_adjust = pdata->factory_vbus_3time_adjust;
	...
	if (pdata->x2bat_mode) {
		...
		x2bat_dc->config.factory_vbus_check_delay = pdata->factory_vbus_check_delay;
		x2bat_dc->config.factory_vbus_3time_adjust = pdata->factory_vbus_3time_adjust;
		x2bat_dc->config.init_pps_c_rate = pdata->init_pps_c_rate;
