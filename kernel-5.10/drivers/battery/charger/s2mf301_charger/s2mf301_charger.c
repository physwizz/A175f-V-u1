...
static void s2mf301_set_regmode(struct s2mf301_charger_data *charger, int mode)
{
	...
	if (charger->pdata->change_switching_freq) {
		s2mf301_read_reg(charger->i2c, S2MF301_CHG_STATUS0, &sts0);

		if (data == CHG_MODE || (data == BUCK_ONLY_MODE && (sts0 & 0x01))) {
			pr_info("%s: modify switching frequency\n", __func__);
			s2mf301_update_reg(charger->i2c, S2MF301_CHG_CHG_OPTION15, 0x0, 0x70);
		} else if (data == BAT_ONLY_MODE || (data == BUCK_ONLY_MODE && !(sts0 & 0x01))) {
			pr_info("%s: restore switching frequency\n", __func__);
			s2mf301_update_reg(charger->i2c, S2MF301_CHG_CHG_OPTION15, 0x30, 0x70);
		}
	}
...
static void s2mf301_set_en_shipmode(struct s2mf301_charger_data *charger, bool enable)
{
	if (enable) {
		...
		s2mf301_update_reg(charger->i2c, S2MF301_CHG_SHIP_MODE_CTRL, 0x10, 0x11); // 0x62[0] = 0
	} else {
		s2mf301_set_time_bat2ship_db(charger, 0);
		s2mf301_update_reg(charger->i2c, S2MF301_CHG_SHIP_MODE_CTRL, 0x00, 0x11); // 0x62[0] = 0
	}
...
{
	u8 data1 = 0x00, data2 = 0x00;
	int i = 0;

	for (i = 0; i < 3; i++) {
		s2mf301_read_reg(charger->i2c, S2MF301_CHG_SHIP_MODE_CTRL, &data1);
		s2mf301_read_reg(charger->i2c, S2MF301_CHG_SHIP_MODE_CTRL, &data2);
	...
	if (enable)
		data2 |= 0x40;
	s2mf301_write_reg(charger->i2c, S2MF301_CHG_SHIP_MODE_CTRL, data2);
...
static void s2mf301_release_bypass(struct s2mf301_charger_data *charger)
{
	...
	/* CHIP2SYS ON */
	s2mf301_update_reg(charger->i2c, S2MF301_CHG_T_CHG_OFF5, 0x0, 0x41); //0x3C[6] = 0
	...
	s2mf301_update_reg(charger->top, S2MF301_TOP_MRSTB_RESET, 0x06, 0x8F); //0xC4[7] = 0
	/* VIO reset default */
...
static void s2mf301_check_multi_tap_off(struct s2mf301_charger_data *charger)
{
	...
	pr_info("%s: sts2(0x%02x), sts7(0x%02x), ichgin_sts(0x%02x)\n", __func__, sts2, sts7, ichgin_hi);
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		charger->attached_dev = val->intval;
		...
		if (val->intval == ATTACHED_DEV_JIG_UART_OFF_VB_MUIC) {
			...
			s2mf301_update_reg(charger->top, S2MF301_TOP_MRSTB_RESET, 0x08, 0x8F); //0xC4[7] = 0
			/* VIO RESET OFF */
			...
			s2mf301_update_reg(charger->i2c, S2MF301_CHG_T_CHG_ON5, 0x0, 0x48);
			/* QBAT OFF */
			s2mf301_update_reg(charger->i2c, S2MF301_CHG_T_CHG_ON6, 0x02, 0x02);
			/* CHIP2SYS OFF */
			s2mf301_update_reg(charger->i2c, S2MF301_CHG_T_CHG_OFF5, 0x01, 0x41); //0x3C[6] = 0
			...
			if (!factory_mode) {
				...
				s2mf301_update_reg(charger->i2c, S2MF301_CHG_T_CHG_ON5, 0x40, 0x48);
			} else {
				/* QBAT OFF DLY ON at factory mode release */
				s2mf301_update_reg(charger->i2c, S2MF301_CHG_OPEN_OTP0, 0x20, 0x20);
				/* BAT2SYS OFF at factory mode release */
			...
			if (charger->bypass) {
				if (val->intval == ATTACHED_DEV_JIG_USB_OFF_MUIC) {
					...
					if (!factory_mode) {
						pr_info("%s, User binary, 255K => async buck write\n", __func__);
						/* Asnyc buckmode enable */
						s2mf301_update_reg(charger->i2c, S2MF301_CHG_T_CHG_ON3, 0x40, 0x40);
					}
#endif
				} else
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_LSI_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_INPUT_VOLTAGE_REGULATION:
			if (val->intval) {
				...
				s2mf301_update_reg(charger->top, S2MF301_TOP_MRSTB_RESET, 0x0E, 0x8F); //0xC4[7] = 0
				/* INOK_INV disable */
...
static irqreturn_t s2mf301_otgocp_isr(int irq, void *data)
{
	struct s2mf301_charger_data *charger = data;
	u8 val;
#ifdef CONFIG_USB_HOST_NOTIFY
	...
	s2mf301_read_reg(charger->i2c, S2MF301_CHG_STATUS2, &val);
	pr_info("%s, status2 : %02x\n", __func__, val);
	if (!(val & OTG_SS_END_STATUS_MASK)) {
		/* OTG OFF, OTG OCP occurs*/
		pr_info("%s, otg ocp occurs\n", __func__);
#ifdef CONFIG_USB_HOST_NOTIFY
...
static void s2mf301_ivr_irq_work(struct work_struct *work)
{
	...
	pr_info("%s: ivr_status 0x12:0x%02x\n", __func__, ivr_state);
	...
	while ((ivr_state & IVR_STATUS_MASK) &&
		...
		if (!(ivr_state & IVR_STATUS_MASK)) {
			pr_info("%s: EXIT IVR WORK: check value (0x12:0x%02x, input current:%d)\n", __func__,
...
static int s2mf301_charger_parse_dt(struct s2mf301_charger_data *charger)
{
	...
	if (!np)
		...
		pdata->change_switching_freq = of_property_read_bool(np, "charger,change_switching_freq");
	}
...
static int s2mf301_charger_probe(struct platform_device *pdev)
{
	...
	charger->irq_otgocp = pdata->irq_base + S2MF301_CHG4_IRQ_OTG_OCP;
	ret = request_threaded_irq(charger->irq_otgocp, NULL, s2mf301_otgocp_isr, 0, "otgocp-irq", charger);
	if (ret < 0) {
		dev_err(s2mf301->dev, "%s: Fail to request otgocp in IRQ: %d: %d\n",
			__func__, charger->irq_otgocp, ret);
		goto err_reg_irq;
	}
...
static int s2mf301_charger_suspend(struct device *dev)
{
	...
	if (factory_mode && charger->attached_dev == ATTACHED_DEV_JIG_UART_OFF_VB_MUIC) {
		/* buck async mode */
...
static void s2mf301_charger_shutdown(struct platform_device *dev)
{
	...
	value.intval = true;
	psy_do_property("battery", set, POWER_SUPPLY_EXT_PROP_DEVICE_SHUTDOWN, value);
