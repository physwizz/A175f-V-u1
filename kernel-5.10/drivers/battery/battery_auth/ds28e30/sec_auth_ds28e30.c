...
static void configure_pinctrl(struct sec_auth_ds28e30_platform_data *pdata, char *state_name)
{
	...
	if (IS_ERR_OR_NULL(state)) {
		auth_err("%s: could not find pull state (%li)\n", __func__, PTR_ERR(state));
		return;
	}
...
//set cpufreq and devfreq
	...
	for (i = 0; i < 2; i++) {
		...
		if (i == 1) {
			...
			count = 0;
			while (!is_page_buf_init2 && count < 10) {
				count++;
				auth_info("%s : Retrying init page data buffer 2 retry count = %d\n", __func__, count);
			...
			//Init page data buffers
			count = 0;
			while (!is_page_buf_init && count < 10) {
				count++;
				auth_info("%s : Retrying init page data buffer retry count = %d\n", __func__, count);
				init_page_data_buffers();
			}
