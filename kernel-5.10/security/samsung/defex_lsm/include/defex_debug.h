...
#define defex_log_crit(fmt, ...)       do{if(defex_log_mask&MSG_CRIT)printf(DEFEX_LOG_TAG fmt "\n", ##__VA_ARGS__);}while(0)
#define defex_log_err(fmt, ...)        do{if(defex_log_mask&MSG_ERR)printf(DEFEX_LOG_TAG fmt "\n", ##__VA_ARGS__);}while(0)
#define defex_log_warn(fmt, ...)       do{if(defex_log_mask&MSG_WARN)printf(DEFEX_LOG_TAG fmt "\n", ##__VA_ARGS__);}while(0)
#define defex_log_info(fmt, ...)       do{if(defex_log_mask&MSG_INFO)printf(DEFEX_LOG_TAG fmt "\n", ##__VA_ARGS__);}while(0)
#define defex_log_debug(fmt, ...)      do{if(defex_log_mask&MSG_DEBUG)printf(DEFEX_LOG_TAG fmt "\n", ##__VA_ARGS__);}while(0)
#define defex_log_timeoff(fmt, ...)    do{if(defex_log_mask&MSG_TIMEOFF)printf(DEFEX_LOG_TAG fmt "\n", ##__VA_ARGS__);}while(0)
#define defex_log_blob(fmt, ...)       do{if(defex_log_mask&MSG_UNKNOWN)printf(fmt "\n", ##__VA_ARGS__);}while(0)
