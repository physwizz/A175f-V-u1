	...
	IN struct ADAPTER *prAdapter, IN uint8_t ucBssIndex)
{
	...
	while (elemsLen >= 2) {
		DBGLOG(RRM, INFO,
			"subelems %x,%x,%x eLen %x"
			, subelems[0], subelems[1], subelems[2], elemsLen);
		slen = subelems[1];
		if (slen > elemsLen - 2) {
			DBGLOG(RRM, WARN,
		...
		case BEACON_REQUEST_SUBELEM_AP_CHANNEL:
		{
			...
			DBGLOG(RRM, INFO, "apChannelsLen %x i %x"
					, data->apChannelsLen, i);
			if (data->apChannelsLen) {
				for (i = 0 ; (end - pos > 3) &&
					...
					DBGLOG(RRM, INFO,
						"end - pos %ld, %d %d %d"
						, end - pos, i
						, data->apChannelsLen, byte);
				}
				*pos = '\0';
				DBGLOG(RRM, INFO, "%x AP chnls %s", i, buf);
			}
			break;
		}
		...
		elemsLen -= 2 + slen;
		subelems += 2 + slen;
		DBGLOG(RRM, INFO, "rrmHandleBeaconReqSubelem is done");
	}
}
